...
addonTable.spells = {
  { spell = "MACRO OptiAction1", name = "VileTaint" },
  --{ spell = "SPELL Vile Taint", name = "Vile Taint" },
  { spell = "SPELL Unstable Affliction", name = "Unstable Affliction" },
  { spell = "SPELL Summon Soulkeeper", name = "Summon Soulkeeper" },
  { spell = "SPELL Summon Pet", name = "Summon Pet" },
  { spell = "SPELL Summon Darkglare", name = "Summon Darkglare" },
  { spell = "SPELL Soul Rot", name = "Soul Rot" },
  { spell = "SPELL Siphon Life", name = "Siphon Life" },
  { spell = "SPELL Shadow Bolt", name = "Shadow Bolt" },
  { spell = "SPELL Seed of Corruption", name = "Seed of Corruption" },
  { spell = "SPELL Phantom Singularity", name = "Phantom Singularity" },
  { spell = "SPELL Malefic Rapture", name = "Malefic Rapture" },
  { spell = "SPELL Haunt", name = "Haunt" },
  { spell = "SPELL Grimoire of Sacrifice", name = "Grimoire of Sacrifice" },
  { spell = "SPELL Drain Soul", name = "Drain Soul" },
  { spell = "SPELL Drain Life", name = "Drain Life" },
  { spell = "SPELL Corruption", name = "Corruption" },
  { spell = "SPELL Agony", name = "Agony" },
...
local function Precombat()
  ...
  if S.GrimoireofSacrifice:IsCastable() then
    if Cast(S.GrimoireofSacrifice, Settings.Affliction.GCDasOffGCD.GrimoireOfSacrifice) then addonTable.cast("Grimoire of Sacrifice") return "grimoire_of_sacrifice precombat 2"; end
  end
  -- snapshot_stats
  ...
  if S.Haunt:IsReady() then
    if Cast(S.Haunt, nil, nil, not Target:IsSpellInRange(S.Haunt)) then addonTable.cast("Haunt") return "haunt precombat 6"; end
  end
  -- unstable_affliction,if=!talent.soul_swap
  if S.UnstableAffliction:IsReady() and (not S.SoulSwap:IsAvailable()) then
    if Cast(S.UnstableAffliction, nil, nil, not Target:IsSpellInRange(S.UnstableAffliction)) then addonTable.cast("Unstable Affliction") return "unstable_affliction precombat 8"; end
  end
  -- shadow_bolt
  if S.ShadowBolt:IsReady() then
    if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt precombat 10"; end
  end
end
...
local function AoE()
  ...
  if S.Haunt:IsReady() and (Target:DebuffRemains(S.HauntDebuff) < 3) then
    if Cast(S.Haunt, nil, nil, not Target:IsSpellInRange(S.Haunt)) then addonTable.cast("Haunt") return "haunt aoe 2"; end
  end
  ...
  if S.VileTaint:IsReady() and ((S.SouleatersGluttony:TalentRank() == 2 and (VarMinAgony < 1.5 or S.SoulRot:CooldownRemains() <= S.VileTaint:ExecuteTime())) or (S.SouleatersGluttony:TalentRank() == 1 and S.SoulRot:CooldownRemains() <= S.VileTaint:ExecuteTime()) or (not S.SouleatersGluttony:IsAvailable() and (S.SoulRot:CooldownRemains() <= S.VileTaint:ExecuteTime() or S.VileTaint:CooldownRemains() > 25))) then
    if Cast(S.VileTaint, Settings.Affliction.GCDasOffGCD.VileTaint, nil, not Target:IsInRange(40)) then addonTable.cast("Vile Taint") return "vile_taint aoe 4"; end
  end
  ...
  if S.PhantomSingularity:IsCastable() and ((S.SoulRot:IsAvailable() and S.SoulRot:CooldownRemains() <= S.PhantomSingularity:ExecuteTime() or not S.SouleatersGluttony:IsAvailable() and (not S.SoulRot:IsAvailable() or S.SoulRot:CooldownRemains() <= S.PhantomSingularity:ExecuteTime() or S.SoulRot:CooldownRemains() >= 25)) and Target:DebuffUp(S.AgonyDebuff)) then
    if Cast(S.PhantomSingularity, Settings.Affliction.GCDasOffGCD.PhantomSingularity, nil, not Target:IsSpellInRange(S.PhantomSingularity)) then addonTable.cast("Phantom Singularity") return "phantom_singularity aoe 6"; end
  end
  ...
  if S.UnstableAffliction:IsReady() and (Target:DebuffRemains(S.UnstableAfflictionDebuff) < 5) then
    if Cast(S.UnstableAffliction, nil, nil, not Target:IsSpellInRange(S.UnstableAffliction)) then addonTable.cast("Unstable Affliction") return "unstable_affliction aoe 8"; end
  end
  ...
  if S.Agony:IsReady() and (S.AgonyDebuff:AuraActiveCount() < 8 and GCDMax * 2 + S.SoulRot:CastTime() < VarMinPS1) then
    if Everyone.CastTargetIf(S.Agony, Enemies40y, "min", EvaluateTargetIfFilterAgony, EvaluateTargetIfAgony3, not Target:IsSpellInRange(S.Agony)) then addonTable.cast("Agony") return "agony aoe 9"; end
  end
  ...
  if S.SiphonLife:IsReady() and (S.SiphonLifeDebuff:AuraActiveCount() < 6 and S.SummonDarkglare:CooldownUp() and HL.CombatTime() < 20 and GCDMax * 2 + S.SoulRot:CastTime() < VarMinPS1) then
    if Everyone.CastCycle(S.SiphonLife, Enemies40y, EvaluateCycleSiphonLife2, not Target:IsSpellInRange(S.SiphonLife)) then addonTable.cast("Siphon Life") return "siphon_life aoe 10"; end
  end
  ...
  if S.SoulRot:IsReady() and (VarVTUp and (VarPSUp or S.SouleatersGluttony:TalentRank() ~= 1) and Target:DebuffUp(S.AgonyDebuff)) then
    if Cast(S.SoulRot, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsSpellInRange(S.SoulRot)) then addonTable.cast("Soul Rot") return "soul_rot aoe 12"; end
  end
  ...
  if S.SeedofCorruption:IsReady() and (Target:DebuffRemains(S.CorruptionDebuff) < 5 and not (S.SeedofCorruption:InFlight() or Target:DebuffUp(S.SeedofCorruptionDebuff))) then
    if Cast(S.SeedofCorruption, nil, nil, not Target:IsSpellInRange(S.SeedofCorruption)) then addonTable.cast("Seed of Corruption") return "seed_of_corruption aoe 14"; end
  end
  ...
  if S.Corruption:IsReady() and (not S.SeedofCorruption:IsAvailable()) then
    if Everyone.CastTargetIf(S.Corruption, Enemies40y, "min", EvaluateTargetIfFilterCorruption, EvaluateTargetIfCorruption, not Target:IsSpellInRange(S.Corruption)) then addonTable.cast("Corruption") return "corruption aoe 15"; end
  end
  ...
  if CDsON() and S.SummonDarkglare:IsCastable() and (VarPSUp and VarVTUp and VarSRUp) then
    if Cast(S.SummonDarkglare, Settings.Affliction.GCDasOffGCD.SummonDarkglare) then addonTable.cast("Summon Darkglare") return "summon_darkglare aoe 18"; end
  end
  ...
  if S.DrainLife:IsReady() and (Player:BuffStack(S.InevitableDemiseBuff) > 30 and Player:BuffUp(S.SoulRot) and Player:BuffRemains(S.SoulRot) <= GCDMax and EnemiesCount10ySplash > 3) then
    if Everyone.CastTargetIf(S.DrainLife, Enemies40y, "min", EvaluateTargetIfFilterSoulRot, nil, not Target:IsSpellInRange(S.DrainLife)) then addonTable.cast("Drain Life") return "drain_life aoe 19"; end
  end
  ...
  if S.MaleficRapture:IsReady() and (Player:BuffUp(S.UmbrafireKindlingBuff) and (((EnemiesCount10ySplash < 6 or HL.CombatTime() < 30) and DarkglareActive()) or not S.DoomBlossom:IsAvailable())) then
    if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture aoe 20"; end
  end
  -- seed_of_corruption,if=talent.sow_the_seeds
  if S.SeedofCorruption:IsReady() and S.SowTheSeeds:IsAvailable() then
    if Cast(S.SeedofCorruption, nil, nil, not Target:IsSpellInRange(S.SeedofCorruption)) then addonTable.cast("Seed of Corruption") return "seed_of_corruption aoe 22"; end
  end
  ...
  if S.MaleficRapture:IsReady() and (((S.SummonDarkglare:CooldownRemains() > 15 or SoulShards > 3) and not S.SowTheSeeds:IsAvailable()) or Player:BuffUp(S.TormentedCrescendoBuff)) then
    if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture aoe 24"; end
  end
  ...
  if S.DrainLife:IsReady() and (Player:BuffUp(S.SoulRot) or not S.SoulRot:IsAvailable()) and Player:BuffStack(S.InevitableDemiseBuff) > 10 then
    if Everyone.CastTargetIf(S.DrainLife, Enemies40y, "min", EvaluateTargetIfFilterSoulRot, nil, not Target:IsSpellInRange(S.DrainLife)) then addonTable.cast("Drain Life") return "drain_life aoe 26"; end
  end
  ...
  if S.DrainSoul:IsReady() and (Player:BuffUp(S.NightfallBuff) and S.ShadowEmbrace:IsAvailable()) then
    if Everyone.CastCycle(S.DrainSoul, Enemies40y, EvaluateCycleDrainSoul, not Target:IsSpellInRange(S.DrainSoul)) then addonTable.cast("Drain Soul") return "drain_soul aoe 28"; end
  end
  ...
  if S.SummonSoulkeeper:IsReady() and (S.SummonSoulkeeper:Count() == 10 or S.SummonSoulkeeper:Count() > 3 and FightRemains < 10) then
    if Cast(S.SummonSoulkeeper) then addonTable.cast("Summon Soulkeeper") return "soul_strike aoe 32"; end
  end
  ...
  if S.SiphonLife:IsReady() and (S.SiphonLifeDebuff:AuraActiveCount() < 5 and (EnemiesCount10ySplash < 8 or not S.DoomBlossom:IsAvailable())) then
    if Everyone.CastCycle(S.SiphonLife, Enemies40y, EvaluateCycleSiphonLife, not Target:IsSpellInRange(S.SiphonLife)) then addonTable.cast("Siphon Life") return "siphon_life aoe 34"; end
  end
  ...
  if S.DrainSoul:IsReady() then
    if Everyone.CastCycle(S.DrainSoul, Enemies40y, EvaluateCycleDrainSoul2, not Target:IsSpellInRange(S.DrainSoul)) then addonTable.cast("Drain Soul") return "drain_soul aoe 36"; end
  end
  -- shadow_bolt
  if S.ShadowBolt:IsReady() then
    if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt aoe 38"; end
  end
end
...
local function Cleave()
  ...
  if S.MaleficRapture:IsReady() and (S.DreadTouch:IsAvailable() and Target:DebuffRemains(S.DreadTouchDebuff) < 2 and (Target:DebuffRemains(S.AgonyDebuff) > GCDMax and Target:DebuffUp(S.CorruptionDebuff) and (not S.SiphonLife:IsAvailable() or Target:DebuffUp(S.SiphonLifeDebuff)) and Target:DebuffUp(S.UnstableAfflictionDebuff)) and (not S.PhantomSingularity:IsAvailable() or S.PhantomSingularity:CooldownDown()) and (not S.VileTaint:IsAvailable() or S.VileTaint:CooldownDown()) and (not S.SoulRot:IsAvailable() or S.SoulRot:CooldownDown()) or SoulShards > 4) then
    if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture cleave 2"; end
  end
  ...
  if S.VileTaint:IsReady() and (not S.SoulRot:IsAvailable() or (VarMinAgony < 1.5 or S.SoulRot:CooldownRemains() <= S.VileTaint:ExecuteTime() + GCDMax) or not S.SouleatersGluttony:IsAvailable() and S.SoulRot:CooldownRemains() >= 12) then
    if Cast(S.VileTaint, Settings.Affliction.GCDasOffGCD.VileTaint, nil, not Target:IsInRange(40)) then addonTable.cast("Vile Taint") return "vile_taint cleave 4"; end
  end
  ...
  if S.PhantomSingularity:IsReady() and ((S.SoulRot:CooldownRemains() <= S.PhantomSingularity:ExecuteTime() or not S.SouleatersGluttony:IsAvailable() and (not S.SoulRot:IsAvailable() or S.SoulRot:CooldownRemains() <= S.PhantomSingularity:ExecuteTime() or S.SoulRot:CooldownRemains() >= 25)) and S.AgonyDebuff:AuraActiveCount() >= 2) then
    if Cast(S.PhantomSingularity, Settings.Affliction.GCDasOffGCD.PhantomSingularity, nil, not Target:IsSpellInRange(S.PhantomSingularity)) then addonTable.cast("Phantom Singularity") return "phantom_singularity cleave 6"; end
  end
  ...
  if S.SoulRot:IsReady() and ((VarVTUp and (VarPSUp or S.SouleatersGluttony:TalentRank() ~= 1)) and S.AgonyDebuff:AuraActiveCount() >= 2) then
    if Cast(S.SoulRot, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsSpellInRange(S.SoulRot)) then addonTable.cast("Soul Rot") return "soul_rot cleave 8"; end
  end
  ...
  if S.Agony:IsReady() then
    if Everyone.CastTargetIf(S.Agony, Enemies40y, "min", EvaluateTargetIfFilterAgony, EvaluateTargetIfAgony4, not Target:IsSpellInRange(S.Agony)) then addonTable.cast("Agony") return "agony cleave 10"; end
  end
  ...
  if S.UnstableAffliction:IsReady() and (Target:DebuffRemains(S.UnstableAfflictionDebuff) < 5 and FightRemains > 3) then
    if Cast(S.UnstableAffliction, nil, nil, not Target:IsSpellInRange(S.UnstableAffliction)) then addonTable.cast("Unstable Affliction") return "unstable_affliction cleave 12"; end
  end
  ...
  if S.SeedofCorruption:IsReady() and (not S.AbsoluteCorruption:IsAvailable() and Target:DebuffRemains(S.CorruptionDebuff) < 5 and S.SowTheSeeds:IsAvailable() and CanSeed(Enemies40y)) then
    if Cast(S.SeedofCorruption, nil, nil, not Target:IsSpellInRange(S.SeedofCorruption)) then addonTable.cast("Seed of Corruption") return "seed_of_corruption cleave 14"; end
  end
  -- haunt,if=debuff.haunt.remains<3
  if S.Haunt:IsReady() and (Target:DebuffRemains(S.HauntDebuff) < 3) then
    if Cast(S.Haunt, nil, nil, not Target:IsSpellInRange(S.Haunt)) then addonTable.cast("Haunt") return "haunt cleave 16"; end
  end
  ...
  if S.Corruption:IsReady() and (not (S.SeedofCorruption:InFlight() or Target:DebuffRemains(S.SeedofCorruptionDebuff) > 0) and FightRemains > 5) then
    if Everyone.CastTargetIf(S.Corruption, Enemies40y, "min", EvaluateTargetIfFilterCorruption, EvaluateTargetIfCorruption, not Target:IsSpellInRange(S.Corruption)) then addonTable.cast("Corruption") return "corruption cleave 18"; end
  end
  -- siphon_life,target_if=min:remains,if=refreshable&fight_remains>5
  if S.SiphonLife:IsReady() and (FightRemains > 5) then
    if Everyone.CastTargetIf(S.SiphonLife, Enemies40y, "min", EvaluateTargetIfFilterSiphonLife, EvaluateTargetIfSiphonLife, not Target:IsSpellInRange(S.SiphonLife)) then addonTable.cast("Siphon Life") return "siphon_life cleave 20"; end
  end
  ...
  if S.SummonDarkglare:IsReady() and ((not S.ShadowEmbrace:IsAvailable() or Target:DebuffStack(S.ShadowEmbraceDebuff) == 3) and VarPSUp and VarVTUp and VarSRUp) then
    if Cast(S.SummonDarkglare, Settings.Affliction.GCDasOffGCD.SummonDarkglare) then addonTable.cast("Summon Darkglare") return "summon_darkglare cleave 22"; end
  end
  ...
  if S.MaleficRapture:IsReady() and (S.TormentedCrescendo:IsAvailable() and Player:BuffStack(S.TormentedCrescendoBuff) == 1 and SoulShards > 3) then
    if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture cleave 24"; end
  end
  ...
  if S.DrainSoul:IsReady() and (S.ShadowEmbrace:IsAvailable() and (Target:DebuffStack(S.ShadowEmbraceDebuff) < 3 or Target:DebuffRemains(S.ShadowEmbraceDebuff) < 3)) then
    if Cast(S.DrainSoul, nil, nil, not Target:IsSpellInRange(S.DrainSoul)) then addonTable.cast("Drain Soul") return "drain_soul cleave 26"; end
  end
  ...
  if S.MaleficRapture:IsReady() and (not S.DreadTouch:IsAvailable() and Player:BuffUp(S.TormentedCrescendoBuff)) then
    if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture cleave 30"; end
  end
  -- malefic_rapture,if=variable.cd_dots_up|variable.vt_ps_up
  if S.MaleficRapture:IsReady() and (VarCDDoTsUp or VarVTPSUp) then
    if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture cleave 32"; end
  end
  -- malefic_rapture,if=soul_shard>3
  if S.MaleficRapture:IsReady() and (SoulShards > 3) then
    if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture cleave 34"; end
  end
  ...
  if S.DrainLife:IsReady() and (Player:BuffStack(S.InevitableDemiseBuff) > 48 or Player:BuffStack(S.InevitableDemiseBuff) > 20 and FightRemains < 4) then
    if Cast(S.DrainLife, nil, nil, not Target:IsSpellInRange(S.DrainLife)) then addonTable.cast("Drain Life") return "drain_life cleave 36"; end
  end
  ...
  if S.DrainLife:IsReady() and (Player:BuffUp(S.SoulRot) and Player:BuffStack(S.InevitableDemiseBuff) > 30) then
    if Cast(S.DrainLife, nil, nil, not Target:IsSpellInRange(S.DrainLife)) then addonTable.cast("Drain Life") return "drain_life cleave 38"; end
  end
  -- agony,target_if=refreshable
  if S.Agony:IsReady() then
    if Everyone.CastCycle(S.Agony, Enemies40y, EvaluateCycleAgonyRefreshable, not Target:IsSpellInRange(S.Agony)) then addonTable.cast("Agony") return "agony cleave 40"; end
  end
  -- corruption,target_if=refreshable
  if S.Corruption:IsCastable() then
    if Everyone.CastCycle(S.Corruption, Enemies40y, EvaluateCycleCorruptionRefreshable, not Target:IsSpellInRange(S.Corruption)) then addonTable.cast("Corruption") return "corruption cleave 42"; end
  end
  -- malefic_rapture,if=soul_shard>1
  if S.MaleficRapture:IsReady() and (SoulShards > 1) then
    if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture cleave 44"; end
  end
...
addonTable.resetPixels()
  ...
  if S.SummonPet:IsCastable() then
    if Cast(S.SummonPet, Settings.Affliction.GCDasOffGCD.SummonPet) then addonTable.cast("Summon Pet") return "summon_pet ooc"; end
  end
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.MaleficRapture:IsReady() and (S.DreadTouch:IsAvailable() and Target:DebuffRemains(S.DreadTouchDebuff) < 2 and (Target:DebuffRemains(S.AgonyDebuff) > GCDMax and Target:DebuffUp(S.CorruptionDebuff) and (not S.SiphonLife:IsAvailable() or Target:DebuffUp(S.SiphonLifeDebuff)) and Target:DebuffUp(S.UnstableAfflictionDebuff)) and (not S.PhantomSingularity:IsAvailable() or S.PhantomSingularity:CooldownDown()) and (not S.VileTaint:IsAvailable() or S.VileTaint:CooldownDown()) and (not S.SoulRot:IsAvailable() or S.SoulRot:CooldownDown())) then
      if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture main 2"; end
    end
    -- malefic_rapture,if=fight_remains<4
    if S.MaleficRapture:IsReady() and (FightRemains < 4) then
      if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture main 4"; end
    end
    -- vile_taint,if=!talent.soul_rot|(variable.min_agony<1.5|cooldown.soul_rot.remains<=execute_time+gcd.max)|talent.souleaters_gluttony.rank<1&cooldown.soul_rot.remains>=12
    if S.VileTaint:IsReady() and (not S.SoulRot:IsAvailable() or (VarMinAgony < 1.5 or S.SoulRot:CooldownRemains() <= S.VileTaint:ExecuteTime() + GCDMax) or not S.SouleatersGluttony:IsAvailable() and S.SoulRot:CooldownRemains() >= 12) then
      if Cast(S.VileTaint, Settings.Affliction.GCDasOffGCD.VileTaint, nil, not Target:IsInRange(40)) then addonTable.cast("Vile Taint") return "vile_taint main 6"; end
    end
    -- phantom_singularity,if=(cooldown.soul_rot.remains<=execute_time|talent.souleaters_gluttony.rank<1&(!talent.soul_rot|cooldown.soul_rot.remains<=execute_time|cooldown.soul_rot.remains>=25))&dot.agony.ticking
    if S.PhantomSingularity:IsCastable() and ((S.SoulRot:CooldownRemains() <= S.PhantomSingularity:ExecuteTime() or not S.SouleatersGluttony:IsAvailable() and (not S.SoulRot:IsAvailable() or S.SoulRot:CooldownRemains() <= S.PhantomSingularity:ExecuteTime() or S.SoulRot:CooldownRemains() >= 25)) and Target:DebuffUp(S.AgonyDebuff)) then
      if Cast(S.PhantomSingularity, Settings.Affliction.GCDasOffGCD.PhantomSingularity, nil, not Target:IsSpellInRange(S.PhantomSingularity)) then addonTable.cast("Phantom Singularity") return "phantom_singularity main 8"; end
    end
    -- soul_rot,if=(variable.vt_up&(variable.ps_up|talent.souleaters_gluttony.rank!=1))&dot.agony.ticking
    if S.SoulRot:IsReady() and (VarVTUp and (VarPSUp or S.SouleatersGluttony:TalentRank() ~= 1) and Target:DebuffUp(S.AgonyDebuff)) then
      if Cast(S.SoulRot, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsSpellInRange(S.SoulRot)) then addonTable.cast("Soul Rot") return "soul_rot main 10"; end
    end
    ...
    if S.Agony:IsCastable() and ((not S.VileTaint:IsAvailable() or Target:DebuffRemains(S.AgonyDebuff) < S.VileTaint:CooldownRemains() + S.VileTaint:CastTime()) and Target:DebuffRemains(S.AgonyDebuff) < 5 and FightRemains > 5) then
      if Cast(S.Agony, nil, nil, not Target:IsSpellInRange(S.Agony)) then addonTable.cast("Agony") return "agony main 12"; end
    end
    -- unstable_affliction,if=remains<5&fight_remains>3
    if S.UnstableAffliction:IsReady() and (Target:DebuffRemains(S.UnstableAfflictionDebuff) < 5 and FightRemains > 3) then
      if Cast(S.UnstableAffliction, nil, nil, not Target:IsSpellInRange(S.UnstableAffliction)) then addonTable.cast("Unstable Affliction") return "unstable_affliction main 14"; end
    end
    -- haunt,if=debuff.haunt.remains<5
    if S.Haunt:IsReady() and (Target:DebuffRemains(S.HauntDebuff) < 5) then
      if Cast(S.Haunt, nil, nil, not Target:IsSpellInRange(S.Haunt)) then addonTable.cast("Haunt") return "haunt main 16"; end
    end
    ...
    if S.Corruption:IsCastable() and (Target:DebuffRefreshable(S.CorruptionDebuff) and FightRemains > 5) then
      if Cast(S.Corruption, nil, nil, not Target:IsSpellInRange(S.Corruption)) then addonTable.cast("Corruption") return "corruption main 18"; end
    end
    ...
    if S.SiphonLife:IsCastable() and (Target:DebuffRefreshable(S.SiphonLifeDebuff) and FightRemains > 5) then
      if Cast(S.SiphonLife, nil, nil, not Target:IsSpellInRange(S.SiphonLife)) then addonTable.cast("Siphon Life") return "siphon_life main 20"; end
    end
    -- summon_darkglare,if=(!talent.shadow_embrace|debuff.shadow_embrace.stack=3)&variable.ps_up&variable.vt_up&variable.sr_up|cooldown.invoke_power_infusion_0.duration>0&cooldown.invoke_power_infusion_0.up&!talent.soul_rot
    if S.SummonDarkglare:IsReady() and ((not S.ShadowEmbrace:IsAvailable() or Target:DebuffStack(S.ShadowEmbraceDebuff) == 3) and VarPSUp and VarVTUp and VarSRUp) then
      if Cast(S.SummonDarkglare, Settings.Affliction.GCDasOffGCD.SummonDarkglare) then addonTable.cast("Summon Darkglare") return "summon_darkglare main 22"; end
    end
    -- drain_soul,interrupt=1,if=talent.shadow_embrace&(debuff.shadow_embrace.stack<3|debuff.shadow_embrace.remains<3)
    if S.DrainSoul:IsReady() and (S.ShadowEmbrace:IsAvailable() and (Target:DebuffStack(S.ShadowEmbraceDebuff) < 3 or Target:DebuffRemains(S.ShadowEmbraceDebuff) < 3)) then
      if Cast(S.DrainSoul, nil, nil, not Target:IsSpellInRange(S.DrainSoul)) then addonTable.cast("Drain Soul") return "drain_soul main 24"; end
    end
    ...
    if S.ShadowBolt:IsReady() and (S.ShadowEmbrace:IsAvailable() and (Target:DebuffStack(S.ShadowEmbraceDebuff) < 3 or Target:DebuffRemains(S.ShadowEmbraceDebuff) < 3)) then
      if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt main 26"; end
    end
    if S.MaleficRapture:IsReady() and (
      ...
      (S.TormentedCrescendo:IsAvailable() and S.Nightfall:IsAvailable() and Player:BuffUp(S.TormentedCrescendoBuff) and Player:BuffUp(S.NightfallBuff))
        ...
        if Cast(S.MaleficRapture, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Malefic Rapture") return "malefic_rapture main 28"; end
    end
    -- drain_life,if=buff.inevitable_demise.stack>48|buff.inevitable_demise.stack>20&fight_remains<4
    if S.DrainLife:IsReady() and (Player:BuffStack(S.InevitableDemiseBuff) > 48 or Player:BuffStack(S.InevitableDemiseBuff) > 20 and FightRemains < 4) then
      if Cast(S.DrainLife, nil, nil, not Target:IsSpellInRange(S.DrainLife)) then addonTable.cast("Drain Life") return "drain_life main 30"; end
    end
    -- drain_soul,if=buff.nightfall.react
    if S.DrainSoul:IsReady() and (Player:BuffUp(S.NightfallBuff)) then
      if Cast(S.DrainSoul, nil, nil, not Target:IsSpellInRange(S.DrainSoul)) then addonTable.cast("Drain Soul") return "drain_soul main 32"; end
    end
    -- shadow_bolt,if=buff.nightfall.react
    if S.ShadowBolt:IsReady() and (Player:BuffUp(S.NightfallBuff)) then
      if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt main 34"; end
    end
    -- drain_soul,interrupt=1
    if S.DrainSoul:IsReady() then
      if Cast(S.DrainSoul, nil, nil, not Target:IsSpellInRange(S.DrainSoul)) then addonTable.cast("Drain Soul") return "drain_soul main 36"; end
    end
    -- shadow_bolt
    if S.ShadowBolt:IsReady() then
      if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt main 38"; end
    end
  end
end
