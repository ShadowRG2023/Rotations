...
-- Assume Sacrificing Felhunter for Interrupt
local settings = {
  Interrupt = true,
}
...
-- DO NOT REMOVE 
addonTable.spells = {
  { spell = "MACRO OptiAction1", name = "Rain of Fire" },
  { spell = "MACRO OptiAction2", name = "SummonInfernal" },
  { spell = "MACRO OptiAction3", name = "Cataclysm" }, 
  { spell = "SPELL Summon Soulkeeper", name = "Summon Soulkeeper" },
  { spell = "SPELL Summon Pet", name = "Summon Pet" },
  --{ spell = "SPELL Summon Infernal", name = "Summon Infernal" },
  { spell = "SPELL Soul Fire", name = "Soul Fire" },
  --{ spell = "SPELL Rain of Fire", name = "Rain of Fire" },
  { spell = "SPELL Incinerate", name = "Incinerate" },
  { spell = "SPELL Immolate", name = "Immolate" },
  { spell = "SPELL Havoc", name = "Havoc" },
  { spell = "SPELL Grimoire of Sacrifice", name = "Grimoire of Sacrifice" },
  { spell = "SPELL Dimensional Rift", name = "Dimensional Rift" },
  { spell = "SPELL Conflagrate", name = "Conflagrate" },
  { spell = "SPELL Chaos Bolt", name = "Chaos Bolt" },
  { spell = "SPELL Channel Demonfire", name = "Channel Demonfire" },
  { spell = "SPELL Cataclysm", name = "Cataclysm" },
  { spell = "SPELL Spell Lock", name = "Spell Lock" },
  -- RACIALS
...
local function Interrupt()
  if Target:IsInterruptible() then
    if (Player:BuffUp(S.GrimoireofSacrifice)) then
      if S.SpellLock:IsCastable() and Target:IsSpellInRange(S.SpellLock) and addonTable.config.Interrupt then
        if Cast(S.SpellLock) then
          addonTable.cast("Spell Lock")
          return "spell lock 1"
        end
      end
    end
...
local function Precombat()
  ...
  if S.GrimoireofSacrifice:IsReady() then
    if Cast(S.GrimoireofSacrifice, Settings.Destruction.GCDasOffGCD.GrimoireOfSacrifice) then addonTable.cast("Grimoire of Sacrifice") return "grimoire_of_sacrifice precombat 2"; end
  end
  -- snapshot_stats
  -- soul_fire
  if S.SoulFire:IsReady() and (not Player:IsCasting(S.SoulFire)) then
    if Cast(S.SoulFire, nil, nil, not Target:IsSpellInRange(S.SoulFire)) then addonTable.cast("Soul Fire") return "soul_fire precombat 4"; end
  end
  -- cataclysm,if=raid_event.adds.in>15
  if S.Cataclysm:IsCastable() then
    if Cast(S.Cataclysm, Settings.Destruction.GCDasOffGCD.Cataclysm, nil, not Target:IsInRange(40)) then addonTable.cast("Cataclysm") return "cataclysm precombat 6"; end
  end
  -- incinerate
  if S.Incinerate:IsCastable() and (not Player:IsCasting(S.Incinerate)) then
    if Cast(S.Incinerate, nil, nil, not Target:IsSpellInRange(S.Incinerate)) then addonTable.cast("Incinerate") return "incinerate precombat 8"; end
  end
end
...
local function oGCD()
  ...
  if S.Berserking:IsCastable() and (VarInfernalActive or not S.SummonInfernal:IsAvailable() or (FightRemains < (S.SummonInfernal:CooldownRemains() + 12) and (FightRemains > 12)) or FightRemains < S.SummonInfernal:CooldownRemains()) then
    if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Berserking") return "berserking ogcd 4"; end
  end
  ...
  if S.BloodFury:IsCastable() and (VarInfernalActive or not S.SummonInfernal:IsAvailable() or (FightRemains < (S.SummonInfernal:CooldownRemains() + 10 + 15) and (FightRemains > 15)) or FightRemains < S.SummonInfernal:CooldownRemains()) then
    if Cast(S.BloodFury, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Blood Fury") return "blood_fury ogcd 6"; end
  end
  ...
  if S.Fireblood:IsCastable() and (VarInfernalActive or not S.SummonInfernal:IsAvailable() or (FightRemains < (S.SummonInfernal:CooldownRemains() + 10 + 8) and (FightRemains > 8)) or FightRemains < S.SummonInfernal:CooldownRemains()) then
    if Cast(S.Fireblood, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Fireblood") return "fireblood ogcd 8"; end
  end
  ...
  if S.AncestralCall:IsCastable() and (VarInfernalActive or not S.SummonInfernal:IsAvailable() or (FightRemains < (S.SummonInfernal:CooldownRemains() + 15) and (FightRemains > 15)) or FightRemains < S.SummonInfernal:CooldownRemains()) then
    if Cast(S.AncestralCall, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Ancestral Call") return "ancestral_call ogcd 10"; end
  end
end
...
local function Havoc()
  ...
  if S.Conflagrate:IsCastable() and (S.Backdraft:IsAvailable() and Player:BuffDown(S.BackdraftBuff) and SoulShards >= 1 and SoulShards <= 4) then
    if Cast(S.Conflagrate, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Conflagrate") return "conflagrate havoc 2"; end
  end
  ...
  if S.SoulFire:IsCastable() and (S.SoulFire:CastTime() < VarHavocRemains and SoulShards < 2.5) then
    if Cast(S.SoulFire, nil, nil, not Target:IsSpellInRange(S.SoulFire)) then addonTable.cast("Soul Fire") return "soul_fire havoc 4"; end
  end
  ...
  if S.ChannelDemonfire:IsReady() and (SoulShards < 4.5 and S.RagingDemonfire:TalentRank() == 2) then
    if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire havoc 6"; end
  end
  ...
  if S.Immolate:IsCastable() and (SoulShards < 4.5) then
    if Everyone.CastTargetIf(S.Immolate, Enemies40y, "min", EvaluateTargetIfFilterImmolate, EvaluateTargetIfImmolateHavoc, not Target:IsSpellInRange(S.Immolate)) then addonTable.cast("Immolate") return "immolate havoc 8"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (((S.CryHavoc:IsAvailable() and not S.Inferno:IsAvailable()) or not S.RainofFire:IsAvailable()) and S.ChaosBolt:CastTime() < VarHavocRemains) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt havoc 10"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (S.ChaosBolt:CastTime() < VarHavocRemains and (EnemiesCount8ySplash <= 3 - num(S.Inferno:IsAvailable()) + num(S.Chaosbringer:IsAvailable() and not S.Inferno:IsAvailable()))) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt havoc 12"; end
  end
  ...
  if S.RainofFire:IsReady() and (EnemiesCount8ySplash >= 3 and S.Inferno:IsAvailable()) then
    if Cast(S.RainofFire, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Rain of Fire") return "rain_of_fire havoc 14"; end
  end
  ...
  if S.RainofFire:IsReady() and (EnemiesCount8ySplash >= 4 - num(S.Inferno:IsAvailable()) + num(S.Chaosbringer:IsAvailable())) then
    if Cast(S.RainofFire, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Rain of Fire") return "rain_of_fire havoc 16"; end
  end
  ...
  if S.RainofFire:IsReady() and (EnemiesCount8ySplash > 2 and (S.AvatarofDestruction:IsAvailable() or (S.RainofChaos:IsAvailable() and Player:BuffUp(S.RainofChaosBuff))) and S.Inferno:IsAvailable()) then
    if Cast(S.RainofFire, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Rain of Fire") return "rain_of_fire havoc 18"; end
  end
  -- channel_demonfire,if=soul_shard<4.5
  if S.ChannelDemonfire:IsReady() and (SoulShards < 4.5) then
    if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire havoc 20"; end
  end
  -- conflagrate,if=!talent.backdraft
  if S.Conflagrate:IsCastable() and not S.Backdraft:IsAvailable() then
    if Cast(S.Conflagrate, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Conflagrate") return "conflagrate havoc 22"; end
  end
  ...
  if CDsON() and S.DimensionalRift:IsCastable() and (SoulShards < 4.7 and (S.DimensionalRift:Charges() > 2 or FightRemains < S.DimensionalRift:Cooldown())) then
    if Cast(S.DimensionalRift, Settings.Destruction.GCDasOffGCD.DimensionalRift, nil, not Target:IsSpellInRange(S.DimensionalRift)) then addonTable.cast("Dimensional Rift") return "dimensional_rift havoc 24"; end
  end
  -- incinerate,if=cast_time<havoc_remains
  if S.Incinerate:IsCastable() and (S.Incinerate:CastTime() < VarHavocRemains) then
    if Cast(S.Incinerate, nil, nil, not Target:IsSpellInRange(S.Incinerate)) then addonTable.cast("Incinerate") return "incinerate havoc 26"; end
  end
end
...
local function Aoe()
  ...
  -- dimensional_rift,if=soul_shard<4.7&(charges>2|fight_remains<cooldown.dimensional_rift.duration)
  if CDsON() and S.DimensionalRift:IsCastable() and (SoulShards < 4.7 and (S.DimensionalRift:Charges() > 2 or FightRemains < S.DimensionalRift:Cooldown())) then
    if Cast(S.DimensionalRift, Settings.Destruction.GCDasOffGCD.DimensionalRift, nil, not Target:IsSpellInRange(S.DimensionalRift)) then addonTable.cast("Dimensional Rift") return "dimensional_rift aoe 2"; end
  end
  ...
  if S.RainofFire:IsReady() and (InfernalTime() > 0 or BlasphemyTime() > 0) then
    if Cast(S.RainofFire, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Rain of Fire") return "rain_of_fire aoe 4"; end
  end
  -- rain_of_fire,if=fight_remains<12
  if S.RainofFire:IsReady() and (FightRemains < 12) then
    if Cast(S.RainofFire, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Rain of Fire") return "rain_of_fire aoe 6"; end
  end
  ...
  if S.RainofFire:IsReady() and (SoulShards >= (4.5 - 0.1 * S.ImmolateDebuff:AuraActiveCount()) and HL.CombatTime() > 5) then
    if Cast(S.RainofFire, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Rain of Fire") return "rain_of_fire aoe 8"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (SoulShards > 3.5 - (0.1 * EnemiesCount8ySplash) and not S.RainofFire:IsAvailable()) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt aoe 10"; end
  end
  -- cataclysm,if=raid_event.adds.in>15
  if CDsON() and S.Cataclysm:IsCastable() then
    if Cast(S.Cataclysm, Settings.Destruction.GCDasOffGCD.Cataclysm, nil, not Target:IsSpellInRange(S.Cataclysm)) then addonTable.cast("Cataclysm") return "cataclysm aoe 12"; end
  end
  ...
  if S.Havoc:IsReady() and (S.SummonInfernal:CooldownDown() or not S.SummonInfernal:IsAvailable() or (S.Inferno:IsAvailable() and EnemiesCount8ySplash > 4)) then
    --if Everyone.CastTargetIf(S.Havoc, Enemies40y, "min", EvaluateTargetIfFilterHavoc, EvaluateTargetIfHavoc, not Target:IsSpellInRange(S.Havoc)) then addonTable.cast("Havoc") return "havoc aoe 14"; end
  ...
  if S.Immolate:IsCastable() and (S.ImmolateDebuff:AuraActiveCount() <= 4) then
    if Everyone.CastTargetIf(S.Immolate, Enemies40y, "min", EvaluateTargetIfFilterImmolate, EvaluateTargetIfImmolateAoE, not Target:IsSpellInRange(S.Immolate)) then addonTable.cast("Immolate") return "immolate aoe 16"; end
  end
  ...
  if S.ChannelDemonfire:IsReady() and (Target:DebuffRemains(S.ImmolateDebuff) > ChannelDemonfireExecuteTime() and S.RagingDemonfire:IsAvailable()) then
    if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire aoe 18"; end
  end
  ...
  if S.SummonSoulkeeper:IsCastable() and (S.SummonSoulkeeper:Count() == 10 or S.SummonSoulkeeper:Count() > 3 and FightRemains < 10) then
    if Cast(S.SummonSoulkeeper, Settings.Destruction.GCDasOffGCD.SummonSoulkeeper) then addonTable.cast("Summon Soulkeeper") return "summon_soulkeeper aoe 20"; end
  end
  -- call_action_list,name=ogcd
  if CDsON() then
    local ShouldReturn = oGCD(); if ShouldReturn then return ShouldReturn; end
  ...
  if CDsON() and S.SummonInfernal:IsCastable() then
    if Cast(S.SummonInfernal, Settings.Destruction.GCDasOffGCD.SummonInfernal) then addonTable.cast("Summon Infernal") return "summon_infernal aoe 22"; end
  end
  ...
  if S.RainofFire:IsReady() and (Target:DebuffDown(S.PyrogenicsDebuff) and EnemiesCount8ySplash <= 4) then
    if Cast(S.RainofFire, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Rain of Fire") return "rain_of_fire aoe 24"; end
  end
  ...
  if S.ChannelDemonfire:IsReady() and (Target:DebuffRemains(S.ImmolateDebuff) > ChannelDemonfireExecuteTime()) then
    if Cast(S.ChannelDemonfire, nil, nil, not Target:IsSpellInRange(S.ChannelDemonfire)) then addonTable.cast("Channel Demonfire") return "channel_demonfire aoe 26"; end
  end
  ...
  if S.Immolate:IsCastable() then
    if Everyone.CastTargetIf(S.Immolate, Enemies40y, "min", EvaluateTargetIfFilterImmolate, EvaluateTargetIfImmolateAoE2, not Target:IsSpellInRange(S.Immolate)) then addonTable.cast("Immolate") return "immolate aoe 28"; end
  end
  ...
  if S.Immolate:IsCastable() then
    if Everyone.CastTargetIf(S.Immolate, Enemies40y, "min", EvaluateTargetIfFilterImmolate, EvaluateTargetIfImmolateAoE3, not Target:IsSpellInRange(S.Immolate)) then addonTable.cast("Immolate") return "immolate aoe 30"; end
  end
  -- dimensional_rift
  if CDsON() and S.DimensionalRift:IsCastable() then
    if Cast(S.DimensionalRift, Settings.Destruction.GCDasOffGCD.DimensionalRift, nil, not Target:IsSpellInRange(S.DimensionalRift)) then addonTable.cast("Dimensional Rift") return "dimensional_rift aoe 32"; end
  end
  -- soul_fire,if=buff.backdraft.up
  if S.SoulFire:IsCastable() and (Player:BuffUp(S.BackdraftBuff)) then
    if Cast(S.SoulFire, nil, nil, not Target:IsSpellInRange(S.SoulFire)) then addonTable.cast("Soul Fire") return "soul_fire aoe 34"; end
  end
  ...
  if S.Incinerate:IsCastable() and (S.FireandBrimstone:IsAvailable() and Player:BuffUp(S.BackdraftBuff)) then
    if Cast(S.Incinerate, nil, nil, not Target:IsSpellInRange(S.Incinerate)) then addonTable.cast("Incinerate") return "incinerate aoe 36"; end
  end
  ...
  if S.Conflagrate:IsCastable() and (Player:BuffStack(S.BackdraftBuff) < 2 or not S.Backdraft:IsAvailable()) then
    if Cast(S.Conflagrate, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Conflagrate") return "conflagrate aoe 38"; end
  end
  -- incinerate
  if S.Incinerate:IsCastable() then
    if Cast(S.Incinerate, nil, nil, not Target:IsSpellInRange(S.Incinerate)) then addonTable.cast("Incinerate") return "incinerate aoe 40"; end
  end
end
...
local function Cleave()
  ...
  if S.Conflagrate:IsCastable() and ((S.RoaringBlaze:IsAvailable() and Target:DebuffRemains(S.RoaringBlazeDebuff) < 1.5) or S.Conflagrate:Charges() == S.Conflagrate:MaxCharges() and not VarPoolSoulShards) then
    if Cast(S.Conflagrate, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Conflagrate") return "conflagrate cleave 2"; end
  end
  -- dimensional_rift,if=soul_shard<4.7&(charges>2|fight_remains<cooldown.dimensional_rift.duration)
  if CDsON() and S.DimensionalRift:IsCastable() and (SoulShards < 4.7 and (S.DimensionalRift:Charges() > 2 or FightRemains < S.DimensionalRift:Cooldown())) then
    if Cast(S.DimensionalRift, Settings.Destruction.GCDasOffGCD.DimensionalRift, nil, not Target:IsSpellInRange(S.DimensionalRift)) then addonTable.cast("Dimensional Rift") return "dimensional_rift cleave 4"; end
  end
  -- cataclysm,if=raid_event.adds.in>15
  if CDsON() and S.Cataclysm:IsCastable() then
    if Cast(S.Cataclysm, Settings.Destruction.GCDasOffGCD.Cataclysm, nil, not Target:IsSpellInRange(S.Cataclysm)) then addonTable.cast("Cataclysm") return "cataclysm cleave 6"; end
  end
  ...
  if S.ChannelDemonfire:IsReady() and (S.RagingDemonfire:IsAvailable() and S.ImmolateDebuff:AuraActiveCount() == 2) then
    if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire cleave 8"; end
  end
  ...
  if S.SoulFire:IsCastable() and (SoulShards <= 3.5 and (Target:DebuffRemains(S.RoaringBlazeDebuff) > S.SoulFire:CastTime() + S.SoulFire:TravelTime() or not S.RoaringBlaze:IsAvailable() and Player:BuffUp(S.BackdraftBuff)) and not VarPoolSoulShards) then
    if Cast(S.SoulFire, nil, nil, not Target:IsSpellInRange(S.SoulFire)) then addonTable.cast("Soul Fire") return "soul_fire cleave 10"; end
  end
  ...
  if S.Immolate:IsCastable() then
    if Everyone.CastTargetIf(S.Immolate, Enemies40y, "min", EvaluateTargetIfFilterImmolate, EvaluateTargetIfImmolateCleave, not Target:IsSpellInRange(S.Immolate)) then addonTable.cast("Immolate") return "immolate cleave 12"; end
  end
  ...
  if S.Havoc:IsCastable() and (S.SummonInfernal:CooldownDown() or not S.SummonInfernal:IsAvailable()) then
    --if Everyone.CastTargetIf(S.Havoc, Enemies40y, "min", EvaluateTargetIfFilterHavoc, EvaluateTargetIfHavoc, not Target:IsSpellInRange(S.Havoc)) then addonTable.cast("Havoc") return "havoc cleave 14"; end
    local BestUnit, BestConditionValue, CUCV = nil, nil, nil
    for _, CycleUnit in pairs(Enemies40y) do
      if CycleUnit:GUID() ~= Target:GUID() then
        if BestConditionValue then
  ...
  if CDsON() and S.DimensionalRift:IsCastable() and (SoulShards < 4.5 and VarPoolSoulShards) then
    if Cast(S.DimensionalRift, Settings.Destruction.GCDasOffGCD.DimensionalRift, nil, not Target:IsSpellInRange(S.DimensionalRift)) then addonTable.cast("Dimensional Rift") return "dimensional_rift cleave 14"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (InfernalTime() > 0 or BlasphemyTime() > 0 or SoulShards >= 4) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt cleave 16"; end
  end
  -- summon_infernal
  if CDsON() and S.SummonInfernal:IsCastable() then
    if Cast(S.SummonInfernal, Settings.Destruction.GCDasOffGCD.SummonInfernal) then addonTable.cast("Summon Infernal") return "summon_infernal cleave 18"; end
  end
  ...
  if S.ChannelDemonfire:IsReady() and (S.Ruin:TalentRank() > 1 and not (S.DiabolicEmbers:IsAvailable() and S.AvatarofDestruction:IsAvailable() and (S.BurntoAshes:IsAvailable() or S.ChaosIncarnate:IsAvailable()))) then
    if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire cleave 20"; end
  end
  -- chaos_bolt,if=soul_shard>3.5
  if S.ChaosBolt:IsReady() and (SoulShards > 3.5) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt cleave 22"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (Player:BuffRemains(S.RainofChaosBuff) > S.ChaosBolt:CastTime()) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt cleave 24"; end
  end
  -- chaos_bolt,if=buff.backdraft.up
  if S.ChaosBolt:IsReady() and (Player:BuffUp(S.BackdraftBuff)) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt cleave 26"; end
  end
  -- soul_fire,if=soul_shard<=4&talent.mayhem
  if S.SoulFire:IsCastable() and (SoulShards <= 4 and S.Mayhem:IsAvailable()) then
    if Cast(S.SoulFire, nil, nil, not Target:IsSpellInRange(S.SoulFire)) then addonTable.cast("Soul Fire") return "soul_fire cleave 28"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (S.Eradication:IsAvailable() and Target:DebuffRemains(S.EradicationDebuff) < S.ChaosBolt:CastTime() + S.ChaosBolt:TravelTime() + 1 and not S.ChaosBolt:InFlight()) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt cleave 30"; end
  end
  ...
  if S.ChannelDemonfire:IsReady() and (not (S.DiabolicEmbers:IsAvailable() and S.AvatarofDestruction:IsAvailable() and (S.BurntoAshes:IsAvailable() or S.ChaosIncarnate:IsAvailable()))) then
    if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire cleave 32"; end
  end
  -- dimensional_rift
  if CDsON() and S.DimensionalRift:IsCastable() then
    if Cast(S.DimensionalRift, Settings.Destruction.GCDasOffGCD.DimensionalRift, nil, not Target:IsSpellInRange(S.DimensionalRift)) then addonTable.cast("Dimensional Rift") return "dimensional_rift cleave 34"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (SoulShards > 3.5 and not VarPoolSoulShards) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt cleave 36"; end
  end
  ...
  if S.ChaosBolt:IsReady() and (FightRemains < 5.25 and Target:TimeToDie() > S.ChaosBolt:CastTime() + S.ChaosBolt:TravelTime() + 0.25) then
    if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt cleave 38"; end
  end
  -- summon_soulkeeper,if=buff.tormented_soul.stack=10|buff.tormented_soul.stack>3&fight_remains<10
  if S.SummonSoulkeeper:IsCastable() and (S.SummonSoulkeeper:Count() == 10 or S.SummonSoulkeeper:Count() > 3 and FightRemains < 10) then
    if Cast(S.SummonSoulkeeper, Settings.Destruction.GCDasOffGCD.SummonSoulkeeper) then addonTable.cast("Summon Soulkeeper") return "summon_soulkeeper cleave 40"; end
  end
  ...
  if S.Conflagrate:IsCastable() and (S.Conflagrate:Charges() > (S.Conflagrate:MaxCharges() - 1) or FightRemains < GCDMax * S.Conflagrate:Charges()) then
    if Cast(S.Conflagrate, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Conflagrate") return "conflagrate cleave 42"; end
  end
  -- incinerate
  if S.Incinerate:IsCastable() then
    if Cast(S.Incinerate, nil, nil, not Target:IsSpellInRange(S.Incinerate)) then addonTable.cast("Incinerate") return "incinerate cleave 44"; end
  end
end
...
addonTable.resetPixels()
  ...
  if S.SummonPet:IsCastable() then
    if Cast(S.SummonPet, Settings.Destruction.GCDasOffGCD.SummonPet) then addonTable.cast("Summon Pet") return "summon_pet ooc"; end
  end
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.Conflagrate:IsReady() and ((S.RoaringBlaze:IsAvailable() and Target:DebuffRemains(S.RoaringBlazeDebuff) < 1.5) and SoulShards > 1.5 or S.Conflagrate:Charges() == S.Conflagrate:MaxCharges()) then
      if Cast(S.Conflagrate, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Conflagrate") return "conflagrate main 2"; end
    end
    -- dimensional_rift,if=soul_shard<4.7&(charges>2|fight_remains<cooldown.dimensional_rift.duration)
    if CDsON() and S.DimensionalRift:IsCastable() and (SoulShards < 4.7 and (S.DimensionalRift:Charges() > 2 or FightRemains < S.DimensionalRift:Cooldown())) then
      if Cast(S.DimensionalRift, Settings.Destruction.GCDasOffGCD.DimensionalRift, nil, not Target:IsSpellInRange(S.DimensionalRift)) then addonTable.cast("Dimensional Rift") return "dimensional_rift main 4"; end
    end
    -- cataclysm,if=raid_event.adds.in>15
    if CDsON() and S.Cataclysm:IsReady() then
      if Cast(S.Cataclysm, Settings.Destruction.GCDasOffGCD.Cataclysm, nil, not Target:IsInRange(40)) then addonTable.cast("Cataclysm") return "cataclysm main 6"; end
    end
    ...
    if S.ChannelDemonfire:IsReady() and (S.RagingDemonfire:IsAvailable() and (Target:DebuffRemains(S.ImmolateDebuff) - 5 * num(S.ChaosBolt:InFlight() and S.InternalCombustion:IsAvailable())) > ChannelDemonfireExecuteTime() and (Target:DebuffRemains(S.ConflagrateDebuff) > ChannelDemonfireExecuteTime() or not S.RoaringBlaze:IsAvailable())) then
      if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire main 8"; end
    end
    ...
    if S.SoulFire:IsCastable() and (SoulShards <= 3.5 and (Target:DebuffRemains(S.RoaringBlazeDebuff) > S.SoulFire:CastTime() + S.SoulFire:TravelTime() or not S.RoaringBlaze:IsAvailable() and Player:BuffUp(S.BackdraftBuff))) then
      if Cast(S.SoulFire, nil, nil, not Target:IsSpellInRange(S.SoulFire)) then addonTable.cast("Soul Fire") return "soul_fire main 10"; end
    end
    ...
    if S.Immolate:IsCastable() and ((((Target:DebuffRemains(S.ImmolateDebuff) - 5 * num(S.ChaosBolt:InFlight() and S.InternalCombustion:IsAvailable())) < S.ImmolateDebuff:PandemicThreshold()) or Target:DebuffRemains(S.ImmolateDebuff) < 3 or (Target:DebuffRemains(S.ImmolateDebuff) - S.ChaosBolt:ExecuteTime()) < 5 and S.InternalCombustion:IsAvailable() and S.ChaosBolt:IsReady()) and (not S.Cataclysm:IsAvailable() or S.Cataclysm:CooldownRemains() > Target:DebuffRemains(S.ImmolateDebuff)) and (not S.SoulFire:IsAvailable() or S.SoulFire:CooldownRemains() + S.SoulFire:CastTime() > (Target:DebuffRemains(S.ImmolateDebuff) - 5 * num(S.InternalCombustion:IsAvailable()))) and Target:TimeToDie() > 8) then
      if Cast(S.Immolate, nil, nil, not Target:IsSpellInRange(S.Immolate)) then addonTable.cast("Immolate") return "immolate main 12"; end
    end
    ...
    if S.ChannelDemonfire:IsReady() and (Target:DebuffRemains(S.ImmolateDebuff) > ChannelDemonfireExecuteTime() and Player:HasTier(30, 4)) then
      if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire main 14"; end
    end
    ...
    if S.ChaosBolt:IsReady() and (S.SummonInfernal:CooldownUp() and SoulShards > 4 and S.CrashingChaos:IsAvailable()) then
      if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt main 16"; end
    end
    -- summon_infernal
    if CDsON() and S.SummonInfernal:IsCastable() then
      if Cast(S.SummonInfernal, Settings.Destruction.GCDasOffGCD.SummonInfernal) then addonTable.cast("Summon Infernal") return "summon_infernal main 18"; end
    end
    -- chaos_bolt,if=pet.infernal.active|pet.blasphemy.active|soul_shard>=4
    if S.ChaosBolt:IsReady() and (InfernalTime() > 0 or BlasphemyTime() > 0 or SoulShards >= 4) then
      if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt main 20"; end
    end
    ...
    if S.ChannelDemonfire:IsReady() and (S.Ruin:TalentRank() > 1 and not (S.DiabolicEmbers:IsAvailable() and S.AvatarofDestruction:IsAvailable() and (S.BurntoAshes:IsAvailable() or S.ChaosIncarnate:IsAvailable())) and Target:DebuffRemains(S.ImmolateDebuff) > ChannelDemonfireExecuteTime()) then
      if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire main 22"; end
    end
    -- chaos_bolt,if=buff.rain_of_chaos.remains>cast_time
    if S.ChaosBolt:IsReady() and (Player:BuffRemains(S.RainofChaosBuff) > S.ChaosBolt:CastTime()) then
      if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt main 24"; end
    end
    -- chaos_bolt,if=buff.backdraft.up
    if S.ChaosBolt:IsReady() and (Player:BuffUp(S.BackdraftBuff)) then
      if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt main 26"; end
    end
    ...
    if S.ChannelDemonfire:IsReady() and (not (S.DiabolicEmbers:IsAvailable() and S.AvatarofDestruction:IsAvailable() and (S.BurntoAshes:IsAvailable() or S.ChaosIncarnate:IsAvailable())) and Target:DebuffRemains(S.ImmolateDebuff) > ChannelDemonfireExecuteTime()) then
      if Cast(S.ChannelDemonfire, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Channel Demonfire") return "channel_demonfire main 28"; end
    end
    -- dimensional_rift
    if CDsON() and S.DimensionalRift:IsCastable() then
      if Cast(S.DimensionalRift, Settings.Destruction.GCDasOffGCD.DimensionalRift, nil, not Target:IsSpellInRange(S.DimensionalRift)) then addonTable.cast("Dimensional Rift") return "dimensional_rift main 30"; end
    end
    -- chaos_bolt,if=fight_remains<5&fight_remains>cast_time+travel_time
    ...
    if S.ChaosBolt:IsReady() and (FightRemains < 5.5 and FightRemains > S.ChaosBolt:CastTime() + S.ChaosBolt:TravelTime() + 0.5) then
      if Cast(S.ChaosBolt, nil, nil, not Target:IsSpellInRange(S.ChaosBolt)) then addonTable.cast("Chaos Bolt") return "chaos_bolt main 32"; end
    end
    -- conflagrate,if=charges>(max_charges-1)|fight_remains<gcd.max*charges
    if S.Conflagrate:IsCastable() and (S.Conflagrate:Charges() > (S.Conflagrate:MaxCharges() - 1) or FightRemains < GCDMax * S.Conflagrate:Charges()) then
      if Cast(S.Conflagrate, nil, nil, not Target:IsSpellInRange(S.Conflagrate)) then addonTable.cast("Conflagrate") return "conflagrate main 34"; end
    end
    -- incinerate
    if S.Incinerate:IsCastable() then
      if Cast(S.Incinerate, nil, nil, not Target:IsSpellInRange(S.Incinerate)) then addonTable.cast("Incinerate") return "incinerate main 36"; end
    end
  end
end
