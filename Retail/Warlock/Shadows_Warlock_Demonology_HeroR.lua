...
-- DO NOT REMOVE 
addonTable.spells = {
  { spell = "MACRO OptiAction1", name = "Guillotine" },
  { spell = "SPELL Unending Resolve", name = "Unending Resolve" },
  { spell = "SPELL Summon Vilefiend", name = "Summon Vilefiend" },
  { spell = "SPELL Summon Soulkeeper", name = "Summon Soulkeeper" },
  { spell = "SPELL Summon Demonic Tyrant", name = "Summon Demonic Tyrant" },
  { spell = "SPELL Shadow Bolt", name = "Shadow Bolt" },
  { spell = "SPELL Power Siphon", name = "Power Siphon" },
  { spell = "SPELL Nether Portal", name = "Nether Portal" },
  { spell = "SPELL Implosion", name = "Implosion" },
  { spell = "SPELL Hand of Gul'dan", name = "Hand of Gul'dan" },
  --{ spell = "SPELL Guillotine", name = "Guillotine" },
  { spell = "SPELL Grimoire: Felguard", name = "Grimoire Felguard" },
  { spell = "SPELL Doom", name = "Doom" },
  { spell = "SPELL Demonic Strength", name = "Demonic Strength" },
  { spell = "SPELL Demonbolt", name = "Demonbolt" },
  { spell = "SPELL Call Dreadstalkers", name = "Call Dreadstalkers" },
  { spell = "SPELL Bilescourge Bombers", name = "Bilescourge Bombers" },
...
local function Precombat()
  ...
  if S.PowerSiphon:IsReady() and (Player:BuffStack(S.DemonicCoreBuff) + mathmax(WildImpsCount(), 2) <= 4 or Player:BuffRemains(S.DemonicCoreBuff) < 3) then
    if Cast(S.PowerSiphon, Settings.Demonology.GCDasOffGCD.PowerSiphon) then addonTable.cast("Power Siphon") return "power_siphon precombat 2"; end
  end
  ...
  if S.Demonbolt:IsReady() and not Target:IsInBossList() and Player:BuffUp(S.DemonicCoreBuff) then
    if Cast(S.Demonbolt, nil, nil, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt precombat 3"; end
  end
  ...
  if S.Demonbolt:IsReady() and Player:BuffDown(S.DemonicCoreBuff) and not Player:PrevGCDP(1, S.PowerSiphon) then
    if Cast(S.Demonbolt, nil, nil, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt precombat 4"; end
  end
  -- shadow_bolt
  if S.ShadowBolt:IsReady() then
    if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt precombat 6"; end
  end
end
...
local function Racials()
  ...
  if S.Berserking:IsCastable() then
    if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Berserking") return "berserking racials 2"; end
  end
  -- blood_fury
  ...
  if S.Fireblood:IsCastable() then
    if Cast(S.Fireblood, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Fireblood") return "fireblood racials 6"; end
  end
  -- ancestral_call
  if S.AncestralCall:IsCastable() then
    if Cast(S.AncestralCall, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Ancestral Call") return "ancestral_call racials 8"; end
  end
end
...
local function Tyrant()
  ...
  if S.HandofGuldan:IsReady() and SoulShards > 0 and (VarPetExpire > GCDMax + S.SummonDemonicTyrant:CastTime() and VarPetExpire < GCDMax * 4) then
    if Cast(S.HandofGuldan, nil, nil, not Target:IsSpellInRange(S.HandofGuldan)) then addonTable.cast("Hand of Gul'dan") return "hand_of_guldan tyrant 2"; end
  end
  ...
  if CDsON() and S.SummonDemonicTyrant:IsCastable() and (VarPetExpire > 0 and VarPetExpire < S.SummonDemonicTyrant:ExecuteTime() + (num(Player:BuffDown(S.DemonicCoreBuff)) * S.ShadowBolt:ExecuteTime() + num(Player:BuffUp(S.DemonicCoreBuff)) * GCDMax) + GCDMax) then
    if Cast(S.SummonDemonicTyrant, Settings.Demonology.GCDasOffGCD.SummonDemonicTyrant) then addonTable.cast("Summon Demonic Tyrant") return "summon_demonic_tyrant tyrant 6"; end
  end
  ...
  if S.Implosion:IsReady() and (WildImpsCount() > 2 and (not DreadstalkerActive() and not GrimoireFelguardActive() and not VilefiendActive()) and (EnemiesCount8ySplash > 3 or EnemiesCount8ySplash > 2 and S.GrandWarlocksDesign:IsAvailable()) and not Player:PrevGCDP(1, S.Implosion)) then
    if Cast(S.Implosion, Settings.Demonology.GCDasOffGCD.Implosion, nil, not Target:IsInRange(40)) then addonTable.cast("Implosion") return "implosion tyrant 8"; end
  end
  ...
  if S.ShadowBolt:IsReady() and (Player:PrevGCDP(1, S.GrimoireFelguard) and CombatTime > 30 and Player:BuffDown(S.NetherPortalBuff) and Player:BuffDown(S.DemonicCoreBuff)) then
    if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt tyrant 10"; end
  end
  ...
  if S.PowerSiphon:IsReady() and (Player:BuffStack(S.DemonicCoreBuff) < 4 and (not VilefiendActive() or not S.SummonVilefiend:IsAvailable() and DreadstalkerTime()) and Player:BuffDown(S.NetherPortalBuff)) then
    if Cast(S.PowerSiphon, Settings.Demonology.GCDasOffGCD.PowerSiphon) then addonTable.cast("Power Siphon") return "power_siphon tyrant 12"; end
  end
  ...
  if S.ShadowBolt:IsReady() and (not VilefiendActive() and Player:BuffDown(S.NetherPortalBuff) and not DreadstalkerActive() and SoulShards < 5 - Player:BuffStack(S.DemonicCoreBuff)) then
    if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt tyrant 14"; end
  end
  -- nether_portal,if=soul_shard=5
  if CDsON() and S.NetherPortal:IsReady() and (SoulShards == 5) then
    if Cast(S.NetherPortal, Settings.Demonology.GCDasOffGCD.NetherPortal) then addonTable.cast("Nether Portal") return "nether_portal tyrant 16"; end
  end
  ...
  if S.SummonVilefiend:IsReady() and ((SoulShards == 5 or Player:BuffUp(S.NetherPortalBuff)) and S.SummonDemonicTyrant:CooldownRemains() < 13 and VarNP) then
    if Cast(S.SummonVilefiend) then addonTable.cast("Summon Vilefiend") return "summon_vilefiend tyrant 18"; end
  end
  ...
  if S.CallDreadstalkers:IsReady() and ((VilefiendActive() or not S.SummonVilefiend:IsAvailable() and (not S.NetherPortal:IsAvailable() or Player:BuffUp(S.NetherPortalBuff) or S.NetherPortal:CooldownRemains() > 30) and (Player:BuffUp(S.NetherPortalBuff) or GrimoireFelguardActive() or SoulShards == 5)) and S.SummonDemonicTyrant:CooldownRemains() < 11 and VarNP) then
    if Cast(S.CallDreadstalkers, nil, nil, not Target:IsSpellInRange(S.CallDreadstalkers)) then addonTable.cast("Call Dreadstalkers") return "call_dreadstalkers tyrant 20"; end
  end
  ...
  if CDsON() and S.GrimoireFelguard:IsReady() and (VilefiendActive() or not S.SummonVilefiend:IsAvailable() and (not S.NetherPortal:IsAvailable() or Player:BuffUp(S.NetherPortalBuff) or S.NetherPortal:CooldownRemains() > 30) and (Player:BuffUp(S.NetherPortalBuff) or DreadstalkerActive() or SoulShards == 5) and VarNP) then
    if Cast(S.GrimoireFelguard, Settings.Demonology.GCDasOffGCD.GrimoireFelguard, nil, not Target:IsSpellInRange(S.GrimoireFelguard)) then addonTable.cast("Grimoire Felguard") return "grimoire_felguard tyrant 22"; end
  end
  ...
  if S.HandofGuldan:IsReady() and (SoulShards > 2 and (VilefiendActive() or not S.SummonVilefiend:IsAvailable() and DreadstalkerActive()) and (SoulShards > 2 or VilefiendTime() < GCDMax * 2 + 2 / Player:SpellHaste()) or (not DreadstalkerActive() and SoulShards == 5)) then
    if Cast(S.HandofGuldan, nil, nil, not Target:IsSpellInRange(S.HandofGuldan)) then addonTable.cast("Hand of Gul'dan") return "hand_of_guldan tyrant 24"; end
  end
  ...
  if S.Demonbolt:IsReady() and (SoulShards < 4 and (Player:BuffStack(S.DemonicCoreBuff) > 1) and (VilefiendActive() or not S.SummonVilefiend:IsAvailable() and DreadstalkerActive() or not CDsON())) then
    if S.DoomBrandDebuff:AuraActiveCount() == EnemiesCount8ySplash or not Player:HasTier(31, 2) then
      if Cast(S.Demonbolt, nil, nil, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt tyrant 26"; end
    else
      if Everyone.CastCycle(S.Demonbolt, Enemies8ySplash, EvaluateCycleDoomBrand, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt tyrant 27"; end
    end
  end
  ...
  if S.PowerSiphon:IsReady() and (Player:BuffStack(S.DemonicCoreBuff) < 3 and VarPetExpire > S.SummonDemonicTyrant:ExecuteTime() + GCDMax * 3 or VarPetExpire == 0) then
    if Cast(S.PowerSiphon, Settings.Demonology.GCDasOffGCD.PowerSiphon) then addonTable.cast("Power Siphon") return "power_siphon tyrant 28"; end
  end
  -- shadow_bolt
  if S.ShadowBolt:IsCastable() then
    if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt tyrant 30"; end
  end
end
...
local function FightEnd()
  if BossFightRemains < 20 then
    ...
    if CDsON() and S.GrimoireFelguard:IsReady() then
      if Cast(S.GrimoireFelguard, Settings.Demonology.GCDasOffGCD.GrimoireFelguard) then addonTable.cast("Grimoire Felguard") return "grimoire_felguard fight_end 2"; end
    end
    -- call_dreadstalkers,if=fight_remains<20
    if S.CallDreadstalkers:IsReady() then
      if Cast(S.CallDreadstalkers, nil, nil, not Target:IsSpellInRange(S.CallDreadstalkers)) then addonTable.cast("Call Dreadstalkers") return "call_dreadstalkers fight_end 4"; end
    end
    -- summon_vilefiend,if=fight_remains<20
    if S.SummonVilefiend:IsReady() then
      if Cast(S.SummonVilefiend) then addonTable.cast("Summon Vilefiend") return "summon_vilefiend fight_end 6"; end
    end
  end
  -- nether_portal,if=fight_remains<30
  if CDsON() and S.NetherPortal:IsReady() and (BossFightRemains < 30) then
    if Cast(S.NetherPortal, Settings.Demonology.GCDasOffGCD.NetherPortal) then addonTable.cast("Nether Portal") return "nether_portal fight_end 8"; end
  end
  -- summon_demonic_tyrant,if=fight_remains<20
  if CDsON() and S.SummonDemonicTyrant:IsCastable() and (BossFightRemains < 20) then
    if Cast(S.SummonDemonicTyrant, Settings.Demonology.GCDasOffGCD.SummonDemonicTyrant) then addonTable.cast("Summon Demonic Tyrant") return "summon_demonic_tyrant fight_end 10"; end
  end
  -- demonic_strength,if=fight_remains<10
  if S.DemonicStrength:IsCastable() and (BossFightRemains < 10) then
    if Cast(S.DemonicStrength, Settings.Demonology.GCDasOffGCD.DemonicStrength) then addonTable.cast("Demonic Strength") return "demonic_strength fight_end 12"; end
  end
  ...
  if S.PowerSiphon:IsReady() and (Player:BuffStack(S.DemonicCoreBuff) < 3 and BossFightRemains < 20) then
    if Cast(S.PowerSiphon, Settings.Demonology.GCDasOffGCD.PowerSiphon) then addonTable.cast("Power Siphon") return "power_siphon fight_end 14"; end
  end
  -- implosion,if=fight_remains<2*gcd.max
  if S.Implosion:IsReady() and (FightRemains < 2 * GCDMax) then
    if Cast(S.Implosion, Settings.Demonology.GCDasOffGCD.Implosion, nil, not Target:IsInRange(40)) then addonTable.cast("Implosion") return "implosion fight_end 16"; end
  end
end
...
addonTable.resetPixels()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.UnendingResolve:IsReady() and (Player:HealthPercentage() < Settings.Demonology.UnendingResolveHP) then
      if Cast(S.UnendingResolve, Settings.Demonology.OffGCDasOffGCD.UnendingResolve) then addonTable.cast("Unending Resolve") return "unending_resolve defensive"; end
    end
    ...
    if S.HandofGuldan:IsReady() and (CombatTime < 0.5 and (FightRemains % 95 > 40 or FightRemains % 95 < 15) and (S.ReignofTyranny:IsAvailable() or EnemiesCount8ySplash > 2)) then
      if Cast(S.HandofGuldan, nil, nil, not Target:IsSpellInRange(S.HandofGuldan)) then addonTable.cast("Hand of Gul'dan") return "hand_of_guldan main 2"; end
    end
    ...
    if CDsON() and S.SummonDemonicTyrant:IsCastable() and (VilefiendActive() or GrimoireFelguardActive() or S.GrimoireFelguard:CooldownRemains() > 90) then
      if Cast(S.SummonDemonicTyrant, Settings.Demonology.GCDasOffGCD.SummonDemonicTyrant) then addonTable.cast("Summon Demonic Tyrant") return "summon_demonic_tyrant main 4"; end
    end
    ...
    if S.SummonVilefiend:IsReady() and (S.SummonDemonicTyrant:CooldownRemains() > 45) then
      if Cast(S.SummonVilefiend) then addonTable.cast("Summon Vilefiend") return "summon_vilefiend main 6"; end
    end
    ...
    if S.Demonbolt:IsReady() and (Player:BuffUp(S.DemonicCoreBuff) and (((not S.SoulStrike:IsAvailable() or S.SoulStrike:CooldownRemains() > GCDMax * 2) and SoulShards < 4) or SoulShards < (4 - (num(EnemiesCount8ySplash > 2)))) and not Player:PrevGCDP(1, S.Demonbolt) and Player:HasTier(31, 2)) then
      if Everyone.CastCycle(S.Demonbolt, Enemies8ySplash, EvaluateCycleDemonbolt, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt main 8"; end
    end
    ...
    if S.PowerSiphon:IsReady() and (Player:BuffDown(S.DemonicCoreBuff) and (Target:DebuffDown(S.DoomBrandDebuff) or (not S.HandofGuldan:InFlight() and Target:DebuffRemains(S.DoomBrandDebuff) < GCDMax + S.Demonbolt:TravelTime()) or (S.HandofGuldan:InFlight() and Target:DebuffRemains(S.DoomBrandDebuff) < GCDMax + S.Demonbolt:TravelTime() + 3)) and Player:HasTier(31, 2)) then
      if Cast(S.PowerSiphon, Settings.Demonology.GCDasOffGCD.PowerSiphon) then addonTable.cast("Power Siphon") return "power_siphon main 10"; end
    end
    ...
    if S.DemonicStrength:IsCastable() and (Player:BuffRemains(S.NetherPortalBuff) < GCDMax) then
      if Cast(S.DemonicStrength, Settings.Demonology.GCDasOffGCD.DemonicStrength) then addonTable.cast("Demonic Strength") return "demonic_strength main 12"; end
    end
    -- bilescourge_bombers
    if S.BilescourgeBombers:IsReady() then
      if Cast(S.BilescourgeBombers, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Bilescourge Bombers") return "bilescourge_bombers main 14"; end
    end
    ...
    if S.Guillotine:IsCastable() and (Player:BuffRemains(S.NetherPortalBuff) < GCDMax and (S.DemonicStrength:CooldownDown() or not S.DemonicStrength:IsAvailable())) then
      if Cast(S.Guillotine, Settings.Demonology.GCDasOffGCD.Guillotine, nil, not Target:IsInRange(40)) then addonTable.cast("Guillotine") return "guillotine main 16"; end
    end
    ...
    if S.CallDreadstalkers:IsReady() and (S.SummonDemonicTyrant:CooldownRemains() > 25 or VarTyrantCD > 25 or Player:BuffUp(S.NetherPortalBuff)) then
      if Cast(S.CallDreadstalkers, nil, nil, not Target:IsSpellInRange(S.CallDreadstalkers)) then addonTable.cast("Call Dreadstalkers") return "call_dreadstalkers main 18"; end
    end
    ...
    if S.Implosion:IsReady() and (CheckImpCasts(2) > 0 and VarImpl and not Player:PrevGCDP(1, S.Implosion) and (EnemiesCount8ySplash == 1 or (EnemiesCount8ySplash > 3 or EnemiesCount8ySplash <= 3 and CheckImpCasts(1) > 0))) then
      if Cast(S.Implosion, Settings.Demonology.GCDasOffGCD.Implosion, nil, not Target:IsInRange(40)) then addonTable.cast("Implosion") return "implosion main 20"; end
    end
    ...
    if S.SummonSoulkeeper:IsReady() and (S.SummonSoulkeeper:Count() == 10 and EnemiesCount8ySplash > 1) then
      if Cast(S.SummonSoulkeeper) then addonTable.cast("Summon Soulkeeper") return "soul_strike main 22"; end
    end
    ...
    if S.HandofGuldan:IsReady() and (((SoulShards > 2 and S.CallDreadstalkers:CooldownRemains() > GCDMax * 4 and S.SummonDemonicTyrant:CooldownRemains() > 17) or SoulShards == 5 or SoulShards == 4 and S.SoulStrike:IsAvailable() and S.SoulStrike:CooldownRemains() < GCDMax * 2) and (EnemiesCount8ySplash == 1 and S.GrandWarlocksDesign:IsAvailable())) then
      if Cast(S.HandofGuldan, nil, nil, not Target:IsSpellInRange(S.HandofGuldan)) then addonTable.cast("Hand of Gul'dan") return "hand_of_guldan main 26"; end
    end
    ...
    if S.HandofGuldan:IsReady() and (SoulShards > 2 and not (EnemiesCount8ySplash == 1 and S.GrandWarlocksDesign:IsAvailable())) then
      if Cast(S.HandofGuldan, nil, nil, not Target:IsSpellInRange(S.HandofGuldan)) then addonTable.cast("Hand of Gul'dan") return "hand_of_guldan main 28"; end
    end
    ...
    if S.Demonbolt:IsReady() and (Player:BuffStack(S.DemonicCoreBuff) > 1 and ((SoulShards < 4 and not S.SoulStrike:IsAvailable() or S.SoulStrike:CooldownRemains() > GCDMax * 2) or SoulShards < 2) and not VarPoolCoresForTyrant) then
      if Everyone.CastCycle(S.Demonbolt, Enemies8ySplash, EvaluateCycleDemonbolt2, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt main 30"; end
    end
    ...
    if S.Demonbolt:IsReady() and (Player:HasTier(31, 2) and Player:BuffUp(S.DemonicCoreBuff) and SoulShards < 4 and not VarPoolCoresForTyrant) then
      if Everyone.CastTargetIf(S.Demonbolt, Enemies8ySplash, "==", EvaluateCycleDemonbolt2, EvaluateTargetIfDemonbolt, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt main 32"; end
    end
    ...
    if S.Demonbolt:IsReady() and (FightRemains < Player:BuffStack(S.DemonicCoreBuff) * GCDMax) then
      if Cast(S.Demonbolt, nil, nil, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt main 34"; end
    end
    ...
    if S.Demonbolt:IsReady() and (Player:BuffUp(S.DemonicCoreBuff) and S.PowerSiphon:CooldownRemains() < 4 and SoulShards < 4 and not VarPoolCoresForTyrant) then
      if Everyone.CastCycle(S.Demonbolt, Enemies8ySplash, EvaluateCycleDemonbolt2, not Target:IsSpellInRange(S.Demonbolt)) then addonTable.cast("Demonbolt") return "demonbolt main 36"; end
    end
    -- power_siphon,if=!buff.demonic_core.up
    if S.PowerSiphon:IsReady() and (Player:BuffDown(S.DemonicCoreBuff)) then
      if Cast(S.PowerSiphon, Settings.Demonology.GCDasOffGCD.PowerSiphon) then addonTable.cast("Power Siphon") return "power_siphon main 38"; end
    end
    ...
    if S.SummonVilefiend:IsReady() and (FightRemains < S.SummonDemonicTyrant:CooldownRemains() + 5) then
      if Cast(S.SummonVilefiend) then addonTable.cast("Summon Vilefiend") return "summon_vilefiend main 40"; end
    end
    -- doom,target_if=refreshable
    if S.Doom:IsReady() then
      if Everyone.CastCycle(S.Doom, Enemies40y, EvaluateCycleDoom, not Target:IsSpellInRange(S.Doom)) then addonTable.cast("Doom") return "doom main 42"; end
    end
    -- shadow_bolt
    if S.ShadowBolt:IsCastable() then
      if Cast(S.ShadowBolt, nil, nil, not Target:IsSpellInRange(S.ShadowBolt)) then addonTable.cast("Shadow Bolt") return "shadow_bolt main 44"; end
    end
  end
end
