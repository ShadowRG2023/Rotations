...
-- DO NOT REMOVE 
addonTable.spells = {
  { spell = "MACRO OptiAction1", name = "Blizzard" },
  { spell = "SPELL Time Warp", name = "Time Warp" },
  { spell = "SPELL Shifting Power", name = "Shifting Power" },
  { spell = "SPELL Ray of Frost", name = "Ray of Frost" },
  { spell = "SPELL Icy Veins", name = "Icy Veins" },
  { spell = "SPELL Ice Nova", name = "Ice Nova" },
  { spell = "SPELL Ice Lance", name = "Ice Lance" },
  { spell = "SPELL Ice Floes", name = "Ice Floes" },
  { spell = "SPELL Glacial Spike", name = "Glacial Spike" },
  { spell = "SPELL Frozen Orb", name = "Frozen Orb" },
  { spell = "SPELL Frostbolt", name = "Frostbolt" },
  { spell = "SPELL Frost Nova", name = "Frost Nova" },
  { spell = "SPELL Freeze", name = "Freeze" },
  { spell = "SPELL Flurry", name = "Flurry" },
  ...
  { spell = "SPELL Cone of Cold", name = "Cone of Cold" },
  { spell = "SPELL Comet Storm", name = "Comet Storm" },
  ...
  --{ spell = "SPELL Blizzard", name = "Blizzard" },
...
local function Precombat()
  ...
  if S.Frostbolt:IsCastable() and not Player:IsCasting(S.Frostbolt) then
    if Cast(S.Frostbolt, nil, nil, not Target:IsSpellInRange(S.Frostbolt)) then addonTable.cast("Frostbolt") return "frostbolt precombat 4"; end
  end
end
...
local function Cooldowns()
  ...
  if Settings.Commons.UseTemporalWarp and S.TimeWarp:IsCastable() and (Player:BloodlustExhaustUp() and S.TemporalWarp:IsAvailable() and Player:BloodlustDown() and (Player:PrevOffGCDP(1, S.IcyVeins) or (Player:BuffUp(S.IcyVeinsBuff) and FightRemains <= 110 or Player:BuffUp(S.IcyVeinsBuff) and FightRemains >= 280) or FightRemains < 40)) then
    if Cast(S.TimeWarp, Settings.CommonsOGCD.OffGCDasOffGCD.TimeWarp) then addonTable.cast("Time Warp") return "time_warp cd 2"; end
  end
  ...
  if S.IcyVeins:IsCastable() then
    if Cast(S.IcyVeins, Settings.Frost.GCDasOffGCD.IcyVeins) then addonTable.cast("Icy Veins") return "icy_veins cd 14"; end
  end
  ...
  if S.BloodFury:IsCastable() then
    if Cast(S.BloodFury, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Blood Fury") return "blood_fury cd 16"; end
  end
  -- berserking
  if S.Berserking:IsCastable() then
    if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Berserking") return "berserking cd 18"; end
  end
  -- lights_judgment
  if S.LightsJudgment:IsCastable() then
    if Cast(S.LightsJudgment, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsSpellInRange(S.LightsJudgment)) then addonTable.cast("Light's Judgment") return "lights_judgment cd 20"; end
  end
  -- fireblood
  if S.Fireblood:IsCastable() then
    if Cast(S.Fireblood, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Fireblood") return "fireblood cd 22"; end
  end
  -- ancestral_call
  if S.AncestralCall:IsCastable() then
    if Cast(S.AncestralCall, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Ancestral Call") return "ancestral_call cd 24"; end
  end
end
...
local function Movement()
  ...
  if S.IceFloes:IsCastable() and (Player:BuffDown(S.IceFloes)) then
    if Cast(S.IceFloes, nil, Settings.Frost.DisplayStyle.Movement) then addonTable.cast("Ice Floes") return "ice_floes movement 2"; end
  end
  -- ice_nova
  if S.IceNova:IsCastable() then
    if Cast(S.IceNova, nil, Settings.Frost.DisplayStyle.Movement, not Target:IsSpellInRange(S.IceNova)) then addonTable.cast("Ice Nova") return "ice_nova movement 4"; end
  end
  ...
  if S.ArcaneExplosion:IsReady() and Target:IsInRange(10) and (Player:ManaPercentage() > 30 and EnemiesCount8ySplash >= 2) then
    if Cast(S.ArcaneExplosion, nil, Settings.Frost.DisplayStyle.Movement) then addonTable.cast("Arcane Explosion") return "arcane_explosion movement 6"; end
  end
  -- fire_blast
  if S.FireBlast:IsReady() then
    if Cast(S.FireBlast, nil, Settings.Frost.DisplayStyle.Movement, not Target:IsSpellInRange(S.FireBlast)) then addonTable.cast("Fire Blast") return "fire_blast movement 8"; end
  end
  -- ice_lance
  if S.IceLance:IsReady() then
    if Cast(S.IceLance, nil, Settings.Frost.DisplayStyle.Movement, not Target:IsSpellInRange(S.IceLance)) then addonTable.cast("Ice Lance") return "ice_lance movement 10"; end
  end
end
...
local function Aoe()
  ...
  if S.ConeofCold:IsCastable() and (S.ColdestSnap:IsAvailable() and (Player:PrevGCDP(1, S.CometStorm) or Player:PrevGCDP(1, S.FrozenOrb) and not S.CometStorm:IsAvailable())) then
    if Cast(S.ConeofCold, nil, nil, not Target:IsInRange(12)) then addonTable.cast("Cone of Cold") return "cone_of_cold aoe 2"; end
  end
  ...
  if S.FrozenOrb:IsCastable() and (not Player:PrevGCDP(1, S.GlacialSpike) or not Freezable()) then
    if Cast(S.FrozenOrb, Settings.Frost.GCDasOffGCD.FrozenOrb, nil, not Target:IsInRange(40)) then addonTable.cast("Frozen Orb") return "frozen_orb aoe 4"; end
  end
  ...
  if S.Blizzard:IsCastable() and (not Player:PrevGCDP(1, S.GlacialSpike) or not Freezable()) then
    if Cast(S.Blizzard, Settings.Frost.GCDasOffGCD.Blizzard, nil, not Target:IsInRange(40)) then addonTable.cast("Blizzard") return "blizzard aoe 6"; end
  end
  ...
  if S.CometStorm:IsCastable() and (not Player:PrevGCDP(1, S.GlacialSpike) and (not S.ColdestSnap:IsAvailable() or S.ConeofCold:CooldownUp() and S.FrozenOrb:CooldownRemains() > 25 or S.ConeofCold:CooldownRemains() > 20)) then
    if Cast(S.CometStorm, Settings.Frost.GCDasOffGCD.CometStorm, nil, not Target:IsSpellInRange(S.CometStorm)) then addonTable.cast("Comet Storm") return "comet_storm aoe 8"; end
  end
  ...
  if Pet:IsActive() and S.Freeze:IsReady() and (Freezable() and FrozenRemains() == 0 and (not S.GlacialSpike:IsAvailable() and not S.Snowstorm:IsAvailable() or Player:PrevGCDP(1, S.GlacialSpike) or S.ConeofCold:CooldownUp() and Player:BuffStack(S.SnowstormBuff) == var_snowstorm_max_stack)) then
    if Cast(S.Freeze, nil, nil, not Target:IsSpellInRange(S.Freeze)) then addonTable.cast("Freeze") return "freeze aoe 10"; end
  end
  ...
  if S.IceNova:IsCastable() and (Freezable() and not Player:PrevOffGCDP(1, S.Freeze) and (Player:PrevGCDP(1, S.GlacialSpike) or S.ConeofCold:CooldownUp() and Player:BuffStack(S.SnowstormBuff) == var_snowstorm_max_stack and GCDMax < 1)) then
    if Cast(S.IceNova, nil, nil, not Target:IsSpellInRange(S.IceNova)) then addonTable.cast("Ice Nova") return "ice_nova aoe 12"; end
  end
  ...
  if S.FrostNova:IsCastable() and (Freezable() and not Player:PrevOffGCDP(1, S.Freeze) and (Player:PrevGCDP(1, S.GlacialSpike) and RemainingWintersChill == 0 or S.ConeofCold:CooldownUp() and Player:BuffStack(S.SnowstormBuff) == var_snowstorm_max_stack and GCDMax < 1)) then
    if Cast(S.FrostNova, nil, nil, not Target:IsInRange(12)) then addonTable.cast("Frost Nova") return "frost_nova aoe 14"; end
  end
  ...
  if S.ConeofCold:IsCastable() and (Player:BuffStackP(S.SnowstormBuff) == var_snowstorm_max_stack) then
    if Cast(S.ConeofCold, nil, nil, not Target:IsInRange(12)) then addonTable.cast("Cone of Cold") return "cone_of_cold aoe 16"; end
  end
  -- shifting_power
  if S.ShiftingPower:IsCastable() and CDsON() then
    if Cast(S.ShiftingPower, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(18)) then addonTable.cast("Shifting Power") return "shifting_power aoe 18"; end
  end
  ...
  if S.GlacialSpike:IsReady() and (Icicles == 5 and S.Blizzard:CooldownRemains() > GCDMax) then
    if Cast(S.GlacialSpike, nil, nil, not Target:IsSpellInRange(S.GlacialSpike)) then addonTable.cast("Glacial Spike") return "glacial_spike aoe 20"; end
  end
  ...
  if S.Flurry:IsCastable() and (not Freezable() and RemainingWintersChill == 0 and (Player:PrevGCDP(1, S.GlacialSpike) or S.Flurry:ChargesFractional() > 1.8)) then
    if Cast(S.Flurry, Settings.Frost.GCDasOffGCD.Flurry, nil, not Target:IsSpellInRange(S.Flurry)) then addonTable.cast("Flurry") return "flurry aoe 22"; end
  end
  ...
  if S.Flurry:IsCastable() and (RemainingWintersChill == 0 and (Player:BuffUp(S.BrainFreezeBuff) or Player:BuffUp(S.FingersofFrostBuff))) then
    if Cast(S.Flurry, Settings.Frost.GCDasOffGCD.Flurry, nil, not Target:IsSpellInRange(S.Flurry)) then addonTable.cast("Flurry") return "flurry aoe 24"; end
  end
  ...
  if S.IceLance:IsReady() and (Player:BuffUp(S.FingersofFrostBuff) or FrozenRemains() > S.IceLance:TravelTime() or bool(RemainingWintersChill)) then
    if Cast(S.IceLance, nil, nil, not Target:IsSpellInRange(S.IceLance)) then addonTable.cast("Ice Lance") return "ice_lance aoe 26"; end
  end
  ...
  if S.IceNova:IsCastable() and (EnemiesCount8ySplash >= 4 and (not S.Snowstorm:IsAvailable() and not S.GlacialSpike:IsAvailable() or not Freezable())) then
    if Cast(S.IceNova, nil, nil, not Target:IsSpellInRange(S.IceNova)) then addonTable.cast("Ice Nova") return "ice_nova aoe 28"; end
  end
  -- dragons_breath,if=active_enemies>=7
  if S.DragonsBreath:IsCastable() and (EnemiesCount16ySplash >= 7) then
    if Cast(S.DragonsBreath, nil, nil, not Target:IsInRange(12)) then addonTable.cast("Dragon's Breath") return "dragons_breath aoe 30"; end
  end
  ...
  if S.ArcaneExplosion:IsCastable() and (Player:ManaPercentage() > 30 and EnemiesCount16ySplash >= 7) then
    if Settings.Frost.StayDistance and not Target:IsInRange(10) then
      ...
      if Cast(S.ArcaneExplosion) then addonTable.cast("Arcane Explosion") return "arcane_explosion aoe 32"; end
    end
  end
  -- frostbolt
  if S.Frostbolt:IsCastable() then
    if Cast(S.Frostbolt, nil, nil, not Target:IsSpellInRange(S.Frostbolt)) then addonTable.cast("Frostbolt") return "frostbolt aoe 34"; end
  end
...
local function Cleave()
  ...
  if S.CometStorm:IsCastable() and (Player:PrevGCDP(1, S.Flurry) or Player:PrevGCDP(1, S.ConeofCold)) then
    if Cast(S.CometStorm, Settings.Frost.GCDasOffGCD.CometStorm, nil, not Target:IsSpellInRange(S.CometStorm)) then addonTable.cast("Comet Storm") return "comet_storm cleave 2"; end
  end
  ...
  if S.Flurry:IsCastable() and ((Player:PrevGCDP(1, S.Frostbolt) and Icicles >= 3) or Player:PrevGCDP(1, S.GlacialSpike) or (Icicles >= 3 and Icicles < 5 and S.Flurry:ChargesFractional() == 2)) then
    if Everyone.CastTargetIf(S.Flurry, Enemies16ySplash, "min", EvaluateTargetIfFilterWCStacks, nil, not Target:IsSpellInRange(S.Flurry), Settings.Frost.GCDasOffGCD.Flurry) then addonTable.cast("Flurry") return "flurry cleave 4"; end
  end
  ...
  if S.IceLance:IsReady() and (S.GlacialSpike:IsAvailable() and S.WintersChillDebuff:AuraActiveCount() == 0 and Icicles == 4 and Player:BuffUp(S.FingersofFrostBuff)) then
    if Cast(S.IceLance, nil, nil, not Target:IsSpellInRange(S.IceLance)) then addonTable.cast("Ice Lance") return "ice_lance cleave 6"; end
  end
  ...
  if S.RayofFrost:IsCastable() and (RemainingWintersChill == 1) then
    if Everyone.CastTargetIf(S.RayofFrost, Enemies16ySplash, "max", EvaluateTargetIfFilterWCStacks, nil, not Target:IsSpellInRange(S.RayofFrost), Settings.Frost.GCDasOffGCD.RayOfFrost) then addonTable.cast("Ray of Frost") return "ray_of_frost cleave 8"; end
  end
  ...
  if S.GlacialSpike:IsReady() and (Icicles == 5 and (S.Flurry:CooldownUp() or RemainingWintersChill > 0)) then
    if Cast(S.GlacialSpike, nil, nil, not Target:IsSpellInRange(S.GlacialSpike)) then addonTable.cast("Glacial Spike") return "glacial_spike cleave 10"; end
  end
  ...
  if S.FrozenOrb:IsCastable() and (Player:BuffStackP(S.FingersofFrostBuff) < 2 and (not S.RayofFrost:IsAvailable() or S.RayofFrost:CooldownDown())) then
    if Cast(S.FrozenOrb, Settings.Frost.GCDasOffGCD.FrozenOrb, nil, not Target:IsInRange(40)) then addonTable.cast("Frozen Orb") return "frozen_orb cleave 12"; end
  end
  ...
  if S.ConeofCold:IsCastable() and (S.ColdestSnap:IsAvailable() and S.CometStorm:CooldownRemains() > 10 and S.FrozenOrb:CooldownRemains() > 10 and RemainingWintersChill == 0 and EnemiesCount16ySplash >= 3) then
    if Cast(S.ConeofCold, nil, nil, not Target:IsInRange(12)) then addonTable.cast("Cone of Cold") return "cone_of_cold cleave 14"; end
  end
  ...
  if S.Blizzard:IsCastable() and (EnemiesCount16ySplash >= 2 and S.IceCaller:IsAvailable() and S.FreezingRain:IsAvailable() and (not S.SplinteringCold:IsAvailable() and not S.RayofFrost:IsAvailable() or Player:BuffUp(S.FreezingRainBuff) or EnemiesCount16ySplash >= 3)) then
    if Cast(S.Blizzard, Settings.Frost.GCDasOffGCD.Blizzard, nil, not Target:IsInRange(40)) then addonTable.cast("Blizzard") return "blizzard cleave 16"; end
  end
  ...
  if S.ShiftingPower:IsCastable() and (S.FrozenOrb:CooldownRemains() > 10 and (not S.CometStorm:IsAvailable() or S.CometStorm:CooldownRemains() > 10) and (not S.RayofFrost:IsAvailable() or S.RayofFrost:CooldownRemains() > 10) or S.IcyVeins:CooldownRemains() < 20) then
    if Cast(S.ShiftingPower, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(18)) then addonTable.cast("Shifting Power") return "shifting_power cleave 18"; end
  end
  -- glacial_spike,if=buff.icicles.react=5
  if S.GlacialSpike:IsReady() and (Icicles == 5) then
    if Cast(S.GlacialSpike, nil, nil, not Target:IsSpellInRange(S.GlacialSpike)) then addonTable.cast("Glacial Spike") return "glacial_spike cleave 20"; end
  end
  ...
  if S.IceLance:IsReady() and (Player:BuffUpP(S.FingersofFrostBuff) and not Player:PrevGCDP(1, S.GlacialSpike) or RemainingWintersChill > 0) then
    if Everyone.CastTargetIf(S.IceLance, Enemies16ySplash, "max", EvaluateTargetIfFilterWCStacks, nil, not Target:IsSpellInRange(S.IceLance)) then addonTable.cast("Ice Lance") return "ice_lance cleave 22"; end
  end
  -- ice_nova,if=active_enemies>=4
  if S.IceNova:IsCastable() and (EnemiesCount16ySplash >= 4) then
    if Cast(S.IceNova, nil, nil, not Target:IsSpellInRange(S.IceNova)) then addonTable.cast("Ice Nova") return "ice_nova cleave 24"; end
  end
  -- frostbolt
  if S.Frostbolt:IsCastable() then
    if Cast(S.Frostbolt, nil, nil, not Target:IsSpellInRange(S.Frostbolt)) then addonTable.cast("Frostbolt") return "frostbolt cleave 26"; end
  end
  -- call_action_list,name=movement
  if Player:IsMoving() then
    local ShouldReturn = Movement(); if ShouldReturn then return ShouldReturn; end
...
local function ST()
  -- comet_storm,if=prev_gcd.1.flurry|prev_gcd.1.cone_of_cold
  if S.CometStorm:IsCastable() and (Player:PrevGCDP(1, S.Flurry) or Player:PrevGCDP(1, S.ConeofCold)) then
    if Cast(S.CometStorm, Settings.Frost.GCDasOffGCD.CometStorm, nil, not Target:IsSpellInRange(S.CometStorm)) then addonTable.cast("Comet Storm") return "comet_storm single 2"; end
  end
  ...
  if S.Flurry:IsCastable() and (RemainingWintersChill == 0 and Target:DebuffDown(S.WintersChillDebuff) and ((Player:PrevGCDP(1, S.Frostbolt) and Icicles >= 3 or Player:PrevGCDP(1, S.Frostbolt) and Player:BuffUp(S.BrainFreezeBuff)) or Player:PrevGCDP(1, S.GlacialSpike) or S.GlacialSpike:IsAvailable() and Icicles == 4 and Player:BuffDown(S.FingersofFrostBuff))) then
    if Cast(S.Flurry, Settings.Frost.GCDasOffGCD.Flurry, nil, not Target:IsSpellInRange(S.Flurry)) then addonTable.cast("Flurry") return "flurry single 4"; end
  end
  ...
  if S.IceLance:IsReady() and (S.GlacialSpike:IsAvailable() and RemainingWintersChill == 0 and Icicles == 4 and Player:BuffUp(S.FingersofFrostBuff)) then
    if Cast(S.IceLance, nil, nil, not Target:IsSpellInRange(S.IceLance)) then addonTable.cast("Ice Lance") return "ice_lance single 6"; end
  end
  -- ray_of_frost,if=remaining_winters_chill=1
  if S.RayofFrost:IsCastable() and (RemainingWintersChill == 1) then
    if Cast(S.RayofFrost, Settings.Frost.GCDasOffGCD.RayOfFrost, nil, not Target:IsSpellInRange(S.RayofFrost)) then addonTable.cast("Ray of Frost") return "ray_of_frost single 8"; end
  end
  -- glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill)
  if S.GlacialSpike:IsReady() and (Icicles == 5 and (S.Flurry:Charges() >= 1 or RemainingWintersChill > 0)) then
    if Cast(S.GlacialSpike, nil, nil, not Target:IsSpellInRange(S.GlacialSpike)) then addonTable.cast("Glacial Spike") return "glacial_spike single 10"; end
  end
  -- frozen_orb,if=buff.fingers_of_frost.react<2&(!talent.ray_of_frost|cooldown.ray_of_frost.remains)
  if S.FrozenOrb:IsCastable() and (Player:BuffStackP(S.FingersofFrostBuff) < 2 and (not S.RayofFrost:IsAvailable() or S.RayofFrost:CooldownDown())) then
    if Cast(S.FrozenOrb, Settings.Frost.GCDasOffGCD.FrozenOrb, nil, not Target:IsInRange(40)) then addonTable.cast("Frozen Orb") return "frozen_orb single 12"; end
  end
  -- cone_of_cold,if=talent.coldest_snap&cooldown.comet_storm.remains>10&cooldown.frozen_orb.remains>10&remaining_winters_chill=0&active_enemies>=3
  if S.ConeofCold:IsCastable() and (S.ColdestSnap:IsAvailable() and S.CometStorm:CooldownRemains() > 10 and S.FrozenOrb:CooldownRemains() > 10 and RemainingWintersChill == 0 and EnemiesCount8ySplash >= 3) then
    if Cast(S.ConeofCold, nil, nil, not Target:IsInRange(12)) then addonTable.cast("Cone of Cold") return "cone_of_cold single 14"; end
  end
  -- blizzard,if=active_enemies>=2&talent.ice_caller&talent.freezing_rain&(!talent.splintering_cold&!talent.ray_of_frost|buff.freezing_rain.up|active_enemies>=3)
  if AoEON() and S.Blizzard:IsCastable() and (EnemiesCount8ySplash >= 2 and S.IceCaller:IsAvailable() and S.FreezingRain:IsAvailable() and (not S.SplinteringCold:IsAvailable() and not S.RayofFrost:IsAvailable() or Player:BuffUp(S.FreezingRainBuff) or EnemiesCount8ySplash >= 3)) then
    if Cast(S.Blizzard, Settings.Frost.GCDasOffGCD.Blizzard, nil, not Target:IsInRange(40)) then addonTable.cast("Blizzard") return "blizzard single 16"; end
  end
  -- shifting_power,if=cooldown.frozen_orb.remains>10&(!talent.comet_storm|cooldown.comet_storm.remains>10)&(!talent.ray_of_frost|cooldown.ray_of_frost.remains>10)|cooldown.icy_veins.remains<20
  if S.ShiftingPower:IsCastable() and (S.FrozenOrb:CooldownRemains() > 10 and (not S.CometStorm:IsAvailable() or S.CometStorm:CooldownRemains() > 10) and (not S.RayofFrost:IsAvailable() or S.RayofFrost:CooldownRemains() > 10) or S.IcyVeins:CooldownRemains() < 20) then
    if Cast(S.ShiftingPower, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(18)) then addonTable.cast("Shifting Power") return "shifting_power single 18"; end
  end
  -- glacial_spike,if=buff.icicles.react=5
  if S.GlacialSpike:IsCastable() and (Icicles == 5) then
    if Cast(S.GlacialSpike, nil, nil, not Target:IsSpellInRange(S.GlacialSpike)) then addonTable.cast("Glacial Spike") return "glacial_spike single 20"; end
  end
  ...
  if S.IceLance:IsReady() and (Player:BuffUp(S.FingersofFrostBuff) and not Player:PrevGCDP(1, S.GlacialSpike) or bool(RemainingWintersChill)) then
    if Cast(S.IceLance, nil, nil, not Target:IsSpellInRange(S.IceLance)) then addonTable.cast("Ice Lance") return "ice_lance single 22"; end
  end
  -- ice_nova,if=active_enemies>=4
  if AoEON() and S.IceNova:IsCastable() and (EnemiesCount16ySplash >= 4) then
    if Cast(S.IceNova, nil, nil, not Target:IsSpellInRange(S.IceNova)) then addonTable.cast("Ice Nova") return "ice_nova single 24"; end
  end
  -- frostbolt
  if S.Frostbolt:IsCastable() then
    if Cast(S.Frostbolt, nil, nil, not Target:IsSpellInRange(S.Frostbolt)) then addonTable.cast("Frostbolt") return "frostbolt single 26"; end
  end
  -- call_action_list,name=movement
  if Player:IsMoving() then
    local ShouldReturn = Movement(); if ShouldReturn then return ShouldReturn; end
...
addonTable.resetPixels()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.Flurry:IsCastable() and (HL.CombatTime() < 5 and (Player:IsCasting(S.Frostbolt) or Player:PrevGCDP(1, S.Frostbolt))) then
      if Cast(S.Flurry, Settings.Frost.GCDasOffGCD.Flurry, nil, not Target:IsSpellInRange(S.Flurry)) then addonTable.cast("Flurry") return "flurry opener"; end
    end
