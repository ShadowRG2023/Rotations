...
-- DO NOT REMOVE 
addonTable.spells = {
  { spell = "SPELL Touch of the Magi", name = "Touch of the Magi" },
  { spell = "SPELL Time Warp", name = "Time Warp" },
  { spell = "SPELL Shifting Power", name = "Shifting Power" },
  { spell = "SPELL Radiant Spark", name = "Radiant Spark" },
  { spell = "SPELL Presence of Mind", name = "Presence of Mind" },
  { spell = "SPELL Nether Tempest", name = "Nether Tempest" },
  { spell = "SPELL Mirror Image", name = "Mirror Image" },
  { spell = "SPELL Evocation", name = "Evocation" },
  { spell = "SPELL Counterspell", name = "Counterspell" },
  { spell = "SPELL Arcane Surge", name = "Arcane Surge" },
  { spell = "SPELL Arcane Orb", name = "Arcane Orb" },
  { spell = "SPELL Arcane Missiles", name = "Arcane Missiles" },
  { spell = "SPELL Arcane Intellect", name = "Arcane Intellect" },
  { spell = "SPELL Arcane Familiar", name = "Arcane Familiar" },
  { spell = "SPELL Arcane Blast", name = "Arcane Blast" },
  { spell = "SPELL Arcane Barrage", name = "Arcane Barrage" },
...
local function Precombat()
  ...
  if S.ArcaneFamiliar:IsCastable() and Player:BuffDown(S.ArcaneFamiliarBuff) then
    if Cast(S.ArcaneFamiliar) then addonTable.cast("Arcane Familiar") return "arcane_familiar precombat 2"; end
  end
  -- conjure_mana_gem
  ...
  if S.MirrorImage:IsCastable() and CDsON() and Settings.Arcane.MirrorImagesBeforePull then
    if Cast(S.MirrorImage, Settings.Arcane.GCDasOffGCD.MirrorImage) then addonTable.cast("Mirror Image") return "mirror_image precombat 6"; end
  end
  ...
  if S.ArcaneBlast:IsReady() and (not S.SiphonStorm:IsAvailable()) then
    if Cast(S.ArcaneBlast, nil, nil, not Target:IsSpellInRange(S.ArcaneBlast)) then addonTable.cast("Arcane Blast") return "arcane_blast precombat 8"; end
  end
  ...
  if S.Evocation:IsReady() and (S.SiphonStorm:IsAvailable()) then
    if Cast(S.Evocation) then addonTable.cast("Evocation") return "evocation precombat 10"; end
  end
end
...
local function AoeCooldownPhase()
  ...
  if S.TouchoftheMagi:IsReady() and (Player:PrevGCDP(1, S.ArcaneBarrage)) then
    if Cast(S.TouchoftheMagi, Settings.Arcane.GCDasOffGCD.TouchOfTheMagi, nil, not Target:IsInRange(40)) then addonTable.cast("Touch of the Magi") return "touch_of_the_magi aoe_cooldown_phase 2"; end
  end
  -- radiant_spark
  if S.RadiantSpark:IsReady() then
    if Cast(S.RadiantSpark, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsSpellInRange(S.RadiantSpark)) then addonTable.cast("Radiant Spark") return "radiant_spark aoe_cooldown_phase 4"; end
  end
  -- arcane_orb,if=buff.arcane_charge.stack<3,line_cd=1
  if S.ArcaneOrb:IsReady() and (Player:ArcaneCharges() < 3) then
    if Cast(S.ArcaneOrb, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Arcane Orb") return "arcane_orb aoe_cooldown_phase 6"; end
  end
  ...
  if S.NetherTempest:IsReady() and S.NetherTempest:TimeSinceLastCast() >= 30 and (S.ArcaneEcho:IsAvailable()) then
    if Cast(S.NetherTempest, nil, nil, not Target:IsSpellInRange(S.NetherTempest)) then addonTable.cast("Nether Tempest") return "nether_tempest aoe_cooldown_phase 8"; end
  end
  -- arcane_surge
  if S.ArcaneSurge:IsReady() then
    if Cast(S.ArcaneSurge, Settings.Arcane.GCDasOffGCD.ArcaneSurge) then addonTable.cast("Arcane Surge") return "arcane_surge aoe_cooldown_phase 10"; end
  end
  ...
  if S.ArcaneBarrage:IsReady() and (S.ArcaneSurge:CooldownRemains() < 75 and Target:DebuffStack(S.RadiantSparkVulnerability) == 4 and not S.OrbBarrage:IsAvailable()) then
    if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage aoe_cooldown_phase 12"; end
  end
  ...
  if S.ArcaneBarrage:IsReady() and ((Target:DebuffStack(S.RadiantSparkVulnerability) == 2 and S.ArcaneSurge:CooldownRemains() > 75) or (Target:DebuffStack(S.RadiantSparkVulnerability) == 1 and S.ArcaneSurge:CooldownRemains() < 75) and not S.OrbBarrage:IsAvailable()) then
    if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage aoe_cooldown_phase 14"; end
  end
  ...
  if S.ArcaneBarrage:IsReady() and ((Target:DebuffStack(S.RadiantSparkVulnerability) == 1 or Target:DebuffStack(S.RadiantSparkVulnerability) == 2 or (Target:DebuffStack(S.RadiantSparkVulnerability) == 3 and EnemiesCount8ySplash > 5) or Target:DebuffStack(S.RadiantSparkVulnerability) == 4) and Player:ArcaneCharges() == Player:ArcaneChargesMax() and S.OrbBarrage:IsAvailable()) then
    if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage aoe_cooldown_phase 16"; end
  end
  -- presence_of_mind
  if S.PresenceofMind:IsCastable() then
    if Cast(S.PresenceofMind, Settings.Arcane.OffGCDasOffGCD.PresenceOfMind) then addonTable.cast("Presence of Mind") return "presence_of_mind aoe_cooldown_phase 18"; end
  end
  ...
  if S.ArcaneBlast:IsReady() and (((Target:DebuffStack(S.RadiantSparkVulnerability) == 2 or Target:DebuffStack(S.RadiantSparkVulnerability) == 3) and not S.OrbBarrage:IsAvailable()) or (Target:DebuffUp(S.RadiantSparkVulnerability) and S.OrbBarrage:IsAvailable())) then
    if Cast(S.ArcaneBlast, nil, nil, not Target:IsSpellInRange(S.ArcaneBlast)) then addonTable.cast("Arcane Blast") return "arcane_blast aoe_cooldown_phase 20"; end
  end
  ...
  if S.ArcaneBarrage:IsReady() and ((Target:DebuffStack(S.RadiantSparkVulnerability) == 4 and Player:BuffUp(S.ArcaneSurgeBuff)) or (Target:DebuffStack(S.RadiantSparkVulnerability) == 3 and Player:BuffDown(S.ArcaneSurgeBuff)) and not S.OrbBarrage:IsAvailable()) then
    if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage aoe_cooldown_phase 22"; end
  end
end
...
local function AoeRotation()
  ...
  if S.ShiftingPower:IsReady() and ((not S.Evocation:IsAvailable() or S.Evocation:CooldownRemains() > 12) and (not S.ArcaneSurge:IsAvailable() or S.ArcaneSurge:CooldownRemains() > 12) and (not S.TouchoftheMagi:IsAvailable() or S.TouchoftheMagi:CooldownRemains() > 12) and Player:BuffDown(S.ArcaneSurgeBuff) and ((not S.ChargedOrb:IsAvailable() and S.ArcaneOrb:CooldownRemains() > 12) or (S.ArcaneOrb:Charges() == 0 or S.ArcaneOrb:CooldownRemains() > 12)) and Target:DebuffDown(S.TouchoftheMagiDebuff)) then
    if Cast(S.ShiftingPower, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(18)) then addonTable.cast("Shifting Power") return "shifting_power aoe_rotation 2"; end
  end
  ...
  if S.NetherTempest:IsReady() and (Target:DebuffRefreshable(S.NetherTempestDebuff) and Player:ArcaneCharges() == Player:ArcaneChargesMax() and Player:BuffDown(S.ArcaneSurgeBuff) and (EnemiesCount8ySplash > 6 or not S.OrbBarrage:IsAvailable()) and Target:DebuffDown(S.TouchoftheMagiDebuff)) then
    if Cast(S.NetherTempest, nil, nil, not Target:IsSpellInRange(S.NetherTempest)) then addonTable.cast("Nether Tempest") return "nether_tempest aoe_rotation 4"; end
  end
  ...
  if S.ArcaneMissiles:IsCastable() and (Player:BuffUp(S.ArcaneArtilleryBuff) and (S.TouchoftheMagi:CooldownRemains() + 5) > Player:BuffRemains(S.ArcaneArtilleryBuff)) then
    if Cast(S.ArcaneMissiles, nil, nil, not Target:IsSpellInRange(S.ArcaneMissiles)) then addonTable.cast("Arcane Missiles") return "arcane_missiles aoe_rotation 6"; end
  end
  ...
  if S.ArcaneBarrage:IsReady() and ((EnemiesCount8ySplash <= 4 and Player:ArcaneCharges() == 3) or Player:ArcaneCharges() == Player:ArcaneChargesMax() or Player:ManaPercentage() < 9) then
    if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage aoe_rotation 8"; end
  end
  ...
  if S.ArcaneOrb:IsReady() and (Player:ArcaneCharges() < 2 and S.TouchoftheMagi:CooldownRemains() > 18) then
    if Cast(S.ArcaneOrb, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Arcane Orb") return "arcane_orb aoe_rotation 10"; end
  end
  -- arcane_explosion
...
local function CooldownPhase()
  -- touch_of_the_magi,use_off_gcd=1,if=prev_gcd.1.arcane_barrage
  if S.TouchoftheMagi:IsReady() and (Player:PrevGCDP(1, S.ArcaneBarrage)) then
    if Cast(S.TouchoftheMagi, Settings.Arcane.GCDasOffGCD.TouchOfTheMagi, nil, not Target:IsInRange(40)) then addonTable.cast("Touch of the Magi") return "touch_of_the_magi cooldown_phase 2"; end
  end
  ...
  if S.ShiftingPower:IsReady() and (Player:BuffDown(S.ArcaneSurgeBuff) and not S.RadiantSpark:IsAvailable()) then
    if Cast(S.ShiftingPower, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(18)) then addonTable.cast("Shifting Power") return "shifting_power cooldown_phase 4"; end
  end
  ...
  if S.ArcaneOrb:IsReady() and ((S.RadiantSpark:CooldownUp() or (EnemiesCount8ySplash >= 2 and Target:DebuffDown(S.RadiantSparkVulnerability))) and Player:ArcaneCharges() < Player:ArcaneChargesMax()) then
    if Cast(S.ArcaneOrb, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Arcane Orb") return "arcane_orb cooldown_phase 6"; end
  end
  ...
  if S.ArcaneMissiles:IsReady() and (var_opener and Player:BuffUp(S.ClearcastingBuff) and S.RadiantSpark:CooldownRemains() < 5 and Player:BuffDown(S.NetherPrecisionBuff) and (Player:BuffDown(S.ArcaneArtilleryBuff) or Player:BuffRemains(S.ArcaneArtilleryBuff) <= (GCDMax * 6))) then
    if Cast(S.ArcaneMissiles, nil, nil, not Target:IsSpellInRange(S.ArcaneMissiles)) then addonTable.cast("Arcane Missiles") return "arcane_missiles cooldown_phase 8"; end
  end
  ...
  if S.ArcaneBlast:IsReady() and (var_opener and S.ArcaneSurge:CooldownUp() and Player:ManaPercentage() > 10 and Player:BuffRemains(S.SiphonStormBuff) > 17 and not Player:HasTier(30, 4)) then
    if Cast(S.ArcaneBlast, nil, nil, not Target:IsSpellInRange(S.ArcaneBlast)) then addonTable.cast("Arcane Blast") return "arcane_blast cooldown_phase 10"; end
  end
  ...
  if S.ArcaneMissiles:IsReady() and (S.RadiantSpark:CooldownUp() and Player:BuffUp(S.ClearcastingBuff) and (S.NetherPrecision:IsAvailable() and (Player:BuffDown(S.NetherPrecisionBuff) or Player:BuffRemains(S.NetherPrecisionBuff) < GCDMax * 3)) and (Player:BuffDown(S.ArcaneArtilleryBuff) or Player:BuffRemains(S.ArcaneArtilleryBuff) <= (GCDMax * 6))) then
    if Cast(S.ArcaneMissiles, nil, nil, not Target:IsSpellInRange(S.ArcaneMissiles)) then addonTable.cast("Arcane Missiles") return "arcane_missiles cooldown_phase 14"; end
  end
  -- radiant_spark
  if S.RadiantSpark:IsReady() then
    if Cast(S.RadiantSpark, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsSpellInRange(S.RadiantSpark)) then addonTable.cast("Radiant Spark") return "radiant_spark cooldown_phase 16"; end
  end
  -- nether_tempest,if=talent.arcane_echo,line_cd=30
  if S.NetherTempest:IsReady() and S.NetherTempest:TimeSinceLastCast() >= 30 and (S.ArcaneEcho:IsAvailable()) then
    if Cast(S.NetherTempest, nil, nil, not Target:IsSpellInRange(S.NetherTempest)) then addonTable.cast("Nether Tempest") return "nether_tempest cooldown_phase 18"; end
  end
  -- arcane_surge
  if S.ArcaneSurge:IsReady() then
    if Cast(S.ArcaneSurge, Settings.Arcane.GCDasOffGCD.ArcaneSurge) then addonTable.cast("Arcane Surge") return "arcane_surge cooldown_phase 20"; end
  end
  -- wait,sec=0.05,if=prev_gcd.1.arcane_surge,line_cd=15
  ...
  if S.ArcaneBarrage:IsReady() and (Player:PrevGCDP(1, S.ArcaneSurge) or Player:PrevGCDP(1, S.NetherTempest) or Player:PrevGCDP(1, S.RadiantSpark) or (EnemiesCount8ySplash >= (4 - 2 * num(S.OrbBarrage:IsAvailable())) and Target:DebuffStack(S.RadiantSparkVulnerability) == 4 and S.ArcingCleave:IsAvailable())) then
    if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage cooldown_phase 22"; end
  end
  ...
  if S.ArcaneBlast:IsReady() and (Target:DebuffUp(S.RadiantSparkVulnerability) and (Target:DebuffStack(S.RadiantSparkVulnerability) < 4 or (var_blast_below_gcd and Target:DebuffStack(S.RadiantSparkVulnerability) == 4))) then
    if Cast(S.ArcaneBlast, nil, nil, not Target:IsSpellInRange(S.ArcaneBlast)) then addonTable.cast("Arcane Blast") return "arcane_blast cooldown_phase 24"; end
  end
  ...
  if S.PresenceofMind:IsCastable() and (Target:DebuffRemains(S.TouchoftheMagiDebuff) <= GCDMax) then
    if Cast(S.PresenceofMind, Settings.Arcane.OffGCDasOffGCD.PresenceOfMind) then addonTable.cast("Presence of Mind") return "presence_of_mind cooldown_phase 26"; end
  end
  -- arcane_blast,if=buff.presence_of_mind.up
  if S.ArcaneBlast:IsReady() and (Player:BuffUp(S.PresenceofMindBuff)) then
    if Cast(S.ArcaneBlast, nil, nil, not Target:IsSpellInRange(S.ArcaneBlast)) then addonTable.cast("Arcane Blast") return "arcane_blast cooldown_phase 28"; end
  end
  ...
  if S.ArcaneMissiles:IsReady() and (((Player:BuffDown(S.NetherPrecisionBuff) and Player:BuffUp(S.ClearcastingBuff)) or (Player:BuffStack(S.ClearcastingBuff) > 2 and Target:DebuffUp(S.TouchoftheMagiDebuff))) and (Target:DebuffDown(S.RadiantSparkVulnerability) or (Target:DebuffStack(S.RadiantSparkVulnerability) == 4 and Player:PrevGCDP(1, S.ArcaneBlast)))) then
    if Cast(S.ArcaneMissiles, nil, nil, not Target:IsSpellInRange(S.ArcaneMissiles)) then addonTable.cast("Arcane Missiles") return "arcane_missiles cooldown_phase 30"; end
  end
  -- arcane_blast
  if S.ArcaneBlast:IsReady() then
    if Cast(S.ArcaneBlast, nil, nil, not Target:IsSpellInRange(S.ArcaneBlast)) then addonTable.cast("Arcane Blast") return "arcane_blast cooldown_phase 32"; end
  end
end
...
local function Rotation()
  ...
  if S.ArcaneOrb:IsReady() and (Player:ArcaneCharges() < 3 and (Player:BloodlustDown() or Player:ManaPercentage() > 70)) then
    if Cast(S.ArcaneOrb, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Arcane Orb") return "arcane_orb rotation 2"; end
  end
  ...
  if S.ShiftingPower:IsReady() and (Player:BuffDown(S.ArcaneSurgeBuff) and S.ArcaneSurge:CooldownRemains() > 45 and FightRemains > 15) then
    if Cast(S.ShiftingPower, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(18)) then addonTable.cast("Shifting Power") return "shifting_power rotation 6"; end
  end
  ...
  if S.NetherTempest:IsReady() and (Target:DebuffRefreshable(S.NetherTempestDebuff) and Player:ArcaneCharges() == Player:ArcaneChargesMax() and (((Player:BuffUp(S.TemporalWarpBuff) or Player:ManaPercentage() < 10 or not S.ShiftingPower:IsAvailable()) and Player:BuffDown(S.ArcaneSurgeBuff)) or I.NeltharionsCalltoChaos:IsEquipped()) and not var_opener and FightRemains >= 12) then
    if Cast(S.NetherTempest, nil, nil, not Target:IsSpellInRange(S.NetherTempest)) then addonTable.cast("Nether Tempest") return "nether_tempest rotation 8"; end
  end
  ...
  if S.ArcaneBarrage:IsReady() and (Player:ArcaneCharges() == Player:ArcaneChargesMax() and Player:ManaPercentage() < 70 and (((S.ArcaneSurge:CooldownRemains() > 30 and S.TouchoftheMagi:CooldownRemains() > 10) and Player:BloodlustUp() and S.TouchoftheMagi:CooldownRemains() > 5 and FightRemains > 30) or (not S.Evocation:IsAvailable() and FightRemains > 20))) then
    if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage rotation 10"; end
  end
  ...
  if S.PresenceofMind:IsCastable() and (Player:ArcaneCharges() < 3 and Target:HealthPercentage() < 35 and S.ArcaneBombardment:IsAvailable()) then
    if Cast(S.PresenceofMind, Settings.Arcane.OffGCDasOffGCD.PresenceOfMind) then addonTable.cast("Presence of Mind") return "presence_of_mind rotation 12"; end
  end
  ...
  if S.ArcaneBlast:IsReady() and ((Player:ArcaneCharges() == Player:ArcaneChargesMax() and Player:BuffUp(S.NetherPrecisionBuff)) or (S.TimeAnomaly:IsAvailable() and Player:BuffUp(S.ArcaneSurgeBuff) and Player:BuffRemains(S.ArcaneSurgeBuff) <= 6)) then
    if Cast(S.ArcaneBlast, nil, nil, not Target:IsSpellInRange(S.ArcaneBlast)) then addonTable.cast("Arcane Blast") return "arcane_blast rotation 14"; end
  end
  ...
  if S.ArcaneMissiles:IsCastable() and (Player:BuffUp(S.ClearcastingBuff) and Player:BuffDown(S.NetherPrecisionBuff) and (not var_opener or (I.BelorrelostheSuncaller:IsEquipped() and var_steroid_trinket_equipped))) then
    if Cast(S.ArcaneMissiles, nil, nil, not Target:IsSpellInRange(S.ArcaneMissiles)) then addonTable.cast("Arcane Missiles") return "arcane_missiles rotation 18"; end
  end
  -- arcane_blast
  if S.ArcaneBlast:IsReady() then
    if Cast(S.ArcaneBlast, nil, nil, not Target:IsSpellInRange(S.ArcaneBlast)) then addonTable.cast("Arcane Blast") return "arcane_blast rotation 20"; end
  end
  -- arcane_barrage
  if S.ArcaneBarrage:IsReady() then
    if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage rotation 22"; end
  end
end
...
addonTable.resetPixels()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.ArcaneIntellect:IsCastable() and Everyone.GroupBuffMissing(S.ArcaneIntellect) then
      if Cast(S.ArcaneIntellect, Settings.CommonsOGCD.GCDasOffGCD.ArcaneIntellect) then addonTable.cast("Arcane Intellect") return "arcane_intellect group_buff"; end
    end
    -- call precombat
    ...
    if CDsON() then
      ...
      if S.TimeWarp:IsReady() and Settings.Commons.UseTemporalWarp and Player:BloodlustDown() and (S.TemporalWarp:IsAvailable() and Player:BloodlustExhaustUp() and (S.ArcaneSurge:CooldownUp() or FightRemains <= 40 or (Player:BuffUp(S.ArcaneSurgeBuff) and FightRemains <= (S.ArcaneSurge:CooldownRemains() + 14)))) then
        if Cast(S.TimeWarp, Settings.CommonsOGCD.OffGCDasOffGCD.TimeWarp) then addonTable.cast("Time Warp") return "time_warp main 4"; end
      end
      ...
      if S.LightsJudgment:IsCastable() and (Player:BuffDown(S.ArcaneSurgeBuff) and Target:DebuffDown(S.TouchoftheMagiDebuff) and EnemiesCount8ySplash >= 2) then
        if Cast(S.LightsJudgment, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsSpellInRange(S.LightsJudgment)) then addonTable.cast("Light's Judgment") return "lights_judgment main 6"; end
      end
      ...
      if S.Berserking:IsCastable() and ((Player:PrevGCDP(1, S.ArcaneSurge) and not (Player:BuffUp(S.TemporalWarpBuff) and Player:BloodlustUp())) or Player:BuffUp(S.ArcaneSurgeBuff) and Target:DebuffUp(S.TouchoftheMagiDebuff)) then
        if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Berserking") return "berserking main 8"; end
      end
      if Player:PrevGCDP(1, S.ArcaneSurge) then
        -- blood_fury,if=prev_gcd.1.arcane_surge
        if S.BloodFury:IsCastable() then
          if Cast(S.BloodFury, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Blood Fury") return "blood_fury main 10"; end
        end
        -- fireblood,if=prev_gcd.1.arcane_surge
        if S.Fireblood:IsCastable() then
          if Cast(S.Fireblood, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Fireblood") return "fireblood main 12"; end
        end
        -- ancestral_call,if=prev_gcd.1.arcane_surge
        if S.AncestralCall:IsCastable() then
          if Cast(S.AncestralCall, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Ancestral Call") return "ancestral_call main 14"; end
        end
      end
    ...
    if S.TouchoftheMagi:IsReady() and (Player:PrevGCDP(1, S.ArcaneBarrage)) then
      if Cast(S.TouchoftheMagi, Settings.Arcane.GCDasOffGCD.TouchOfTheMagi, nil, not Target:IsInRange(40)) then addonTable.cast("Touch of the Magi") return "touch_of_the_magi main 54"; end
    end
    ...
    if S.ArcaneBarrage:IsReady() and (FightRemains < 2) then
      if Cast(S.ArcaneBarrage, nil, nil, not Target:IsSpellInRange(S.ArcaneBarrage)) then addonTable.cast("Arcane Barrage") return "arcane_barrage main 58"; end
    end
    ...
    if S.Evocation:IsCastable() and (Player:BuffDown(S.ArcaneSurgeBuff) and Target:DebuffDown(S.TouchoftheMagiDebuff) and ((Player:ManaPercentage() < 10 and S.TouchoftheMagi:CooldownRemains() < 20) or S.TouchoftheMagi:CooldownRemains() < 15) and ((Player:BloodlustRemains() < 31 and Player:BloodlustUp()) or not var_belor_extended_opener or not var_opener)) then
      if Cast(S.Evocation, Settings.Arcane.GCDasOffGCD.Evocation) then addonTable.cast("Evocation") return "evocation main 60"; end
    end
