...
-- DO NOT REMOVE 
addonTable.spells = {
  { spell = "MACRO OptiAction1", name = "Flamestrike" },
  { spell = "MACRO OptiAction2", name = "Meteor" },
  { spell = "SPELL Time Warp", name = "Time Warp" },
  { spell = "SPELL Shifting Power", name = "Shifting Power" },
  { spell = "SPELL Scorch", name = "Scorch" },
  { spell = "SPELL Pyroblast", name = "Pyroblast" },
  { spell = "SPELL Phoenix Flames", name = "Phoenix Flames" },
  { spell = "SPELL Mirror Image", name = "Mirror Image" },
  --{ spell = "SPELL Meteor", name = "Meteor" },
  { spell = "SPELL Living Bomb", name = "Living Bomb" },
  { spell = "SPELL Ice Nova", name = "Ice Nova" },
  { spell = "SPELL Flamestrike", name = "Flamestrike" },
  { spell = "SPELL Fireball", name = "Fireball" },
  { spell = "SPELL Fire Blast", name = "Fire Blast" },
  { spell = "SPELL Dragon's Breath", name = "Dragon's Breath" },
  { spell = "SPELL Combustion", name = "Combustion" },
  { spell = "SPELL Counterspell", name = "Counterspell" },
  ...
  { spell = "SPELL Arcane Intellect", name = "Arcane Intellect" },
  { spell = "SPELL Arcane Explosion", name = "Arcane Explosion" },
...
local function Interrupt()
  if Target:IsInterruptible() then
    if S.Counterspell:IsCastable() and Target:IsSpellInRange(S.Counterspell) and addonTable.config.Interrupt then
      if Cast(S.Counterspell) then
        addonTable.cast("Counterspell")
...
local function Precombat()
  ...
  if S.ArcaneIntellect:IsCastable() and Everyone.GroupBuffMissing(S.ArcaneIntellect) then
    if Cast(S.ArcaneIntellect, Settings.CommonsOGCD.GCDasOffGCD.ArcaneIntellect) then addonTable.cast("Arcane Intellect") return "arcane_intellect precombat 2"; end
  end
  ...
  if CDsON() and S.MirrorImage:IsCastable() and Settings.Fire.MirrorImagesBeforePull then
    if Cast(S.MirrorImage, Settings.Fire.GCDasOffGCD.MirrorImage) then addonTable.cast("Mirror Image") return "mirror_image precombat 2"; end
  end
  ...
  if S.Pyroblast:IsReady() and not Player:IsCasting(S.Pyroblast) then
    if Cast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast precombat 4"; end
  end
  -- Manually added: fireball
  if S.Fireball:IsReady() then
    if Cast(S.Fireball, nil, nil, not Target:IsSpellInRange(S.Fireball)) then addonTable.cast("Fireball") return "fireball precombat 6"; end
  end
end
...
local function ActiveTalents()
  ...
  if S.LivingBomb:IsReady() and (EnemiesCount10ySplash > 1 and CombustionDown and (var_time_to_combustion > S.LivingBomb:CooldownRemains() or var_time_to_combustion <= 0)) then
    if Cast(S.LivingBomb, Settings.Fire.GCDasOffGCD.LivingBomb, nil, not Target:IsSpellInRange(S.LivingBomb)) then addonTable.cast("Living Bomb") return "living_bomb active_talents 2"; end
  end
  ...
  if S.Meteor:IsReady() and (var_time_to_combustion <= 0 or CombustionRemains > S.Meteor:TravelTime() or not S.SunKingsBlessing:IsAvailable() and (45 < var_time_to_combustion or FightRemains < var_time_to_combustion)) then
    if Cast(S.Meteor, Settings.Fire.GCDasOffGCD.Meteor, nil, not Target:IsInRange(40)) then addonTable.cast("Meteor") return "meteor active_talents 4"; end
  end
  ...
  if S.DragonsBreath:IsReady() and (S.AlexstraszasFury:IsAvailable() and (CombustionDown and Player:BuffDown(S.HotStreakBuff)) and (Player:BuffUp(S.FeeltheBurnBuff) or HL.CombatTime() > 15) and not ImprovedScorchActive() and FirestarterRemains() == 0 and not S.TemperedFlames:IsAvailable()) then
    if Settings.Fire.StayDistance and not Target:IsInRange(12) then
      ...
      if Cast(S.DragonsBreath, Settings.Fire.GCDasOffGCD.DragonsBreath) then addonTable.cast("Dragon's Breath") return "dragons_breath active_talents 6"; end
    end
  end
  ...
  if S.DragonsBreath:IsReady() and (S.AlexstraszasFury:IsAvailable() and (CombustionDown and Player:BuffDown(S.HotStreakBuff)) and (Player:BuffUp(S.FeeltheBurnBuff) or HL.CombatTime() > 15) and not ImprovedScorchActive() and S.TemperedFlames:IsAvailable()) then
    if Settings.Fire.StayDistance and not Target:IsInRange(12) then
      ...
      if Cast(S.DragonsBreath, Settings.Fire.GCDasOffGCD.DragonsBreath) then addonTable.cast("Dragon's Breath") return "dragons_breath active_talents 8"; end
    end
  end
end
...
local function CombustionCooldowns()
  ...
  if S.BloodFury:IsCastable() then
    if Cast(S.BloodFury, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Blood Fury") return "blood_fury combustion_cooldowns 4"; end
  end
  -- berserking,if=buff.combustion.up
  if S.Berserking:IsCastable() and (CombustionUp) then
    if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Berserking") return "berserking combustion_cooldowns 6"; end
  end
  -- fireblood
  if S.Fireblood:IsCastable() then
    if Cast(S.Fireblood, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Fireblood") return "fireblood combustion_cooldowns 8"; end
  end
  -- ancestral_call
  if S.AncestralCall:IsCastable() then
    if Cast(S.AncestralCall, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Ancestral Call") return "ancestral_call combustion_cooldowns 10"; end
  end
  ...
  if S.TimeWarp:IsReady() and Settings.Commons.UseTemporalWarp and (S.TemporalWarp:IsAvailable() and Player:BloodlustExhaustUp()) then
    if Cast(S.TimeWarp, Settings.CommonsOGCD.OffGCDasOffGCD.TimeWarp) then addonTable.cast("Time Warp") return "time_warp combustion_cooldowns 12"; end
  end
...
local function CombustionPhase()
  ...
  if CDsON() and S.LightsJudgment:IsCastable() and (CombustionDown) then
    if Cast(S.LightsJudgment, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Light's Judgment") return "lights_judgment combustion_phase 2"; end
  end
  -- bag_of_tricks,if=buff.combustion.down
  if CDsON() and S.BagofTricks:IsCastable() and (CombustionDown) then
    if Cast(S.BagofTricks, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Bag of Tricks") return "bag_of_tricks combustion_phase 4"; end
  end
  ...
  if S.LivingBomb:IsReady() and AoEON() and (EnemiesCount10ySplash > 1 and CombustionDown) then
    if Cast(S.LivingBomb, Settings.Fire.GCDasOffGCD.LivingBomb, nil, not Target:IsSpellInRange(S.LivingBomb)) then addonTable.cast("Living Bomb") return "living_bomb combustion_phase 6"; end
  end
  ...
  if S.PhoenixFlames:IsCastable() and (Player:BuffDown(S.CombustionBuff) and Player:HasTier(30, 2) and not S.PhoenixFlames:InFlight() and Target:DebuffRemains(S.CharringEmbersDebuff) < 4 * GCDMax and Player:BuffDown(S.HotStreakBuff)) then
    if Cast(S.PhoenixFlames, nil, nil, not Target:IsSpellInRange(S.PhoenixFlames)) then addonTable.cast("Phoenix Flames") return "phoenix_flames combustion_phase 8"; end
  end
  ...
  if S.Combustion:IsReady() and (HotStreakInFlight() == 0 and CombustionDown and var_time_to_combustion <= 0 and (Player:IsCasting(S.Scorch) and S.Scorch:ExecuteRemains() < var_combustion_cast_remains or Player:IsCasting(S.Fireball) and S.Fireball:ExecuteRemains() < var_combustion_cast_remains or Player:IsCasting(S.Pyroblast) and S.Pyroblast:ExecuteRemains() < var_combustion_cast_remains or Player:IsCasting(S.Flamestrike) and S.Flamestrike:ExecuteRemains() < var_combustion_cast_remains or S.Meteor:InFlight() and S.Meteor:InFlightRemains() < var_combustion_cast_remains)) then
    if Cast(S.Combustion, Settings.Fire.OffGCDasOffGCD.Combustion) then addonTable.cast("Combustion") return "combustion combustion_phase 10"; end
  end
  ...
  if AoEON() and S.Flamestrike:IsReady() and not Player:IsCasting(S.Flamestrike) and (CombustionDown and Player:BuffUp(S.FuryoftheSunKingBuff) and Player:BuffRemains(S.FuryoftheSunKingBuff) > S.Flamestrike:CastTime() and S.Combustion:CooldownRemains() < S.Flamestrike:CastTime() and EnemiesCount8ySplash >= var_skb_flamestrike) then
    if Cast(S.Flamestrike, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Flamestrike") return "flamestrike combustion_phase 12"; end
  end
  ...
  if S.Pyroblast:IsReady() and not Player:IsCasting(S.Pyroblast) and (CombustionDown and Player:BuffUp(S.FuryoftheSunKingBuff) and Player:BuffRemains(S.FuryoftheSunKingBuff) > S.Pyroblast:CastTime()) then
    if Cast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast combustion_phase 14"; end
  end
  ...
  if S.Fireball:IsReady() and (CombustionDown and S.Combustion:CooldownRemains() < S.Fireball:CastTime() and EnemiesCount8ySplash < 2 and not ImprovedScorchActive()) then
    if Cast(S.Fireball, nil, nil, not Target:IsSpellInRange(S.Fireball)) then addonTable.cast("Fireball") return "fireball combustion_phase 16"; end
  end
  ...
  if S.Scorch:IsReady() and (CombustionDown and S.Combustion:CooldownRemains() < S.Scorch:CastTime()) then
    if Cast(S.Scorch, nil, nil, not Target:IsSpellInRange(S.Scorch)) then addonTable.cast("Scorch") return "scorch combustion_phase 18"; end
  end
  ...
  if AoEON() and S.Flamestrike:IsReady() and ((Player:BuffUp(S.HotStreakBuff) and EnemiesCount8ySplash >= var_combustion_flamestrike) or (Player:BuffUp(S.HyperthermiaBuff) and EnemiesCount8ySplash >= var_combustion_flamestrike - num(S.Hyperthermia:IsAvailable()))) then
    if Cast(S.Flamestrike, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Flamestrike") return "flamestrike combustion_phase 22"; end
  end
  -- pyroblast,if=buff.hyperthermia.react
  if S.Pyroblast:IsReady() and (Player:BuffUp(S.HyperthermiaBuff)) then
    if PBCast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast combustion_phase 24"; end
  end
  ...
  if S.Pyroblast:IsReady() and (Player:BuffUp(S.HotStreakBuff) and CombustionUp) then
    if PBCast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast combustion_phase 26"; end
  end
  ...
  if S.Pyroblast:IsReady() and (Player:PrevGCDP(1, S.Scorch) and Player:BuffUp(S.HeatingUpBuff) and EnemiesCount8ySplash < var_combustion_flamestrike and CombustionUp) then
    if PBCast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast combustion_phase 28"; end
  end
  ...
  if S.ShiftingPower:IsReady() and (CombustionUp and S.FireBlast:Charges() == 0 and (S.PhoenixFlames:Charges() < S.PhoenixFlames:MaxCharges() or S.AlexstraszasFury:IsAvailable()) and var_combustion_shifting_power <= EnemiesCount8ySplash) then
    if Cast(S.ShiftingPower, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(18)) then addonTable.cast("Shifting Power") return "shifting_power combustion_phase 30"; end
  end
  ...
  if AoEON() and S.Flamestrike:IsReady() and not Player:IsCasting(S.Flamestrike) and (Player:BuffUp(S.FuryoftheSunKingBuff) and Player:BuffRemains(S.FuryoftheSunKingBuff) > S.Flamestrike:CastTime() and EnemiesCount8ySplash >= var_skb_flamestrike) then
    if Cast(S.Flamestrike, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Flamestrike") return "flamestrike combustion_phase 32"; end
  end
  ...
  if S.Pyroblast:IsReady() and not Player:IsCasting(S.Pyroblast) and (Player:BuffUp(S.FuryoftheSunKingBuff) and Player:BuffRemains(S.FuryoftheSunKingBuff) > S.Pyroblast:CastTime()) then
    if Cast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast combustion_phase 34"; end
  end
  ...
  if S.Scorch:IsReady() and (ImprovedScorchActive() and (Target:DebuffRemains(S.ImprovedScorchDebuff) < 4 * GCDMax) and EnemiesCount16ySplash < var_combustion_flamestrike) then
    if Cast(S.Scorch, nil, nil, not Target:IsSpellInRange(S.Scorch)) then addonTable.cast("Scorch") return "scorch combustion_phase 36"; end
  end
  ...
  if S.PhoenixFlames:IsCastable() and (Player:HasTier(30, 2) and S.PhoenixFlames:TravelTime() < CombustionRemains and num(Player:BuffUp(S.HeatingUpBuff)) + HotStreakInFlight() < 2 and (Target:DebuffRemains(S.CharringEmbersDebuff) < 4 * GCDMax or Player:BuffStack(S.FlamesFuryBuff) > 1 or Player:BuffUp(S.FlamesFuryBuff))) then
    if Cast(S.PhoenixFlames, nil, nil, not Target:IsSpellInRange(S.PhoenixFlames)) then addonTable.cast("Phoenix Flames") return "phoenix_flames combustion_phase 38"; end
  end
  ...
  if S.Fireball:IsReady() and (CombustionRemains > S.Fireball:CastTime() and Player:BuffUp(S.FlameAccelerantBuff)) then
    if Cast(S.Fireball, nil, nil, not Target:IsSpellInRange(S.Fireball)) then addonTable.cast("Fireball") return "fireball combustion_phase 40"; end
  end
  ...
  if S.PhoenixFlames:IsCastable() and ((not S.AlexstraszasFury:IsAvailable() and S.PhoenixFlames:TravelTime() < CombustionRemains or S.AlexstraszasFury:IsAvailable()) and num(Player:BuffUp(S.HeatingUpBuff)) + HotStreakInFlight() < 2) then
    if Cast(S.PhoenixFlames, nil, nil, not Target:IsSpellInRange(S.PhoenixFlames)) then addonTable.cast("Phoenix Flames") return "phoenix_flames combustion_phase 42"; end
  end
  ...
  if S.Scorch:IsReady() and (CombustionRemains > S.Scorch:CastTime() and S.Scorch:CastTime() >= Player:GCD()) then
    if Cast(S.Scorch, nil, nil, not Target:IsSpellInRange(S.Scorch)) then addonTable.cast("Scorch") return "scorch combustion_phase 44"; end
  end
  -- fireball,if=buff.combustion.remains>cast_time
  if S.Fireball:IsReady() and (CombustionRemains > S.Fireball:CastTime()) then
    if Cast(S.Fireball, nil, nil, not Target:IsSpellInRange(S.Fireball)) then addonTable.cast("Fireball") return "fireball combustion_phase 46"; end
  end
  ...
  if S.LivingBomb:IsReady() and (CombustionRemains < GCDMax and EnemiesCount10ySplash > 1) then
    if Cast(S.LivingBomb, Settings.Fire.GCDasOffGCD.LivingBomb, nil, not Target:IsSpellInRange(S.LivingBomb)) then addonTable.cast("Living Bomb") return "living_bomb combustion_phase 48"; end
  end
end
...
local function FirestarterFireBlasts()
  ...
  if S.FireBlast:IsReady() and not FreeCastAvailable() and (not var_fire_blast_pooling and Player:BuffDown(S.HotStreakBuff) and num(Player:BuffUp(S.HeatingUpBuff)) + HotStreakInFlight() == 1 and (S.ShiftingPower:CooldownUp() or S.FireBlast:Charges() > 1 or Player:BuffRemains(S.FeeltheBurnBuff) < 2 * GCDMax)) then
    if FBCast(S.FireBlast) then addonTable.cast("Fire Blast") return "fire_blast firestarter_fire_blasts 2"; end
  end
...
local function StandardRotation()
  ...
  if AoEON() and S.Flamestrike:IsReady() and (EnemiesCount8ySplash >= var_hot_streak_flamestrike and FreeCastAvailable()) then
    if Cast(S.Flamestrike, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Flamestrike") return "flamestrike standard_rotation 2"; end
  end
  -- pyroblast,if=buff.hyperthermia.react
  ...
  if S.Pyroblast:IsReady() and (FreeCastAvailable()) then
    if PBCast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast standard_rotation 4"; end
  end
  ...
  if AoEON() and S.Flamestrike:IsReady() and not Player:IsCasting(S.Flamestrike) and (EnemiesCount8ySplash >= var_skb_flamestrike and Player:BuffUp(S.FuryoftheSunKingBuff)) then
    if Cast(S.Flamestrike, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Flamestrike") return "flamestrike standard_rotation 12"; end
  end
  ...
  if S.Scorch:IsReady() and (ImprovedScorchActive() and Target:DebuffRemains(S.ImprovedScorchDebuff) < S.Pyroblast:CastTime() + 5 * GCDMax and Player:BuffUp(S.FuryoftheSunKingBuff) and not Player:IsCasting(S.Scorch)) then
    if Cast(S.Scorch, nil, nil, not Target:IsSpellInRange(S.Scorch)) then addonTable.cast("Scorch") return "scorch standard_rotation 13"; end
  end
  ...
  if S.Pyroblast:IsReady() and not Player:IsCasting(S.Pyroblast) and (Player:BuffUp(S.FuryoftheSunKingBuff)) then
    if Cast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast standard_rotation 14"; end
  end
  ...
  if S.Pyroblast:IsReady() and (((Player:IsCasting(S.Scorch) and Player:BuffUp(S.HeatingUpBuff)) or (Player:PrevGCDP(1, S.Scorch) and Player:BuffUp(S.HotStreakBuff))) and SearingTouchActive() and EnemiesCount8ySplash < var_hot_streak_flamestrike) then
    if PBCast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast standard_rotation 18"; end
  end
  ...
  if S.Scorch:IsReady() and (ImprovedScorchActive() and Target:DebuffRemains(S.ImprovedScorchDebuff) < 4 * GCDMax) then
    if Cast(S.Scorch, nil, nil, not Target:IsSpellInRange(S.Scorch)) then addonTable.cast("Scorch") return "scorch standard_rotation 19"; end
  end
  ...
  if S.PhoenixFlames:IsCastable() and (S.AlexstraszasFury:IsAvailable() and (not S.FeeltheBurn:IsAvailable() or Player:BuffRemains(S.FeeltheBurnBuff) < 2 * GCDMax)) then
    if Cast(S.PhoenixFlames, nil, nil, not Target:IsSpellInRange(S.PhoenixFlames)) then addonTable.cast("Phoenix Flames") return "phoenix_flames standard_rotation 20"; end
  end
  ...
  if S.PhoenixFlames:IsCastable() and (Player:HasTier(30, 2) and Target:DebuffRemains(S.CharringEmbersDebuff) < 2 * GCDMax and Player:BuffDown(S.HotStreakBuff)) then
    if Cast(S.PhoenixFlames, nil, nil, not Target:IsSpellInRange(S.PhoenixFlames)) then addonTable.cast("Phoenix Flames") return "phoenix_flames standard_rotation 21"; end
  end
  ...
  if S.Scorch:IsReady() and (ImprovedScorchActive() and Target:DebuffStack(S.ImprovedScorchDebuff) < var_improved_scorch_max_stack) then
    if Cast(S.Scorch, nil, nil, not Target:IsSpellInRange(S.Scorch)) then addonTable.cast("Scorch") return "scorch standard_rotation 22"; end
  end
  ...
  if S.PhoenixFlames:IsCastable() and (not S.AlexstraszasFury:IsAvailable() and Player:BuffDown(S.HotStreakBuff) and not var_phoenix_pooling and Player:BuffUp(S.FlamesFuryBuff)) then
    if Cast(S.PhoenixFlames, nil, nil, not Target:IsSpellInRange(S.PhoenixFlames)) then addonTable.cast("Phoenix Flames") return "phoenix_flames standard_rotation 24"; end
  end
  ...
  if S.PhoenixFlames:IsCastable() and (S.AlexstraszasFury:IsAvailable() and Player:BuffDown(S.HotStreakBuff) and HotStreakInFlight() == 0 and (not var_phoenix_pooling and Player:BuffUp(S.FlamesFuryBuff) or S.PhoenixFlames:ChargesFractional() > 2.5 or S.PhoenixFlames:ChargesFractional() > 1.5 and (not S.FeeltheBurn:IsAvailable() or Player:BuffRemains(S.FeeltheBurnBuff) < 3 * GCDMax))) then
    if Cast(S.PhoenixFlames, nil, nil, not Target:IsSpellInRange(S.PhoenixFlames)) then addonTable.cast("Phoenix Flames") return "phoenix_flames standard_rotation 26"; end
  end
  -- call_action_list,name=active_talents
  local ShouldReturn = ActiveTalents(); if ShouldReturn then return ShouldReturn; end
  ...
  if AoEON() and S.DragonsBreath:IsReady() and (EnemiesCount16ySplash > 1 and S.AlexstraszasFury:IsAvailable()) then
    if Settings.Fire.StayDistance and not Target:IsInRange(12) then
      ...
      if Cast(S.DragonsBreath, Settings.Fire.GCDasOffGCD.DragonsBreath) then addonTable.cast("Dragon's Breath") return "dragons_breath standard_rotation 28"; end
    end
  end
  -- scorch,if=searing_touch.active
  if S.Scorch:IsReady() and (SearingTouchActive()) then
    if Cast(S.Scorch, nil, nil, not Target:IsSpellInRange(S.Scorch)) then addonTable.cast("Scorch") return "scorch standard_rotation 30"; end
  end
  ...
  if AoEON() and S.ArcaneExplosion:IsReady() and (EnemiesCount10yMelee >= var_arcane_explosion and Player:ManaPercentageP() >= var_arcane_explosion_mana) then
    if Settings.Fire.StayDistance and not Target:IsInRange(10) then
      ...
      if Cast(S.ArcaneExplosion) then addonTable.cast("Arcane Explosion") return "arcane_explosion standard_rotation 32"; end
    end
  end
  ...
  if AoEON() and S.Flamestrike:IsReady() and (EnemiesCount8ySplash >= var_hard_cast_flamestrike) then
    if Cast(S.Flamestrike, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Flamestrike") return "flamestrike standard_rotation 34"; end
  end
  ...
  if S.Pyroblast:IsReady() and (S.TemperedFlames:IsAvailable() and Player:BuffDown(S.FlameAccelerantBuff)) then
    if Cast(S.Pyroblast, nil, nil, not Target:IsSpellInRange(S.Pyroblast)) then addonTable.cast("Pyroblast") return "pyroblast standard_rotation 35"; end
  end
  -- fireball
  if S.Fireball:IsReady() and (not FreeCastAvailable()) then
    if Cast(S.Fireball, nil, nil, not Target:IsSpellInRange(S.Fireball)) then addonTable.cast("Fireball") return "fireball standard_rotation 36"; end
  end
end
...
addonTable.resetPixels()
  ...
  if Everyone.TargetIsValid() then
    ...
    if CDsON() and S.TimeWarp:IsReady() and Settings.Commons.UseTemporalWarp and (Player:BloodlustExhaustUp() and S.TemporalWarp:IsAvailable() and (FirestarterActive() or FightRemains < 40)) then
      if Cast(S.TimeWarp, Settings.CommonsOGCD.OffGCDasOffGCD.TimeWarp) then addonTable.cast("Time Warp") return "time_warp main 2"; end
    end
    ...
    if S.FireBlast:IsReady() and not FreeCastAvailable() and (Player:IsChanneling(S.ShiftingPower) and S.FireBlast:FullRechargeTime() < ShiftingPowerTickReduction) then
      if FBCast(S.FireBlast) then addonTable.cast("Fire Blast") return "fire_blast main 30"; end
    end
    ...
    if S.ShiftingPower:IsReady() and (CombustionDown and (S.FireBlast:Charges() == 0 or var_fire_blast_pooling) and (not ImprovedScorchActive() or Target:DebuffRemains(S.ImprovedScorchDebuff) > S.ShiftingPower:CastTime() + S.Scorch:CastTime() and Player:BuffDown(S.FuryoftheSunKingBuff)) and Player:BuffDown(S.HotStreakBuff) and var_shifting_power_before_combustion) then
      if Cast(S.ShiftingPower, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(18)) then addonTable.cast("Shifting Power") return "shifting_power main 26"; end
    end
    ...
    if S.IceNova:IsCastable() and (not SearingTouchActive()) then
      if Cast(S.IceNova, nil, nil, not Target:IsSpellInRange(S.IceNova)) then addonTable.cast("Ice Nova") return "ice_nova main 32"; end
    end
    -- scorch
    if S.Scorch:IsReady() then
      if Cast(S.Scorch, nil, nil, not Target:IsSpellInRange(S.Scorch)) then addonTable.cast("Scorch") return "scorch main 34"; end
    end
  end
end
