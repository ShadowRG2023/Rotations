...
addonTable.newMacro("OptiAction6", "INV_MISC_QUESTIONMARK", "#showtooltip\n/cast Death Grip")
...
addonTable.spells = {
  ...
  { spell = "SPELL Remorseless Winter", name = "Remorseless Winter" },
  { spell = "SPELL Obliterate", name = "Obliterate" },
  { spell = "SPELL Howling Blast", name = "Howling Blast" },
  { spell = "SPELL Horn of Winter", name = "Horn of Winter" },
  { spell = "SPELL Glacial Advance", name = "Glacial Advance" },
  { spell = "SPELL Frostwyrm's Fury", name = "Frostwyrm's Fury" },
  { spell = "SPELL Frostscythe", name = "Frostscythe" },
  { spell = "SPELL Frost Strike", name = "Frost Strike" },
  { spell = "SPELL Death Strike", name = "Death Strike" },
  { spell = "SPELL Chill Streak", name = "Chill Streak" },
  { spell = "SPELL Chains of Ice", name = "Chains of Ice" },
  -- INTERRUPTS
  { spell = "SPELL Mind Freeze", name = "Mind Freeze" },
  { spell = "SPELL Asphyxiate", name = "Asphyxiate" },
  { spell = "MACRO OptiAction6", name = "DeathGrip" },
   -- RACIALS
  ...
  --{ spell = "SPELL Pillar of Frost", name = "Pillar of Frost" },
  --{ spell = "SPELL Breath of Sindragosa", name = "Breath of Sindragosa" },
  --{ spell = "SPELL Anti-Magic Zone", name = "Anti-Magic Zone" },
  --  END SPELLS LIST
}
...
local function Precombat()
  ...
  if S.HowlingBlast:IsReady() and not Target:IsInRange(8) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast precombat 2"; end
  end
  if S.RemorselessWinter:IsReady() and Target:IsInRange(8) then
    if Cast(S.RemorselessWinter) then addonTable.cast("Remorseless Winter") return "remorseless_winter precombat 4"; end
  end
end
...
local function AoE()
  ...
  if S.HowlingBlast:IsReady() and (Player:BuffUp(S.RimeBuff) or Target:DebuffDown(S.FrostFeverDebuff)) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast aoe 2"; end
  end
  ...
  if S.GlacialAdvance:IsReady() and (not VarPoolingRP and VarRPBuffs) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance aoe 4"; end
  end
  ...
  if S.Frostscythe:IsReady() and (Player:BuffDown(S.DeathAndDecayBuff) and I.Fyralath:IsEquipped() and (I.Fyralath:CooldownRemains() < 3 or S.MarkofFyralathDebuff:AuraActiveCount() == 0)) then
    if Cast(S.Frostscythe, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Frostscythe") return "frostscythe aoe 6"; end
  end
  ...
  if S.DeathAndDecay:IsReady() and Player:BuffDown(S.DeathAndDecayBuff) then
    if Cast(S.DeathAndDecay, Settings.CommonsOGCD.GCDasOffGCD.DeathAndDecay, nil, Target:IsInRange(8)) then addonTable.cast("Death And Decay") return "death_and_decay obliteration 34"; end
  end
  ...
  if S.Obliterate:IsReady() and (Player:BuffUp(S.KillingMachineBuff) and S.CleavingStrikes:IsAvailable() and Player:BuffUp(S.DeathAndDecayBuff) and not VarFrostscythePriority) then
    if Cast(S.Obliterate, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate aoe 10"; end
  end
  -- glacial_advance,if=!variable.pooling_runic_power
  if S.GlacialAdvance:IsReady() and (not VarPoolingRP) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance aoe 12"; end
  end
  -- frostscythe,if=variable.frostscythe_priority
  if S.Frostscythe:IsReady() and (VarFrostscythePriority) then
    if Cast(S.Frostscythe, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Frostscythe") return "frostscythe aoe 14"; end
  end
  -- obliterate,if=!variable.frostscythe_priority
  if S.Obliterate:IsReady() and (not VarFrostscythePriority) then
    if Cast(S.Obliterate, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate aoe 16"; end
  end
  ...
  if S.FrostStrike:IsReady() and (not VarPoolingRP and not S.GlacialAdvance:IsAvailable()) then
    if Cast(S.FrostStrike, Settings.Frost.GCDasOffGCD.FrostStrike, nil, not Target:IsSpellInRange(S.FrostStrike)) then addonTable.cast("Frost Strike") return "frost_strike aoe 18"; end
  end
  ...
  if S.HornofWinter:IsCastable() and (Player:Rune() < 2 and Player:RunicPowerDeficit() > 25) then
    if Cast(S.HornofWinter, Settings.Frost.GCDasOffGCD.HornOfWinter) then addonTable.cast("Horn of Winter") return "horn_of_winter aoe 20"; end
  end
  -- arcane_torrent,if=runic_power.deficit>25
  if CDsON() and S.ArcaneTorrent:IsReady() and (Player:RunicPowerDeficit() > 25) then
    if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Arcane Torrent") return "arcane_torrent aoe 22"; end
  end
end
...
local function Breath()
  ...
  if S.HowlingBlast:IsReady() and (VarRimeBuffs and Player:RunicPower() > (45 - (num(S.RageoftheFrozenChampion:IsAvailable()) * 8) + (5 * num(Player:BuffUp(S.RuneofHysteriaBuff))))) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast breath 2"; end
  end
  ...
  if S.HornofWinter:IsReady() and (Player:Rune() < 2 and Player:RunicPowerDeficit() > 25 + (5 * num(Player:BuffUp(S.RuneofHysteriaBuff)))) then
    if Cast(S.HornofWinter, Settings.Frost.GCDasOffGCD.HornOfWinter) then addonTable.cast("Horn of Winter") return "horn_of_winter breath 4"; end
  end
  ...
  if S.Obliterate:IsReady() and (Player:BuffUp(S.KillingMachineBuff) and not VarFrostscythePriority) then
    if Everyone.CastTargetIf(S.Obliterate, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate breath 6"; end
  end
  ...
  if S.Frostscythe:IsReady() and (VarFrostscythePriority and (Player:BuffUp(S.KillingMachineBuff) or Player:RunicPower() > 45)) then
    if Cast(S.Frostscythe, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Frostscythe") return "frostscythe breath 8"; end
  end
  ...
  if S.Obliterate:IsReady() and (Player:RunicPowerDeficit() > 40 or Player:BuffUp(S.PillarofFrostBuff)) then
    if Everyone.CastTargetIf(S.Obliterate, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate breath 10"; end
  end
  ...
  if S.RemorselessWinter:IsReady() and (Player:RunicPower() < 36 and Player:RuneTimeToX(2) > Player:RunicPower() / 18) then
    if Cast(S.RemorselessWinter, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Remorseless Winter") return "remorseless_winter breath 12"; end
  end
  -- death_and_decay,if=variable.st_planning&talent.unholy_ground&!death_and_decay.ticking&runic_power.deficit>=10|runic_power<36&rune.time_to_2>runic_power%18
  if S.DeathAndDecay:IsReady() and (VarSTPlanning and Player:BuffDown(S.DeathAndDecayBuff) and Player:RunicPowerDeficit() >= 10 or Player:RunicPower() < 36 and Player:RuneTimeToX(2) > Player:RunicPower() / 18) then
    if Cast(S.DeathAndDecay, Settings.CommonsOGCD.GCDasOffGCD.DeathAndDecay, nil, not Target:IsSpellInRange(S.DeathAndDecay)) then addonTable.cast("Death And Decay") return "death_and_decay breath 14"; end
  end
  ...
  if S.HowlingBlast:IsReady() and (Player:RunicPower() < 36 and Player:RuneTimeToX(2) > Player:RunicPower() / 18) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast breath 16"; end
  end
  ...
  if S.Obliterate:IsReady() and (Player:RunicPowerDeficit() > 25) then
    if Everyone.CastTargetIf(S.Obliterate, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate breath 18"; end
  end
  -- howling_blast,if=buff.rime.react
  if S.HowlingBlast:IsReady() and (Player:BuffUp(S.RimeBuff)) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast breath 20"; end
  end
  -- arcane_torrent,if=runic_power<60
  if S.ArcaneTorrent:IsReady() and (Player:RunicPower() < 60) then
    if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Arcane Torrent") return "arcane_torrent breath 22"; end
  end
end
...
local function BreathOblit()
  ...
  if S.Frostscythe:IsReady() and (Player:BuffUp(S.KillingMachineBuff) and VarFrostscythePriority) then
    if Cast(S.Frostscythe, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Frostscythe") return "frostscythe breath_oblit 2"; end
  end
  ...
  if S.Obliterate:IsReady() and (Player:BuffUp(S.KillingMachineBuff)) then
    if Everyone.CastTargetIf(S.Obliterate, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate breath_oblit 4"; end
  end
  -- howling_blast,if=buff.rime.react
  if S.HowlingBlast:IsReady() and (Player:BuffUp(S.RimeBuff)) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast breath_oblit 6"; end
  end
  -- howling_blast,if=!buff.killing_machine.up
  if S.HowlingBlast:IsReady() and (Player:BuffDown(S.KillingMachineBuff)) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast breath_oblit 8"; end
  end
  -- horn_of_winter,if=runic_power.deficit>25
  if S.HornofWinter:IsReady() and (Player:RunicPowerDeficit() > 25) then
    if Cast(S.HornofWinter, Settings.Frost.GCDasOffGCD.HornOfWinter) then addonTable.cast("Horn of Winter") return "horn_of_winter breath_oblit 10"; end
  end
  -- arcane_torrent,if=runic_power.deficit>20
  if S.ArcaneTorrent:IsReady() and (Player:RunicPowerDeficit() > 20) then
    if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Arcane Torrent") return "arcane_torrent breath_oblit 12"; end
  end
end
...
local function ColdHeart()
  ...
  if S.ChainsofIce:IsReady() and (FightRemains < Player:GCD() and (Player:Rune() < 2 or Player:BuffDown(S.KillingMachineBuff) and (not Var2HCheck and Player:BuffStack(S.ColdHeartBuff) >= 4 or Var2HCheck and Player:BuffStack(S.ColdHeartBuff) > 8) or Player:BuffUp(S.KillingMachineBuff) and (not Var2HCheck and Player:BuffStack(S.ColdHeartBuff) > 8 or Var2HCheck and Player:BuffStack(S.ColdHeartBuff) > 10))) then
    if Cast(S.ChainsofIce, Settings.CommonsOGCD.GCDasOffGCD.ChainsOfIce, nil, not Target:IsSpellInRange(S.ChainsofIce)) then addonTable.cast("Chains of Ice") return "chains_of_ice cold_heart 2"; end
  end
  ...
  if S.ChainsofIce:IsReady() and (not S.Obliteration:IsAvailable() and Player:BuffUp(S.PillarofFrostBuff) and Player:BuffStack(S.ColdHeartBuff) >= 10 and (Player:BuffRemains(S.PillarofFrostBuff) < Player:GCD() * (1 + num(S.FrostwyrmsFury:IsAvailable() and S.FrostwyrmsFury:IsReady())) or Player:BuffUp(S.UnholyStrengthBuff) and Player:BuffRemains(S.UnholyStrengthBuff) < Player:GCD())) then
    if Cast(S.ChainsofIce, Settings.CommonsOGCD.GCDasOffGCD.ChainsOfIce, nil, not Target:IsSpellInRange(S.ChainsofIce)) then addonTable.cast("Chains of Ice") return "chains_of_ice cold_heart 4"; end
  end
  ...
  if S.ChainsofIce:IsReady() and (not S.Obliteration:IsAvailable() and UsingFallenCrusader and Player:BuffDown(S.PillarofFrostBuff) and S.PillarofFrost:CooldownRemains() > 15 and (Player:BuffStack(S.ColdHeartBuff) >= 10 and Player:BuffUp(S.UnholyStrengthBuff) or Player:BuffStack(S.ColdHeartBuff) >= 13)) then
    if Cast(S.ChainsofIce, Settings.CommonsOGCD.GCDasOffGCD.ChainsOfIce, nil, not Target:IsSpellInRange(S.ChainsofIce)) then addonTable.cast("Chains of Ice") return "chains_of_ice cold_heart 6"; end
  end
  ...
  if S.ChainsofIce:IsReady() and (not S.Obliteration:IsAvailable() and not UsingFallenCrusader and Player:BuffStack(S.ColdHeartBuff) >= 10 and Player:BuffDown(S.PillarofFrostBuff) and S.PillarofFrost:CooldownRemains() > 20) then
    if Cast(S.ChainsofIce, Settings.CommonsOGCD.GCDasOffGCD.ChainsOfIce, nil, not Target:IsSpellInRange(S.ChainsofIce)) then addonTable.cast("Chains of Ice") return "chains_of_ice cold_heart 8"; end
  end
  ...
  if S.ChainsofIce:IsReady() and (S.Obliteration:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff) and (Player:BuffStack(S.ColdHeartBuff) >= 14 and (Player:BuffUp(S.UnholyStrengthBuff)) or Player:BuffStack(S.ColdHeartBuff) >= 19 or S.PillarofFrost:CooldownRemains() < 3 and Player:BuffStack(S.ColdHeartBuff) >= 14)) then
    if Cast(S.ChainsofIce, Settings.CommonsOGCD.GCDasOffGCD.ChainsOfIce, nil, not Target:IsSpellInRange(S.ChainsofIce)) then addonTable.cast("Chains of Ice") return "chains_of_ice cold_heart 10"; end
  end
end
...
local function Cooldowns()
  ...
  if S.AbominationLimb:IsCastable() and (S.Obliteration:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff) and S.PillarofFrost:CooldownRemains() < 3 and (VarAddsRemain or VarSTPlanning) or FightRemains < 15) then
    if Cast(S.AbominationLimb, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(20)) then addonTable.cast("Abomination Limb") return "abomination_limb_talent cooldowns 4"; end
  end
  ...
  if S.AbominationLimb:IsCastable() and (S.BreathofSindragosa:IsAvailable() and (VarAddsRemain or VarSTPlanning)) then
    if Cast(S.AbominationLimb, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(20)) then addonTable.cast("Abomination Limb") return "abomination_limb_talent cooldowns 6"; end
  end
  ...
  if S.AbominationLimb:IsCastable() and (not S.BreathofSindragosa:IsAvailable() and not S.Obliteration:IsAvailable() and (VarAddsRemain or VarSTPlanning)) then
    if Cast(S.AbominationLimb, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(20)) then addonTable.cast("Abomination Limb") return "abomination_limb_talent cooldowns 8"; end
  end
  ...
  if S.EmpowerRuneWeapon:IsCastable() and (S.Obliteration:IsAvailable() and Player:BuffDown(S.EmpowerRuneWeaponBuff) and Player:Rune() < 6 and ((S.PillarofFrost:CooldownRemains() < 7 and Player:BloodlustUp()) or ((EnemiesMeleeCount >= 2 or VarSTPlanning) and S.PillarofFrost:CooldownUp())) or FightRemains < 20) then
    if Cast(S.EmpowerRuneWeapon, Settings.CommonsOGCD.GCDasOffGCD.EmpowerRuneWeapon) then addonTable.cast("Empower Rune Weapon") return "empower_rune_weapon cooldowns 10"; end
  end
  ...
  if S.EmpowerRuneWeapon:IsCastable() and (Player:BuffUp(S.BreathofSindragosa) and Player:BuffDown(S.EmpowerRuneWeaponBuff) and ((HL.CombatTime() < 10 and Player:BloodlustUp()) or (Player:RunicPower() < 70 and Player:Rune() < 3 and (S.BreathofSindragosa:CooldownRemains() > VarERWPoolingTime or S.EmpowerRuneWeapon:FullRechargeTime() < 10)))) then
    if Cast(S.EmpowerRuneWeapon, Settings.CommonsOGCD.GCDasOffGCD.EmpowerRuneWeapon) then addonTable.cast("Empower Rune Weapon") return "empower_rune_weapon cooldowns 12"; end
  end
  ...
  if S.EmpowerRuneWeapon:IsCastable() and (not S.BreathofSindragosa:IsAvailable() and not S.Obliteration:IsAvailable() and Player:BuffDown(S.EmpowerRuneWeaponBuff) and Player:Rune() < 5 and (S.PillarofFrostBuff:CooldownRemains() < 7 or Player:BuffUp(S.PillarofFrostBuff) or not S.PillarofFrost:IsAvailable())) then
    if Cast(S.EmpowerRuneWeapon, Settings.CommonsOGCD.GCDasOffGCD.EmpowerRuneWeapon) then addonTable.cast("Empower Rune Weapon") return "empower_rune_weapon cooldowns 14"; end
  end
  -- empower_rune_weapon,use_off_gcd=1,if=!talent.obliteration&!buff.empower_rune_weapon.up&rune<5&(cooldown.pillar_of_frost.remains_expected<7|buff.pillar_of_frost.up|!talent.pillar_of_frost)
  if S.EmpowerRuneWeapon:IsCastable() and (not S.BreathofSindragosa:IsAvailable() and not S.Obliteration:IsAvailable() and Player:BuffDown(S.EmpowerRuneWeaponBuff) and Player:Rune() < 5)then
    if Cast(S.EmpowerRuneWeapon, Settings.CommonsOGCD.GCDasOffGCD.EmpowerRuneWeapon) then addonTable.cast("Empower Rune Weapon") return "empower_rune_weapon cooldowns 16"; end
  end
  -- abomination_limb,if=talent.obliteration&!buff.pillar_of_frost.up&cooldown.pillar_of_frost.remains<3&(variable.adds_remain|variable.st_planning)|fight_remains<15
  if S.AbominationLimb:IsCastable() and (S.Obliteration:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff) and S.PillarofFrost:CooldownRemains() < 3 and (VarAddsRemain or VarSTPlanning) or FightRemains < 15) then
    if Cast(S.AbominationLimb, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(20)) then addonTable.cast("Abomination Limb") return "abomination_limb_talent cooldowns 18"; end
  end
  -- abomination_limb,if=talent.breath_of_sindragosa&(variable.adds_remain|variable.st_planning)
  if S.AbominationLimb:IsCastable() and (S.BreathofSindragosa:IsAvailable() and (VarAddsRemain or VarSTPlanning)) then
    if Cast(S.AbominationLimb, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(20)) then addonTable.cast("Abomination Limb") return "abomination_limb_talent cooldowns 20"; end
  end
  -- abomination_limb,if=!talent.breath_of_sindragosa&!talent.obliteration&(variable.adds_remain|variable.st_planning)
  if S.AbominationLimb:IsCastable() and (not S.BreathofSindragosa:IsAvailable() and not S.Obliteration:IsAvailable() and (VarAddsRemain or VarSTPlanning)) then
    if Cast(S.AbominationLimb, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(20)) then addonTable.cast("Abomination Limb") return "abomination_limb_talent cooldowns 22"; end
  end
  ...
  if S.ChillStreak:IsReady() and (Player:HasTier(31, 2) and Player:BuffRemains(S.ChillingRageBuff) < 3) then
    if Cast(S.ChillStreak, Settings.Frost.GCDasOffGCD.ChillStreak, nil, not Target:IsSpellInRange(S.ChillStreak)) then addonTable.cast("Chill Streak") return "chill_streak cooldowns 24"; end
  end
  ...
  if S.ChillStreak:IsReady() and (not Player:HasTier(31, 2) and EnemiesMeleeCount >= 2 and (Player:BuffDown(S.DeathAndDecayBuff) and S.CleavingStrikes:IsAvailable() or not S.CleavingStrikes:IsAvailable() or EnemiesMeleeCount <= 5)) then
    if Cast(S.ChillStreak, Settings.Frost.GCDasOffGCD.ChillStreak, nil, not Target:IsSpellInRange(S.ChillStreak)) then addonTable.cast("Chill Streak") return "chill_streak cooldowns 26"; end
  end
  ...
  if S.PillarofFrost:IsCastable() and (S.Obliteration:IsAvailable() and (VarAddsRemain or VarSTPlanning) and (Player:BuffUp(S.EmpowerRuneWeaponBuff) or S.EmpowerRuneWeapon:CooldownRemains() > 0) or FightRemains < 12) then
    if Cast(S.PillarofFrost, Settings.Frost.GCDasOffGCD.PillarOfFrost) then addonTable.cast("Pillar of Frost") return "pillar_of_frost cooldowns 28"; end
  end
  ...
  if S.PillarofFrost:IsCastable() and (S.BreathofSindragosa:IsAvailable() and (VarAddsRemain or VarSTPlanning) and (not S.Icecap:IsAvailable() and (Player:RunicPower() > 70 or S.BreathofSindragosa:CooldownRemains() > 40) or S.Icecap:IsAvailable() and (S.BreathofSindragosa:CooldownRemains() > 5)) or Player:BuffUp(S.BreathofSindragosa)) then
    if Cast(S.PillarofFrost, Settings.Frost.GCDasOffGCD.PillarOfFrost) then addonTable.cast("Pillar of Frost") return "pillar_of_frost cooldowns 30"; end
  end
  ...
  if S.PillarofFrost:IsCastable() and (S.Icecap:IsAvailable() and not S.Obliteration:IsAvailable() and not S.BreathofSindragosa:IsAvailable() and (VarAddsRemain or VarSTPlanning)) then
    if Cast(S.PillarofFrost, Settings.Frost.GCDasOffGCD.PillarOfFrost) then addonTable.cast("Pillar of Frost") return "pillar_of_frost cooldowns 32"; end
  end
  ...
  if S.BreathofSindragosa:IsReady() and (Player:BuffDown(S.BreathofSindragosa) and (Player:RunicPower() > 50 and S.EmpowerRuneWeapon:CooldownUp() or Player:RunicPower() > 60 and S.EmpowerRuneWeapon:CooldownRemains() < 30 or Player:RunicPower() > 80 and S.EmpowerRuneWeapon:CooldownRemains() > 30) and (VarAddsRemain or VarSTPlanning or FightRemains < 30)) then
    if Cast(S.BreathofSindragosa, Settings.Frost.GCDasOffGCD.BreathOfSindragosa, nil, not Target:IsInRange(12)) then addonTable.cast("Breath of Sindragosa") return "breath_of_sindragosa cooldowns 34"; end
  end
  ...
  if S.FrostwyrmsFury:IsCastable() and (EnemiesMeleeCount == 1 and (S.PillarofFrost:IsAvailable() and Player:BuffRemains(S.PillarofFrostBuff) < Player:GCD() * 2 and Player:BuffUp(S.PillarofFrostBuff) and not S.Obliteration:IsAvailable() or not S.PillarofFrost:IsAvailable()) or FightRemains < 3) then
    if Cast(S.FrostwyrmsFury, Settings.Frost.GCDasOffGCD.FrostwyrmsFury, nil, not Target:IsInRange(40)) then addonTable.cast("Frostwyrm's Fury") return "frostwyrms_fury cooldowns 36"; end
  end
  ...
  if S.FrostwyrmsFury:IsCastable() and (EnemiesMeleeCount >= 2 and (S.PillarofFrost:IsAvailable() and Player:BuffUp(S.PillarofFrostBuff)) and (Player:BuffRemains(S.PillarofFrostBuff) < Player:GCD() * 2)) then
    if Cast(S.FrostwyrmsFury, Settings.Frost.GCDasOffGCD.FrostwyrmsFury, nil, not Target:IsInRange(40)) then addonTable.cast("Frostwyrm's Fury") return "frostwyrms_fury cooldowns 38"; end
  end
  ...
  if S.FrostwyrmsFury:IsCastable() and (S.Obliteration:IsAvailable() and (S.PillarofFrost:IsAvailable() and Player:BuffUp(S.PillarofFrostBuff) and not Using2H or Player:BuffDown(S.PillarofFrostBuff) and Using2H and S.PillarofFrost:CooldownDown() or not S.PillarofFrost:IsAvailable()) and ((Player:BuffRemains(S.PillarofFrostBuff) < Player:GCD() or Player:BuffUp(S.UnholyStrengthBuff) and Player:BuffRemains(S.UnholyStrengthBuff) < Player:GCD()) and (Target:DebuffStack(S.RazoriceDebuff) == 5 or not UsingRazorice and not S.GlacialAdvance:IsAvailable()))) then
    if Cast(S.FrostwyrmsFury, Settings.Frost.GCDasOffGCD.FrostwyrmsFury, nil, not Target:IsInRange(40)) then addonTable.cast("Frostwyrm's Fury") return "frostwyrms_fury cooldowns 40"; end
  end
  -- raise_dead
  if S.RaiseDead:IsCastable() then
    if Cast(S.RaiseDead, nil, Settings.CommonsDS.DisplayStyle.RaiseDead) then addonTable.cast("Raise Dead") return "raise_dead cooldowns 42"; end
  end
  ...
  if S.SoulReaper:IsReady() and (FightRemains > 5 and (Target:TimeToX(35) < 5 or Target:HealthPercentage() <= 35) and EnemiesMeleeCount <= 2 and (S.Obliteration:IsAvailable() and (Player:BuffUp(S.PillarofFrostBuff) and Player:BuffDown(S.KillingMachineBuff) and Player:Rune() > 2 or Player:BuffDown(S.PillarofFrostBuff)) or S.BreathofSindragosa:IsAvailable() and (Player:BuffUp(S.BreathofSindragosa) and Player:RunicPower() > 50 or Player:BuffDown(S.BreathofSindragosa)) or not S.BreathofSindragosa:IsAvailable() and not S.Obliteration:IsAvailable())) then
    if Cast(S.SoulReaper, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Soul Reaper") return "soul_reaper cooldowns 48"; end
  end
end
...
local function HighPrioActions()
  ...
  if Settings.Commons.UseAMSAMZOffensively and CDsON() then
    ...
    if S.AntiMagicShell:IsCastable() and (Player:RunicPowerDeficit() > 40 and 20 < HL.CombatTime()) then
      if Cast(S.AntiMagicShell, Settings.CommonsOGCD.GCDasOffGCD.AntiMagicShell) then addonTable.cast("Anti-Magic Shell") return "antimagic_shell high_prio_actions 2"; end
    end
    ...
    if S.AntiMagicZone:IsCastable() and (Player:RunicPowerDeficit() > 70 and S.Assimilation:IsAvailable() and (Player:BuffUp(S.BreathofSindragosa) or S.BreathofSindragosa:CooldownUp() or not S.BreathofSindragosa:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff))) then
      if Cast(S.AntiMagicZone, Settings.CommonsOGCD.GCDasOffGCD.AntiMagicZone) then addonTable.cast("Anti-Magic Zone") return "antimagic_zone high_prio_actions 4"; end
    end
  ...
  if S.HowlingBlast:IsReady() and (Target:DebuffDown(S.FrostFeverDebuff) and EnemiesMeleeCount >= 2 and ((not S.Obliteration:IsAvailable() or S.Obliteration:IsAvailable() and (S.PillarofFrost:CooldownDown() or Player:BuffUp(S.PillarofFrostBuff) and Player:BuffDown(S.KillingMachineBuff))) or (I.Fyralath:IsEquipped() and S.MarkofFyralathDebuff:AuraActiveCount() == 0))) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast high_prio_actions 6"; end
  end
  ...
  if S.GlacialAdvance:IsReady() and (EnemiesMeleeCount >= 2 and VarRPBuffs and S.Obliteration:IsAvailable() and S.BreathofSindragosa:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff) and Player:BuffDown(S.BreathofSindragosa) and S.BreathofSindragosa:CooldownRemains() > VarBreathPoolingTime) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance high_prio_actions 8"; end
  end
  ...
  if S.GlacialAdvance:IsReady() and (EnemiesMeleeCount >= 2 and VarRPBuffs and S.BreathofSindragosa:IsAvailable() and Player:BuffDown(S.BreathofSindragosa) and S.BreathofSindragosa:CooldownRemains() > VarBreathPoolingTime) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance high_prio_actions 10"; end
  end
  ...
  if S.GlacialAdvance:IsReady() and (EnemiesMeleeCount >= 2 and VarRPBuffs and not S.BreathofSindragosa:IsAvailable() and S.Obliteration:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff)) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance high_prio_actions 12"; end
  end
  ...
  if S.FrostStrike:IsReady() and (EnemiesMeleeCount == 1 and VarRPBuffs and S.Obliteration:IsAvailable() and S.BreathofSindragosa:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff) and Player:BuffDown(S.BreathofSindragosa) and S.BreathofSindragosa:CooldownRemains() > VarBreathPoolingTime) then
    if Cast(S.FrostStrike, Settings.Frost.GCDasOffGCD.FrostStrike, nil, not Target:IsSpellInRange(S.FrostStrike)) then addonTable.cast("Frost Strike") return "frost_strike high_prio_actions 14"; end
  end
  ...
  if S.FrostStrike:IsReady() and (EnemiesMeleeCount == 1 and VarRPBuffs and S.BreathofSindragosa:IsAvailable() and Player:BuffDown(S.BreathofSindragosa) and S.BreathofSindragosa:CooldownRemains() > VarBreathPoolingTime) then
    if Cast(S.FrostStrike, Settings.Frost.GCDasOffGCD.FrostStrike, nil, not Target:IsSpellInRange(S.FrostStrike)) then addonTable.cast("Frost Strike") return "frost_strike high_prio_actions 16"; end
  end
  ...
  if S.FrostStrike:IsReady() and (EnemiesMeleeCount == 1 and VarRPBuffs and not S.BreathofSindragosa:IsAvailable() and S.Obliteration:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff)) then
    if Cast(S.FrostStrike, Settings.Frost.GCDasOffGCD.FrostStrike, nil, not Target:IsSpellInRange(S.FrostStrike)) then addonTable.cast("Frost Strike") return "frost_strike high_prio_actions 18"; end
  end
  ...
  if S.RemorselessWinter:IsReady() and (VarRWBuffs or VarAddsRemain) then
    if Cast(S.RemorselessWinter, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Remorseless Winter") return "remorseless_winter high_prio_actions 22"; end
  end
end
...
local function Obliteration()
  ...
  if S.HowlingBlast:IsReady() and (Player:BuffStack(S.KillingMachineBuff) < 2 and Player:BuffRemains(S.PillarofFrostBuff) < Player:GCD() and Player:BuffUp(S.RimeBuff)) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast obliteration 2"; end
  end
  ...
  if S.FrostStrike:IsReady() and ((EnemiesMeleeCount <= 1 or not S.GlacialAdvance:IsAvailable()) and Player:BuffStack(S.KillingMachineBuff) < 2 and Player:BuffRemains(S.PillarofFrostBuff) < Player:GCD() and Player:BuffDown(S.DeathAndDecayBuff)) then
    if Cast(S.FrostStrike, Settings.Frost.GCDasOffGCD.FrostStrike, nil, not Target:IsSpellInRange(S.FrostStrike)) then addonTable.cast("Frost Strike") return "frost_strike obliteration 4"; end
  end
  ...
  if S.GlacialAdvance:IsReady() and (Player:BuffStack(S.KillingMachineBuff) < 2 and Player:BuffRemains(S.PillarofFrostBuff) < Player:GCD() and Player:BuffDown(S.DeathAndDecayBuff)) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance obliteration 6"; end
  end
  ...
  if S.Frostscythe:IsReady() and (Player:BuffUp(S.KillingMachineBuff) and (VarFrostscythePriority or EnemiesMeleeCount > 3 and Player:BuffDown(S.DeathAndDecayBuff) and I.Fyralath:IsEquipped() and (I.Fyralath:CooldownRemains() < 3 or S.MarkofFyralathDebuff:AuraActiveCount() == 0))) then
    if Cast(S.Frostscythe, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Frostscythe") return "frostscythe obliteration 8"; end
  end
  -- obliterate,target_if=max:(debuff.razorice.stack+1)%(debuff.razorice.remains+1)*death_knight.runeforge.razorice,if=buff.killing_machine.react&!variable.frostscythe_priority
  if S.Obliterate:IsReady() and (Player:BuffUp(S.KillingMachineBuff) and not VarFrostscythePriority) then
    if Everyone.CastTargetIf(S.Obliterate, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate obliteration 10"; end
  end
  ...
  if S.HowlingBlast:IsReady() and (Player:BuffDown(S.KillingMachineBuff) and (Target:DebuffDown(S.FrostFeverDebuff) or Player:BuffUp(S.RimeBuff) and Player:HasTier(30, 2) and not VarRPBuffs)) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast obliteration 12"; end
  end
  ...
  if S.GlacialAdvance:IsReady() and (Player:BuffDown(S.KillingMachineBuff) and (not UsingRazorice and (not S.Avalanche:IsAvailable() or Target:DebuffStack(S.RazoriceDebuff) < 5 or Target:DebuffRemains(S.RazoriceDebuff) < Player:GCD() * 3) or ((VarRPBuffs or Player:Rune() < 2) and EnemiesMeleeCount > 1))) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance obliteration 14"; end
  end
  ...
  if S.FrostStrike:IsReady() and (Player:BuffDown(S.KillingMachineBuff) and (Player:Rune() < 2 or VarRPBuffs or Target:DebuffStack(S.RazoriceDebuff) == 5 and S.ShatteringBlade:IsAvailable()) and not VarPoolingRP and (not S.GlacialAdvance:IsAvailable() or EnemiesMeleeCount == 1)) then
    if Everyone.CastTargetIf(S.FrostStrike, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5), Settings.Frost.GCDasOffGCD.FrostStrike) then addonTable.cast("Frost Strike") return "frost_strike obliteration 16"; end
  end
  ...
  if S.HowlingBlast:IsReady() and (Player:BuffUp(S.RimeBuff) and Player:BuffDown(S.KillingMachineBuff)) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast obliteration 18"; end
  end
  ...
  if S.FrostStrike:IsReady() and (Player:BuffDown(S.KillingMachineBuff) and not VarPoolingRP and (not S.GlacialAdvance:IsAvailable() or EnemiesMeleeCount == 1)) then
    if Everyone.CastTargetIf(S.FrostStrike, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5), Settings.Frost.GCDasOffGCD.FrostStrike) then addonTable.cast("Frost Strike") return "frost_strike obliteration 20"; end
  end
  ...
  if S.HowlingBlast:IsReady() and (Player:BuffDown(S.KillingMachineBuff) and Player:RunicPower() < 30) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast obliteration 22"; end
  end
  ...
  if CDsON() and S.ArcaneTorrent:IsReady() and (Player:Rune() < 1 and Player:RunicPower() < 30) then
    if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Arcane Torrent") return "arcane_torrent obliteration 24"; end
  end
  ...
  if S.GlacialAdvance:IsReady() and (not VarPoolingRP and EnemiesMeleeCount >= 2) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance obliteration 26"; end
  end
  ...
  if S.FrostStrike:IsReady() and (not VarPoolingRP and (not S.GlacialAdvance:IsAvailable() or EnemiesMeleeCount == 1)) then
    if Everyone.CastTargetIf(S.FrostStrike, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5), Settings.Frost.GCDasOffGCD.FrostStrike) then addonTable.cast("Frost Strike") return "frost_strike obliteration 28"; end
  end
  -- howling_blast,if=buff.rime.react
  if S.HowlingBlast:IsReady() and (Player:BuffUp(S.RimeBuff)) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast obliteration 30"; end
  end
  ...
  if S.Obliterate:IsReady() then
    if Everyone.CastTargetIf(S.Obliterate, EnemiesMelee, "max", EvaluateTargetIfRazoriceStacks, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate obliteration 32"; end
  end
end
...
local function Racials()
  ...
  if S.BagofTricks:IsCastable() and (S.Obliteration:IsAvailable() and Player:BuffDown(S.PillarofFrostBuff) and Player:BuffUp(S.UnholyStrengthBuff)) then
    if Cast(S.BagofTricks, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsInRange(40)) then addonTable.cast("Bag of Tricks") return "bag_of_tricks racials 14"; end
  end
  ...
  if S.BagofTricks:IsCastable() and (not S.Obliteration:IsAvailable() and Player:BuffUp(S.PillarofFrostBuff) and (Player:BuffUp(S.UnholyStrengthBuff) and Player:BuffRemains(S.UnholyStrengthBuff) < Player:GCD() * 3 or Player:BuffRemains(S.PillarofFrostBuff) < Player:GCD() * 3)) then
    if Cast(S.BagofTricks, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsInRange(40)) then addonTable.cast("Bag of Tricks") return "bag_of_tricks racials 16"; end
  end
end
...
local function SingleTarget()
  ...
  --  if Cast(S.DeathAndDecay, Settings.CommonsOGCD.GCDasOffGCD.DeathAndDecay) then addonTable.cast("Death And Decay") return "death_and_decay single_target 28"; end
  --end
  ...
  if S.FrostStrike:IsReady() and (Player:BuffStack(S.KillingMachineBuff) < 2 and Player:RunicPowerDeficit() < 20 + (4 * num(Player:BuffUp(S.RuneofHysteriaBuff))) and not Var2HCheck) then
    if Cast(S.FrostStrike, Settings.Frost.GCDasOffGCD.FrostStrike, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Frost Strike") return "frost_strike single_target 2"; end
  end
  ...
  if S.HowlingBlast:IsReady() and (Player:BuffUp(S.RimeBuff) and Player:HasTier(30, 2) and Player:BuffStack(S.KillingMachineBuff) < 2) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast single_target 4"; end
  end
  -- frostscythe,if=buff.killing_machine.react&variable.frostscythe_priority
  if S.Frostscythe:IsReady() and (Player:BuffUp(S.KillingMachineBuff) and VarFrostscythePriority) then
    if Cast(S.Frostscythe, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Frostscythe") return "frostscythe single_target 6"; end
  end
  -- obliterate,if=buff.killing_machine.react
  if S.Obliterate:IsReady() and (Player:BuffUp(S.KillingMachineBuff)) then
    if Cast(S.Obliterate, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate single_target 8"; end
  end
  ...
  if S.HowlingBlast:IsReady() and (Player:BuffUp(S.RimeBuff) and S.Icebreaker:TalentRank() == 2) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast single_target 10"; end
  end
  ...
  if S.HornofWinter:IsReady() and (Player:Rune() < 4 and Player:RunicPowerDeficit() > 25 + (5 * num(Player:BuffUp(S.RuneofHysteriaBuff))) and S.Obliteration:IsAvailable() and S.BreathofSindragosa:IsAvailable()) then
    if Cast(S.HornofWinter, Settings.Frost.GCDasOffGCD.HornOfWinter) then addonTable.cast("Horn of Winter") return "horn_of_winter single_target 12"; end
  end
  ...
  if S.FrostStrike:IsReady() and (not VarPoolingRP and (VarRPBuffs or Player:RunicPowerDeficit() < 25 + (5 * num(Player:BuffUp(S.RuneofHysteriaBuff))) or Target:DebuffStack(S.RazoriceDebuff) == 5 and S.ShatteringBlade:IsAvailable())) then
    if Cast(S.FrostStrike, Settings.Frost.GCDasOffGCD.FrostStrike, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Frost Strike") return "frost_strike single_target 14"; end
  end
  -- howling_blast,if=variable.rime_buffs
  if S.HowlingBlast:IsReady() and (VarRimeBuffs) then
    if Cast(S.HowlingBlast, nil, nil, not Target:IsSpellInRange(S.HowlingBlast)) then addonTable.cast("Howling Blast") return "howling_blast single_target 16"; end
  end
  ...
  if S.GlacialAdvance:IsReady() and (not VarPoolingRP and not UsingRazorice and (Target:DebuffStack(S.RazoriceDebuff) < 5 or Target:DebuffRemains(S.RazoriceDebuff) < Player:GCD() * 3)) then
    if Cast(S.GlacialAdvance, nil, nil, not Target:IsInRange(100)) then addonTable.cast("Glacial Advance") return "glacial_advance single_target 18"; end
  end
  -- obliterate,if=!variable.pooling_runes
  if S.Obliterate:IsReady() and (not VarPoolingRunes) then
    if Cast(S.Obliterate, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Obliterate") return "obliterate single_target 20"; end
  end
  ...
  if S.HornofWinter:IsReady() and (Player:Rune() < 4 and Player:RunicPowerDeficit() > 25 and (not S.BreathofSindragosa:IsAvailable() or S.BreathofSindragosa:CooldownRemains() > 45)) then
    if Cast(S.HornofWinter, Settings.Frost.GCDasOffGCD.HornOfWinter) then addonTable.cast("Horn of Winter") return "horn_of_winter single_target 22"; end
  end
  -- arcane_torrent,if=runic_power.deficit>20
  if CDsON() and S.ArcaneTorrent:IsReady() and (Player:RunicPowerDeficit() > 20) then
    if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Arcane Torrent") return "arcane_torrent single_target 24"; end
  end
  -- frost_strike,if=!variable.pooling_runic_power
  if S.FrostStrike:IsReady() and (not VarPoolingRP) then
    if Cast(S.FrostStrike, Settings.Frost.GCDasOffGCD.FrostStrike, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Frost Strike") return "frost_strike single_target 26"; end
  end
end
...
addonTable.resetPixels()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.DeathStrike:IsReady() and not no_heal then
      if Cast(S.DeathStrike, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Death Strike") return "death_strike low hp or proc"; end
    end
    -- auto_attack
