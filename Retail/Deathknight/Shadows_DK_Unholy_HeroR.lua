...
addonTable.newMacro("OptiAction7", "INV_MISC_QUESTIONMARK", "#showtooltip\n/cast Death Grip")
...
addonTable.spells = {
  ...
  { spell = "MACRO OptiAction3", name = "SummonGargoyle" },
  ...
  { spell = "SPELL Scourge Strike", name = "Scourge Strike" },
  { spell = "SPELL Sacrificial Pact", name = "Sacrificial Pact" },
  { spell = "SPELL Raise Dead", name = "Raise Dead" },
  { spell = "SPELL Outbreak", name = "Outbreak" },
  { spell = "SPELL Festering Strike", name = "Festering Strike" },
  { spell = "SPELL Epidemic", name = "Epidemic" },
  { spell = "SPELL Death Strike", name = "Death Strike" },
  { spell = "SPELL Death Coil", name = "Death Coil" },
  { spell = "SPELL Dark Transformation", name = "Dark Transformation" },
  { spell = "SPELL Apocalypse", name = "Apocalypse" },
  ...
  --{ spell = "SPELL Summon Gargoyle", name = "Summon Gargoyle" },
...
local function Precombat()
  ...
  if S.RaiseDead:IsCastable() then
    if Settings.Unholy.RaiseDeadCastLeft then
      ...
      if Cast(S.RaiseDead, nil, Settings.CommonsDS.DisplayStyle.RaiseDead) then addonTable.cast("Raise Dead") return "raise_dead precombat 2 displaystyle"; end
    end
  end
  ...
  if CDsON() then
    if S.ArmyoftheDead:IsReady() and (not I.Fyralath:IsEquipped()) then
      if Cast(S.ArmyoftheDead, nil, Settings.Unholy.DisplayStyle.ArmyOfTheDead) then addonTable.cast("Army of the Dead") return "army_of_the_dead precombat 4"; end
    end
  end
  ...
  if S.Outbreak:IsReady() then
    if Cast(S.Outbreak, nil, nil, not Target:IsSpellInRange(S.Outbreak)) then addonTable.cast("Outbreak") return "outbreak precombat 6"; end
  end
  -- Manually added: festering_strike if in melee range
  if S.FesteringStrike:IsReady() then
    if Cast(S.FesteringStrike, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Festering Strike") return "festering_strike precombat 8"; end
  end
end
...
local function AoE()
  ...
  if S.Epidemic:IsReady() and (not VarPoolingRunicPower or FightRemains < 10) then
    if Cast(S.Epidemic, Settings.Unholy.GCDasOffGCD.Epidemic, nil, not Target:IsInRange(40)) then addonTable.cast("Epidemic") return "epidemic aoe 2"; end
  end
  ...
  if WoundSpender:IsReady() and (VarPopWounds) then
    if Everyone.CastTargetIf(WoundSpender, EnemiesMelee, "max", EvaluateTargetIfFilterFWStack, nil, not Target:IsSpellInRange(WoundSpender)) then addonTable.cast("Scourge Strike") return "wound_spender aoe 4"; end
  end
  ...
  if S.FesteringStrike:IsReady() and (not VarPopWounds) then
    if Everyone.CastTargetIf(S.FesteringStrike, EnemiesMelee, "max", EvaluateTargetIfFilterFWStack, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Festering Strike") return "festering_strike aoe 6"; end
  end
  ...
  if S.DeathCoil:IsReady() and (not VarPoolingRunicPower and not S.Epidemic:IsAvailable()) then
    if Cast(S.DeathCoil, nil, nil, not Target:IsSpellInRange(S.DeathCoil)) then addonTable.cast("Death Coil") return "death_coil aoe 8"; end
  end
end
...
local function AoEBurst()
  ...
  if S.Epidemic:IsReady() and ((Player:Rune() < 1 or S.BurstingSores:IsAvailable() and S.FesteringWoundDebuff:AuraActiveCount() == 0 or not S.BurstingSores:IsAvailable()) and not VarPoolingRunicPower and (ActiveEnemies >= 6 or Player:RunicPowerDeficit() < 30 or Player:BuffStack(S.FestermightBuff) == 20)) then
    if Cast(S.Epidemic, Settings.Unholy.GCDasOffGCD.Epidemic, nil, not Target:IsInRange(40)) then addonTable.cast("Epidemic") return "epidemic aoe_burst 2"; end
  end
  -- wound_spender,target_if=max:debuff.festering_wound.stack,if=debuff.festering_wound.stack>=1
  if WoundSpender:IsReady() then
    if Everyone.CastTargetIf(WoundSpender, EnemiesMelee, "max", EvaluateTargetIfFilterFWStack, EvaluateTargetIfWoundSpenderAoEBurst, not Target:IsSpellInRange(WoundSpender)) then addonTable.cast("Scourge Strike") return "wound_spender aoe_burst 4"; end
  end
  -- epidemic,if=!variable.pooling_runic_power|fight_remains<10
  if S.Epidemic:IsReady() and (not VarPoolingRunicPower or FightRemains < 10) then
    if Cast(S.Epidemic, Settings.Unholy.GCDasOffGCD.Epidemic, nil, not Target:IsInRange(40)) then addonTable.cast("Epidemic") return "epidemic aoe_burst 6"; end
  end
  -- death_coil,if=!variable.pooling_runic_power&!talent.epidemic
  if S.DeathCoil:IsReady() and (not VarPoolingRunicPower and not S.Epidemic:IsAvailable()) then
    if Cast(S.DeathCoil, nil, nil, not Target:IsSpellInRange(S.DeathCoil)) then addonTable.cast("Death Coil") return "death_coil aoe_burst 8"; end
  end
  -- wound_spender
  if WoundSpender:IsReady() then
    if Cast(WoundSpender, nil, nil, not Target:IsSpellInRange(WoundSpender)) then addonTable.cast("Scourge Strike") return "wound_spender aoe_burst 10"; end
  end
end
...
local function AoECDs()
  ...
  if S.VileContagion:IsReady() and (AnyDnD:CooldownRemains() < 3) then
    if Everyone.CastTargetIf(S.VileContagion, EnemiesMelee, "max", EvaluateTargetIfFilterFWStack, EvaluateTargetIfVileContagionAoECDs, not Target:IsSpellInRange(S.VileContagion)) then addonTable.cast("Vile Contagion") return "vile_contagion aoe_cooldowns 2"; end
  end
  -- summon_gargoyle
  if S.SummonGargoyle:IsReady() then
    if Cast(S.SummonGargoyle, Settings.Unholy.GCDasOffGCD.SummonGargoyle) then addonTable.cast("Summon Gargoyle") return "summon_gargoyle aoe_cooldowns 4"; end
  end
  ...
  if S.AbominationLimb:IsCastable() and (Player:Rune() < 2 or FesterStacks > 10 or not S.Festermight:IsAvailable() or Player:BuffUp(S.FestermightBuff) and Player:BuffRemains(S.FestermightBuff) < 12) then
    if Cast(S.AbominationLimb, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(20)) then addonTable.cast("Abomination Limb") return "abomination_limb aoe_cooldowns 6"; end
  end
  ...
  if S.Apocalypse:IsReady() then
    if Everyone.CastTargetIf(S.Apocalypse, EnemiesMelee, "min", EvaluateTargetIfFilterFWStack, EvaluateTargetIfApocalypseAoECDs, not Target:IsInMeleeRange(5)) then addonTable.cast("Apocalypse") return "apocalypse aoe_cooldowns 8"; end
  end
  ...
  if S.Apocalypse:IsReady() then
    if Everyone.CastTargetIf(S.Apocalypse, EnemiesMelee, "max", EvaluateTargetIfFilterFWStack, EvaluateTargetIfApocalypseAoECDs2, not Target:IsInMeleeRange(5)) then addonTable.cast("Apocalypse") return "apocalypse aoe_cooldowns 9"; end
  end
  ...
  if S.UnholyAssault:IsCastable() then
    if Everyone.CastTargetIf(S.UnholyAssault, EnemiesMelee, "min", EvaluateTargetIfFilterFWStack, EvaluateTargetIfUnholyAssaultAoECDs, not Target:IsInMeleeRange(5), Settings.Unholy.GCDasOffGCD.UnholyAssault) then addonTable.cast("Unholy Assault") return "unholy_assault aoe_cooldowns 10"; end
  end
  ...
  if S.RaiseDead:IsCastable() then
    if Settings.Unholy.RaiseDeadCastLeft then
      ...
      if Cast(S.RaiseDead, nil, Settings.CommonsDS.DisplayStyle.RaiseDead) then addonTable.cast("Raise Dead") return "raise_dead aoe_cooldowns 12 displaystyle"; end
    end
  end
  ...
  if S.DarkTransformation:IsReady() and (AnyDnD:CooldownRemains() < 10 and S.InfectedClaws:IsAvailable() and (S.FesteringWoundDebuff:AuraActiveCount() < ActiveEnemies or not S.VileContagion:IsAvailable()) or not S.InfectedClaws:IsAvailable()) then
    if Cast(S.DarkTransformation, Settings.Unholy.GCDasOffGCD.DarkTransformation) then addonTable.cast("Dark Transformation") return "dark_transformation aoe_cooldowns 14"; end
  end
  ...
  if S.EmpowerRuneWeapon:IsCastable() and (Pet:BuffUp(S.DarkTransformation)) then
    if Cast(S.EmpowerRuneWeapon, Settings.CommonsOGCD.GCDasOffGCD.EmpowerRuneWeapon) then addonTable.cast("Empower Rune Weapon") return "empower_rune_weapon aoe_cooldowns 16"; end
  end
  ...
  if S.SacrificialPact:IsReady() and (Pet:BuffDown(S.DarkTransformation) and S.DarkTransformation:CooldownRemains() > 6 or FightRemains < Player:GCD()) then
    if Cast(S.SacrificialPact, Settings.CommonsOGCD.GCDasOffGCD.SacrificialPact) then addonTable.cast("Sacrificial Pact") return "sacrificial_pact aoe_cooldowns 18"; end
  end
end
...
local function AoESetup()
  ...
  if AnyDnD:IsReady() and ((not S.BurstingSores:IsAvailable() or S.FesteringWoundDebuff:AuraActiveCount() == ActiveEnemies or S.FesteringWoundDebuff:AuraActiveCount() >= 8) and (not S.Defile:IsAvailable() or S.Defile:IsAvailable() and Player:BuffRemains(S.DefileBuff) < Player:GCD() + 0.25)) then
    if Cast(AnyDnD, Settings.CommonsOGCD.GCDasOffGCD.DeathAndDecay) then addonTable.cast("Death and Decay") return "any_dnd aoe_setup 2"; end
  end
  ...
  if S.FesteringStrike:IsReady() and (S.FesteringWoundDebuff:AuraActiveCount() < ActiveEnemies and S.BurstingSores:IsAvailable()) then
    if Everyone.CastTargetIf(S.FesteringStrike, EnemiesMelee, "min", EvaluateTargetIfFilterFWStack, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Festering Strike") return "festering_strike aoe_setup 4"; end
  end
  -- epidemic,if=!variable.pooling_runic_power|fight_remains<10
  if S.Epidemic:IsReady() and (not VarPoolingRunicPower or FightRemains < 10) then
    if Cast(S.Epidemic, Settings.Unholy.GCDasOffGCD.Epidemic, nil, not Target:IsInRange(40)) then addonTable.cast("Epidemic") return "epidemic aoe_setup 6"; end
  end
  ...
  if S.FesteringStrike:IsReady() and (S.FesteringWoundDebuff:AuraActiveCount() < ActiveEnemies) then
    if Everyone.CastTargetIf(S.FesteringStrike, EnemiesMelee, "min", EvaluateTargetIfFilterFWStack, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Festering Strike") return "festering_strike aoe_setup 8"; end
  end
  ...
  if S.FesteringStrike:IsReady() and (S.Apocalypse:CooldownRemains() < VarApocTiming) then
    if Everyone.CastTargetIf(S.FesteringStrike, EnemiesMelee, "max", EvaluateTargetIfFilterFWStack, EvaluateTargetIfFesteringStrikeAoESetup, not Target:IsInMeleeRange(5)) then addonTable.cast("Festering Strike") return "festering_strike aoe_setup 10"; end
  end
  -- death_coil,if=!variable.pooling_runic_power&!talent.epidemic
  if S.DeathCoil:IsReady() and (not VarPoolingRunicPower and not S.Epidemic:IsAvailable()) then
    if Cast(S.DeathCoil, nil, nil, not Target:IsSpellInRange(S.DeathCoil)) then addonTable.cast("Death Coil") return "death_coil aoe_setup 12"; end
  end
end
...
local function Cooldowns()
  ...
  if S.SummonGargoyle:IsCastable() and (Player:BuffUp(S.CommanderoftheDeadBuff) or not S.CommanderoftheDead:IsAvailable()) then
    if Cast(S.SummonGargoyle, Settings.Unholy.GCDasOffGCD.SummonGargoyle) then addonTable.cast("Summon Gargoyle") return "summon_gargoyle cooldowns 2"; end
  end
  -- raise_dead,if=!pet.ghoul.active
  if S.RaiseDead:IsCastable() then
    if Settings.Unholy.RaiseDeadCastLeft then
      ...
      if Cast(S.RaiseDead, nil, Settings.CommonsDS.DisplayStyle.RaiseDead) then addonTable.cast("Raise Dead") return "raise_dead cooldowns 4 displaystyle"; end
    end
  end
  ...
  if S.DarkTransformation:IsReady() and (S.Apocalypse:CooldownRemains() < 5) then
    if Cast(S.DarkTransformation, Settings.Unholy.GCDasOffGCD.DarkTransformation) then addonTable.cast("Dark Transformation") return "dark_transformation cooldowns 6"; end
  end
  ...
  if S.Apocalypse:IsReady() and (VarSTPlanning) then
    if Everyone.CastTargetIf(S.Apocalypse, EnemiesMelee, "max", EvaluateTargetIfFilterFWStack, EvaluateTargetIfApocalypseCDs, not Target:IsInMeleeRange(5), Settings.Unholy.GCDasOffGCD.Apocalypse) then addonTable.cast("Apocalypse") return "apocalypse cooldowns 8"; end
  end
  -- abomination_limb,if=rune<3&variable.st_planning
  if S.AbominationLimb:IsCastable() and (Player:Rune() < 3 and VarSTPlanning) then
    if Cast(S.AbominationLimb, nil, Settings.CommonsDS.DisplayStyle.Signature) then addonTable.cast("Abomination Limb") return "abomination_limb cooldowns 12"; end
  end
  ...
  if S.UnholyAssault:IsReady() and (VarSTPlanning) then
    if Everyone.CastTargetIf(S.UnholyAssault, EnemiesMelee, "min", EvaluateTargetIfFilterFWStack, nil, not Target:IsInMeleeRange(5), Settings.Unholy.GCDasOffGCD.UnholyAssault) then addonTable.cast("Unholy Assault") return "unholy_assault cooldowns 14"; end
  end
  ...
  if S.SoulReaper:IsReady() and (ActiveEnemies == 1 and (Target:TimeToX(35) < 5 or Target:HealthPercentage() <= 35) and Target:TimeToDie() > 5) then
    if Cast(S.SoulReaper, nil, nil, not Target:IsSpellInRange(S.SoulReaper)) then addonTable.cast("Soul Reaper") return "soul_reaper cooldowns 16"; end
  end
  ...
  if S.SoulReaper:IsReady() and (ActiveEnemies >= 2) then
    if Everyone.CastTargetIf(S.SoulReaper, EnemiesMelee, "min", EvaluateTargetIfFilterSoulReaper, EvaluateTargetIfSoulReaperCDs, not Target:IsSpellInRange(S.SoulReaper)) then addonTable.cast("Soul Reaper") return "soul_reaper cooldowns 18"; end
  end
end
...
local function GargSetup()
  ...
  if S.Apocalypse:IsReady() and (FesterStacks >= 4 and (Player:BuffUp(S.CommanderoftheDeadBuff) and VarGargRemains < 23 or not S.CommanderoftheDead:IsAvailable())) then
    if Cast(S.Apocalypse, Settings.Unholy.GCDasOffGCD.Apocalypse, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Apocalypse") return "apocalypse garg_setup 2"; end
  end
  -- soul_reaper,if=active_enemies=1&target.time_to_pct_35<5&target.time_to_die>5
  if S.SoulReaper:IsReady() and (ActiveEnemies == 1 and (Target:TimeToX(35) < 5 or Target:HealthPercentage() <= 35) and Target:TimeToDie() > 5) then
    if Cast(S.SoulReaper, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Soul Reaper") return "soul_reaper garg_setup 4"; end
  end
  ...
  if AnyDnD:IsReady() and (Player:BuffDown(S.DeathAndDecayBuff) and FesterStacks > 1) then
    if Cast(AnyDnD, Settings.CommonsOGCD.GCDasOffGCD.DeathAndDecay) then addonTable.cast("Death and Decay") return "any_dnd garg_setup 6"; end
  end
  ...
  if S.SummonGargoyle:IsCastable() and CDsON() and (Player:BuffUp(S.CommanderoftheDeadBuff) or not S.CommanderoftheDead:IsAvailable() and Player:RunicPower() >= 40) then
    if Cast(S.SummonGargoyle, Settings.Unholy.GCDasOffGCD.SummonGargoyle) then addonTable.cast("Summon Gargoyle") return "summon_gargoyle garg_setup 8"; end
  end
  if CDsON() and (VarGargActive and VarGargRemains <= 23) then
    ...
    if S.EmpowerRuneWeapon:IsCastable() then
      if Cast(S.EmpowerRuneWeapon, Settings.CommonsOGCD.GCDasOffGCD.EmpowerRuneWeapon) then addonTable.cast("Empower Rune Weapon") return "empower_rune_weapon garg_setup 10"; end
    end
    -- unholy_assault,if=pet.gargoyle.active&pet.gargoyle.remains<=23
    if S.UnholyAssault:IsCastable() then
      if Cast(S.UnholyAssault, Settings.Unholy.GCDasOffGCD.UnholyAssault, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Unholy Assault") return "unholy_assault garg_setup 12"; end
    end
  end
  ...
  if S.DarkTransformation:IsReady() and (S.CommanderoftheDead:IsAvailable() and Player:RunicPower() > 40 or not S.CommanderoftheDead:IsAvailable()) then
    if Cast(S.DarkTransformation, Settings.Unholy.GCDasOffGCD.DarkTransformation) then addonTable.cast("Dark Transformation") return "dark_transformation garg_setup 14"; end
  end
  ...
  if S.FesteringStrike:IsReady() and (FesterStacks == 0 or not S.Apocalypse:IsAvailable() or Player:RunicPower() < 40 and not VarGargActive) then
    if Cast(S.FesteringStrike, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Festering Strike") return "festering_strike garg_setup 16"; end
  end
  -- death_coil,if=rune<=1
  if S.DeathCoil:IsReady() and (Player:Rune() <= 1) then
    if Cast(S.DeathCoil, nil, nil, not Target:IsSpellInRange(S.DeathCoil)) then addonTable.cast("Death Coil") return "death_coil garg_setup 18"; end
  end
end
...
local function HighPrioActions()
  ...
  if Settings.Commons.UseAMSAMZOffensively then
    ...
    if S.AntiMagicShell:IsCastable() and (Player:RunicPowerDeficit() > 40 and (VarGargActive or not S.SummonGargoyle:IsAvailable() or S.SummonGargoyle:CooldownRemains() > 40)) then
      if Cast(S.AntiMagicShell, Settings.CommonsOGCD.GCDasOffGCD.AntiMagicShell) then addonTable.cast("Anti-Magic Shell") return "antimagic_shell ams_amz 2"; end
    end
    ...
    if S.AntiMagicZone:IsCastable() and (Player:RunicPowerDeficit() > 70 and S.Assimilation:IsAvailable() and (VarGargActive or not S.SummonGargoyle:IsAvailable())) then
      if Cast(S.AntiMagicZone, Settings.CommonsOGCD.GCDasOffGCD.AntiMagicZone) then addonTable.cast("Anti-Magic Zone") return "antimagic_zone ams_amz 4"; end
    end
  ...
  if AnyDnD:IsReady() and (VarAddsRemain and Player:BuffDown(S.DeathAndDecayBuff) and not S.BurstingSores:IsAvailable() and S.Defile:IsAvailable() and Player:BuffRemains(S.DefileBuff) < Player:GCD() + 0.25) then
    if Cast(AnyDnD, Settings.CommonsOGCD.GCDasOffGCD.DeathAndDecay) then addonTable.cast("Death and Decay") return "any_dnd high_prio_actions 3"; end
  end
  ...
  if CDsON() then
    if S.ArmyoftheDead:IsReady() and (S.SummonGargoyle:IsAvailable() and S.SummonGargoyle:CooldownRemains() < 3 * Player:GCD() or not S.SummonGargoyle:IsAvailable() or FightRemains < 35) then
      if Cast(S.ArmyoftheDead, nil, Settings.Unholy.DisplayStyle.ArmyOfTheDead) then addonTable.cast("Army of the Dead") return "army_of_the_dead high_prio_actions 4"; end
    end
  end
  ...
  if S.DeathCoil:IsReady() and ((ActiveEnemies <= 3 or not S.Epidemic:IsAvailable()) and (VarGargActive and S.CommanderoftheDead:IsAvailable() and Player:BuffUp(S.CommanderoftheDeadBuff) and S.Apocalypse:CooldownRemains() < 5 and Player:BuffRemains(S.CommanderoftheDeadBuff) > 26 or Target:DebuffUp(S.DeathRotDebuff) and Target:DebuffRemains(S.DeathRotDebuff) < Player:GCD())) then
    if Cast(S.DeathCoil, nil, nil, not Target:IsSpellInRange(S.DeathCoil)) then addonTable.cast("Death Coil") return "death_coil high_prio_actions 6"; end
  end
  ...
  if S.Epidemic:IsReady() and (ActiveEnemies >= 4 and (S.CommanderoftheDead:IsAvailable() and Player:BuffUp(S.CommanderoftheDeadBuff) and S.Apocalypse:CooldownRemains() < 5 or Target:DebuffUp(S.DeathRotDebuff) and Target:DebuffRemains(S.DeathRotDebuff) < Player:GCD())) then
    if Cast(S.Epidemic, Settings.Unholy.GCDasOffGCD.Epidemic, nil, not Target:IsInRange(40)) then addonTable.cast("Epidemic") return "epidemic high_prio_actions 8"; end
  end
  ...
  if WoundSpender:IsReady() and ((S.Apocalypse:CooldownRemains() > VarApocTiming + 3 or S.UnholyAssault:CooldownUp() or ActiveEnemies >= 3) and S.Plaguebringer:IsAvailable() and (S.Superstrain:IsAvailable() or S.UnholyBlight:IsAvailable()) and Player:BuffRemains(S.PlaguebringerBuff) < Player:GCD()) then
    if Cast(WoundSpender, nil, nil, not Target:IsSpellInRange(WoundSpender)) then addonTable.cast("Scourge Strike") return "wound_spender high_prio_actions 10"; end
  end
  ...
  if S.UnholyBlight:IsReady() and (VarSTPlanning and ((not S.Apocalypse:IsAvailable() or S.Apocalypse:CooldownDown() or not S.SummonGargoyle:IsAvailable()) and S.Morbidity:IsAvailable() or not S.Morbidity:IsAvailable()) or VarAddsRemain or FightRemains < 21) then
    if Cast(S.UnholyBlight, Settings.Unholy.GCDasOffGCD.UnholyBlight, nil, not Target:IsInRange(8)) then addonTable.cast("Unholy Blight") return "unholy_blight high_prio_actions 12"; end
  end
  ...
  if S.Outbreak:IsReady() then
    if Everyone.CastCycle(S.Outbreak, EnemiesMelee, EvaluateCycleOutbreak, not Target:IsSpellInRange(S.Outbreak)) then addonTable.cast("Outbreak") return "outbreak high_prio_actions 14"; end
  end
end
...
local function Racials()
  ...
  if S.ArcaneTorrent:IsCastable() and (Player:RunicPowerDeficit() > 20 and (S.SummonGargoyle:CooldownRemains() < Player:GCD() and HL.CombatTime() > 15 or not S.SummonGargoyle:IsAvailable() or VarGargActive and Player:Rune() < 2 and FesterStacks < 1)) then
    if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsInRange(8)) then addonTable.cast("Arcane Torrent") return "arcane_torrent racials 2"; end
  end
  ...
  if S.BloodFury:IsCastable() and ((S.BloodFury:BaseDuration() + 3 >= VarGargRemains and VarGargActive) or (not S.SummonGargoyle:IsAvailable() or S.SummonGargoyle:CooldownRemains() > 60) and (VarArmyGhoulActive and VarArmyGhoulRemains <= S.BloodFury:BaseDuration() + 3 or VarApocGhoulActive and VarApocGhoulRemains <= S.BloodFury:BaseDuration() + 3 or ActiveEnemies >= 2 and Player:BuffUp(S.DeathAndDecayBuff)) or FightRemains <= S.BloodFury:BaseDuration() + 3) then
    if Cast(S.BloodFury, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Blood Fury") return "blood_fury racials 4"; end
  end
  ...
  if S.Berserking:IsCastable() and ((S.Berserking:BaseDuration() + 3 >= VarGargRemains and VarGargActive) or (not S.SummonGargoyle:IsAvailable() or S.SummonGargoyle:CooldownRemains() > 60) and (VarArmyGhoulActive and VarArmyGhoulRemains <= S.Berserking:BaseDuration() + 3 or VarApocGhoulActive and VarApocGhoulRemains <= S.Berserking:BaseDuration() + 3 or ActiveEnemies >= 2 and Player:BuffUp(S.DeathAndDecayBuff)) or FightRemains <= S.Berserking:BaseDuration() + 3) then
    if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Berserking") return "berserking racials 6"; end
  end
  ...
  if S.ArcanePulse:IsCastable() and (ActiveEnemies >= 2 or (Player:Rune() <= 1 and Player:RunicPowerDeficit() >= 60)) then
    if Cast(S.ArcanePulse, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsInRange(8)) then addonTable.cast("Arcane Pulse") return "arcane_pulse racials 12"; end
  end
  ...
  if S.Fireblood:IsCastable() and ((S.Fireblood:BaseDuration() + 3 >= VarGargRemains and VarGargActive) or (not S.SummonGargoyle:IsAvailable() or S.SummonGargoyle:CooldownRemains() > 60) and (VarArmyGhoulActive and VarArmyGhoulRemains <= S.Fireblood:BaseDuration() + 3 or VarApocGhoulActive and VarApocGhoulRemains <= S.Fireblood:BaseDuration() + 3 or ActiveEnemies >= 2 and Player:BuffUp(S.DeathAndDecayBuff)) or FightRemains <= S.Fireblood:BaseDuration() + 3) then
    if Cast(S.Fireblood, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Fireblood") return "fireblood racials 14"; end
  end
  ...
  if S.BagofTricks:IsCastable() and (ActiveEnemies == 1 and (Player:BuffUp(S.UnholyStrengthBuff) or HL.FilteredFightRemains(EnemiesMelee, "<", 5))) then
    if Cast(S.BagofTricks, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsSpellInRange(S.BagofTricks)) then addonTable.cast("Bag of Tricks") return "bag_of_tricks racials 16"; end
  end
end
...
local function ST()
  ...
  if S.DeathCoil:IsReady() and (not VarEpidemicPriority and (not VarPoolingRunicPower and VarSpendRP or FightRemains < 10)) then
    if Cast(S.DeathCoil, nil, nil, not Target:IsSpellInRange(S.DeathCoil)) then addonTable.cast("Death Coil") return "death_coil st 2"; end
  end
  ...
  if S.Epidemic:IsReady() and (VarEpidemicPriority and (not VarPoolingRunicPower and VarSpendRP or FightRemains < 10)) then
    if Cast(S.Epidemic, Settings.Unholy.GCDasOffGCD.Epidemic, nil, not Target:IsInRange(40)) then addonTable.cast("Epidemic") return "epidemic st 4"; end
  end
  ...
  if AnyDnD:IsReady() and (Player:BuffDown(S.DeathAndDecayBuff) and (ActiveEnemies >= 2 or S.UnholyGround:IsAvailable() and (VarApocGhoulActive and VarApocGhoulRemains >= 13 or VarGargActive and VarGargRemains > 8 or VarArmyGhoulActive and VarArmyGhoulRemains > 8 or not VarPopWounds and FesterStacks >= 4) or S.Defile:IsAvailable() and (VarGargActive or VarApocGhoulActive or VarArmyGhoulActive or Pet:BuffUp(S.DarkTransformation))) and (S.FesteringWoundDebuff:AuraActiveCount() == ActiveEnemies or ActiveEnemies == 1)) then
    if Cast(AnyDnD, Settings.CommonsOGCD.GCDasOffGCD.DeathAndDecay) then addonTable.cast("Death and Decay") return "any_dnd st 6"; end
  end
  ...
  if WoundSpender:IsReady() and (VarPopWounds or ActiveEnemies >= 2 and Player:BuffUp(S.DeathAndDecayBuff)) then
    if Cast(WoundSpender, nil, nil, not Target:IsSpellInRange(WoundSpender)) then addonTable.cast("Scourge Strike") return "wound_spender st 8"; end
  end
  -- festering_strike,target_if=min:debuff.festering_wound.stack,if=!variable.pop_wounds&debuff.festering_wound.stack<4
  if S.FesteringStrike:IsReady() and (not VarPopWounds) then
    if Everyone.CastTargetIf(S.FesteringStrike, EnemiesMelee, "min", EvaluateTargetIfFilterFWStack, EvaluateTargetIfFesteringStrikeST, not Target:IsInMeleeRange(5)) then addonTable.cast("Festering Strike") return "festering_strike st 10"; end
  end
  -- death_coil
  if S.DeathCoil:IsReady() then
    if Cast(S.DeathCoil, nil, nil, not Target:IsSpellInRange(S.DeathCoil)) then addonTable.cast("Death Coil") return "death_coil st 12"; end
  end
  ...
  if WoundSpender:IsReady() and (not VarPopWounds) then
    if Everyone.CastTargetIf(WoundSpender, EnemiesMelee, "max", EvaluateTargetIfFilterFWStack, EvaluateTargetIfFesteringStrikeST2, not Target:IsSpellInRange(WoundSpender)) then addonTable.cast("Scourge Strike") return "wound_spender st 14"; end
  end
end
...
addonTable.resetPixels()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.DeathStrike:IsReady() and not no_heal then
      if Cast(S.DeathStrike) then addonTable.cast("Death Strike") return "death_strike low hp or proc"; end
    end
    -- auto_attack
    ...
    if not Target:IsInRange(10) then
      ...
      if S.Outbreak:IsReady() and (EnemiesWithoutVP > 0) then
        if Cast(S.Outbreak, nil, nil, not Target:IsSpellInRange(S.Outbreak)) then addonTable.cast("Outbreak") return "outbreak out_of_range"; end
      end
      ...
      if S.Epidemic:IsReady() and AoEON() and S.VirulentPlagueDebuff:AuraActiveCount() > 1 and not VarPoolingRunicPower then
        if Cast(S.Epidemic, Settings.Unholy.GCDasOffGCD.Epidemic, nil, not Target:IsInRange(40)) then addonTable.cast("Epidemic") return "epidemic out_of_range"; end
      end
      ...
      if S.DeathCoil:IsReady() and S.VirulentPlagueDebuff:AuraActiveCount() < 2 and not VarPoolingRunicPower then
        if Cast(S.DeathCoil, nil, nil, not Target:IsSpellInRange(S.DeathCoil)) then addonTable.cast("Death Coil") return "death_coil out_of_range"; end
      end
    end
