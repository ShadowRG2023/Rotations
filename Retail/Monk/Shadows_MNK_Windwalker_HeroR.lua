...
addonTable.spells = {
  ...
  { spell = "MACRO OptiAction3", name = "StormEarthAndFire" },
  ...
  { spell = "SPELL Whirling Dragon Punch", name = "Whirling Dragon Punch" },
  { spell = "SPELL Touch of Karma", name = "Touch of Karma" },
  { spell = "SPELL Touch of Death", name = "Touch of Death" },
  { spell = "SPELL Tiger Palm", name = "Tiger Palm" },
  { spell = "SPELL Strike of the Windlord", name = "Strike of the Windlord" },
  --{ spell = "SPELL StormEarthandFire", name = "StormEarthandFire" },
  { spell = "SPELL Spinning Crane Kick", name = "Spinning Crane Kick" },
  { spell = "SPELL Serenity", name = "Serenity" },
  ...
  { spell = "SPELL Jadefire Stomp", name = "Jadefire Stomp" },
  { spell = "SPELL Invoke Xuen, the White Tiger", name = "Invoke Xuen, the White Tiger" },
  { spell = "SPELL Fortifying Brew", name = "Fortifying Brew" },
  { spell = "SPELL Flying Serpent Kick", name = "Flying Serpent Kick" },
  { spell = "SPELL Fists of Fury", name = "Fists of Fury" },
  { spell = "SPELL Expel Harm", name = "Expel Harm" },
  { spell = "SPELL Chi Wave", name = "Chi Wave" },
  { spell = "SPELL Chi Burst", name = "Chi Burst" },
  { spell = "SPELL Crackling Jade Lightning", name = "Crackling Jade Lightning" },
...
local function Precombat()
  ...
  if S.SummonWhiteTigerStatue:IsCastable() and CDsON() then
    if Cast(S.SummonWhiteTigerStatue, Settings.CommonsOGCD.GCDasOffGCD.SummonWhiteTigerStatue, nil, not Target:IsInRange(40)) then addonTable.cast("Summon White Tiger Statue") return "summon_white_tiger_statue precombat 2"; end
  end
  -- expel_harm,if=chi<chi.max
  if S.ExpelHarm:IsReady() and (Player:Chi() < Player:ChiMax()) then
    if Cast(S.ExpelHarm) then addonTable.cast("Expel Harm") return "expel_harm precombat 4"; end
  end
  -- chi_burst,if=!talent.jadefire_stomp
  if S.ChiBurst:IsCastable() and (not S.JadefireStomp:IsAvailable()) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst precombat 6"; end
  end
  -- chi_wave
  if S.ChiWave:IsReady() then
    if Cast(S.ChiWave, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Wave") return "chi_wave precombat 8"; end
  end
end
...
local function Opener()
  -- summon_white_tiger_statue
  if S.SummonWhiteTigerStatue:IsCastable() and CDsON() then
    if Cast(S.SummonWhiteTigerStatue, Settings.CommonsOGCD.GCDasOffGCD.SummonWhiteTigerStatue, nil, not Target:IsInRange(40)) then addonTable.cast("Summon White Tiger Statue") return "summon_white_tiger_statue opener 2"; end
  end
  ...
  if S.ExpelHarm:IsReady() and (S.ChiBurst:IsAvailable() and Player:ChiDeficit() >= 3) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm opener 4"; end
  end
  ...
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 2 and Target:DebuffDown(S.SkyreachExhaustionDebuff)) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Jadefire Stomp") return "tiger_palm opener 6"; end
  end
  -- expel_harm,if=talent.chi_burst.enabled&chi=3
  if S.ExpelHarm:IsReady() and (S.ChiBurst:IsAvailable() and Player:Chi() == 3) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm opener 8"; end
  end
  -- chi_wave,if=chi.max-chi=2
  if S.ChiWave:IsReady() and (Player:ChiDeficit() >= 2) then
    if Cast(S.ChiWave, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Wave") return "chi_wave opener 10"; end
  end
  -- expel_harm
  if S.ExpelHarm:IsReady() then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm opener 12"; end
  end
  -- chi_burst,if=chi>1&chi.max-chi>=2
  if S.ChiBurst:IsCastable() and (Player:Chi() > 1 and Player:ChiDeficit() >= 2) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst opener 14"; end
  end
end
...
local function BDBSetup()
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and EnemiesCount8y > 3) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord bdb_setup 2"; end
  end
  -- bonedust_brew,if=spinning_crane_kick.max&chi>=4
  if S.BonedustBrew:IsCastable() and (SCKMax() and Player:Chi() >= 4) then
    if Cast(S.BonedustBrew, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(40)) then addonTable.cast("Bonedust Brew") return "bonedust_brew bdb_setup 4"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:ChiDeficit() >= 2 and Player:BuffUp(S.StormEarthAndFireBuff)) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane101, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm bdb_setup 6"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and not S.WhirlingDragonPunch:IsAvailable() and not SCKMax()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick bdb_setup 8"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (ComboStrike(S.RisingSunKick) and Player:Chi() >= 5 and S.WhirlingDragonPunch:IsAvailable()) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick bdb_setup 10"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (ComboStrike(S.RisingSunKick) and EnemiesCount8y >= 2 and S.WhirlingDragonPunch:IsAvailable()) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick bdb_setup 12"; end
  end
end
...
local function CDSEF()
  ...
  if S.InvokeXuenTheWhiteTiger:IsCastable() and ((Target:TimeToDie() > 35 and VarDungeonRoute or Target:TimeToDie() > 14 and not VarDungeonRoute) and FightRemains > 120 and (Trinket1:ID() ~= I.AshesoftheEmbersoul:ID() and Trinket1:ID() ~= I.WitherbarksBranch:ID() and Trinket2:ID() ~= I.AshesoftheEmbersoul:ID() and Trinket2:ID() ~= I.WitherbarksBranch:ID() or (Trinket1:ID() == I.AshesoftheEmbersoul:ID() or Trinket1:ID() == I.WitherbarksBranch:ID()) and not (Trinket1:CooldownRemains() > 0) or (Trinket2:ID() == I.AshesoftheEmbersoul:ID() or Trinket2:ID() == I.WitherbarksBranch:ID()) and not (Trinket2:CooldownRemains() > 0))) then
    if Cast(S.InvokeXuenTheWhiteTiger, Settings.Windwalker.GCDasOffGCD.InvokeXuenTheWhiteTiger, nil, not Target:IsInRange(40)) then addonTable.cast("Invoke Xuen, the White Tiger") return "invoke_xuen_the_white_tiger cd_sef 2"; end
  end
  ...
  if S.InvokeXuenTheWhiteTiger:IsCastable() and (FightRemains < 60 and (Target:DebuffRemains(S.SkyreachExhaustionDebuff) < 2 or Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 55) and EnemiesCount8y < 3 or BossFightRemains < 25) then
    if Cast(S.InvokeXuenTheWhiteTiger, Settings.Windwalker.GCDasOffGCD.InvokeXuenTheWhiteTiger, nil, not Target:IsInRange(40)) then addonTable.cast("Invoke Xuen, the White Tiger") return "invoke_xuen_the_white_tiger cd_sef 4"; end
  end
  ...
  if CDsON() then
    if S.StormEarthAndFire:IsCastable() and (XuenActive or Target:TimeToDie() > 15 and S.StormEarthAndFire:FullRechargeTime() < S.InvokeXuenTheWhiteTiger:CooldownRemains()) then
      if Cast(S.StormEarthAndFire, Settings.Windwalker.OffGCDasOffGCD.StormEarthAndFire) then addonTable.cast("Storm Earth And Fire") return "storm_earth_and_fire cd_sef 6"; end
    end
  end
  ...
  if S.BonedustBrew:IsCastable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 45 and (Player:BuffDown(S.BonedustBrewBuff) and Player:BuffUp(S.StormEarthAndFireBuff) and Player:BuffRemains(S.StormEarthAndFireBuff) < 11 and SCKMax()) or (Player:BuffDown(S.BonedustBrewBuff) and FightRemains < 30 and FightRemains > 10 and SCKMax() and Player:Chi() >= 4) or BossFightRemains < 10 or (Target:DebuffDown(S.SkyreachExhaustionDebuff) and EnemiesCount8y >= 4 and SCKModifier() >= 2) or (XuenActive and SCKMax() and EnemiesCount8y >= 4 or BossFightRemains < 15 or S.InvokeXuenTheWhiteTiger:CooldownRemains() > 45 and (Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 55 or Target:DebuffRemains(S.SkyreachExhaustionDebuff) < 2) or S.InvokeXuenTheWhiteTiger:CooldownRemains() > 45 and Target:TimeToDie() > 10)) then
    if Cast(S.BonedustBrew, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(40)) then addonTable.cast("Bonedust Brew") return "bonedust_brew cd_sef 8"; end
  end
  ...
  if CDsON() then
    if S.StormEarthAndFire:IsCastable() and (BossFightRemains < 20 or (S.StormEarthAndFire:Charges() == 2 and S.InvokeXuenTheWhiteTiger:CooldownRemains() > S.StormEarthAndFire:FullRechargeTime()) and S.FistsofFury:CooldownRemains() <= 9 and Player:Chi() >= 2 and S.WhirlingDragonPunch:CooldownRemains() <= 12) then
      if Cast(S.StormEarthAndFire, Settings.Windwalker.OffGCDasOffGCD.StormEarthAndFire) then addonTable.cast("Storm Earth And Fire") return "storm_earth_and_fire cd_sef 10"; end
    end
  end
    ...
    if ToDTar then
      ...
      if VarDungeonRoute and Player:BuffDown(S.SerenityBuff) and ComboStrike(S.TouchofDeath) and ToDTar:Health() < Player:Health() or Player:BuffRemains(S.HiddenMastersForbiddenTouchBuff) < 2 or Player:BuffRemains(S.HiddenMastersForbiddenTouchBuff) > ToDTar:TimeToDie() then
        if ToDTar:GUID() == Target:GUID() then
          if Cast(S.TouchofDeath, Settings.Windwalker.GCDasOffGCD.TouchOfDeath, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Touch of Death") return "touch_of_death cd_sef main-target 12"; end
        else
    ...
    if ToDTar and ComboStrike(S.TouchofDeath) then
      if VarDungeonRoute then
        ...
        if ToDTar:TimeToDie() > 60 or ToDTar:DebuffUp(S.BonedustBrewDebuff) or BossFightRemains < 10 then
          if ToDTar:GUID() == Target:GUID() then
            if Cast(S.TouchofDeath, Settings.Windwalker.GCDasOffGCD.TouchOfDeath, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Touch of Death") return "touch_of_death cd_sef main-target 14"; end
          else
        ...
        if ToDTar:GUID() == Target:GUID() then
          if Cast(S.TouchofDeath, Settings.Windwalker.GCDasOffGCD.TouchOfDeath, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Touch of Death") return "touch_of_death cd_sef main-target 16"; end
        else
  ...
  if S.TouchofKarma:IsCastable() and not Settings.Windwalker.IgnoreToK and ((S.InvokeXuenTheWhiteTiger:IsAvailable() and (FightRemains > 90 or XuenActive or VarHoldXuen or BossFightRemains < 16)) or (not S.InvokeXuenTheWhiteTiger:IsAvailable() and (FightRemains > 159 or VarHoldXuen))) then
    if Cast(S.TouchofKarma, Settings.Windwalker.GCDasOffGCD.TouchOfKarma, nil, not Target:IsInRange(20)) then addonTable.cast("Touch of Karma") return "touch_of_karma cd_sef 18"; end
  end
  ...
  if S.AncestralCall:IsCastable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 30 or VarHoldXuen or BossFightRemains < 20) then
    if Cast(S.AncestralCall, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Ancestral Call") return "ancestral_call cd_sef 20"; end
  end
  ...
  if S.BloodFury:IsCastable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 30 or VarHoldXuen or BossFightRemains < 20) then
    if Cast(S.BloodFury, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Blood Fury") return "blood_fury cd_sef 22"; end
  end
  ...
  if S.Fireblood:IsCastable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 30 or VarHoldXuen or BossFightRemains < 10) then
    if Cast(S.Fireblood, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Fireblood") return "fireblood cd_sef 24"; end
  end
  ...
  if S.Berserking:IsCastable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 30 or VarHoldXuen or BossFightRemains < 15) then
    if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Berserking") return "berserking cd_sef 26"; end
  end
  ...
  if S.BagofTricks:IsCastable() and (Player:BuffDown(S.StormEarthAndFireBuff)) then
    if Cast(S.BagofTricks, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Bag of Tricks") return "bag_of_tricks cd_sef 28"; end
  end
  -- lights_judgment
  if S.LightsJudgment:IsCastable() then
    if Cast(S.LightsJudgment, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Light's Judgment") return "lights_judgment cd_sef 30"; end
  end
end
...
local function CDSerenity()
  ...
  if S.InvokeXuenTheWhiteTiger:IsCastable() and (Player:BloodlustUp() or BossFightRemains < 25) then
    if Cast(S.InvokeXuenTheWhiteTiger, Settings.Windwalker.GCDasOffGCD.InvokeXuenTheWhiteTiger, nil, not Target:IsInRange(40)) then addonTable.cast("Invoke Xuen, the White Tiger") return "invoke_xuen_the_white_tiger cd_serenity 2"; end
  end
  ...
  if S.InvokeXuenTheWhiteTiger:IsCastable() and ((Target:TimeToDie() > 16 or VarDungeonRoute and S.Serenity:CooldownRemains() < 2) and FightRemains > 120 and (Trinket1:ID() ~= I.AshesoftheEmbersoul:ID() and Trinket1:ID() ~= I.WitherbarksBranch:ID() and Trinket2:ID() ~= I.AshesoftheEmbersoul:ID() and Trinket2:ID() ~= I.WitherbarksBranch:ID() or (Trinket1:ID() == I.AshesoftheEmbersoul:ID() or Trinket1:ID() == I.WitherbarksBranch:ID()) and not (Trinket1:CooldownRemains() > 0) or (Trinket2:ID() == I.AshesoftheEmbersoul:ID() or Trinket2:ID() == I.WitherbarksBranch:ID()) and not (Trinket2:CooldownRemains() > 0)) and Target:TimeToDie() > 14) then
    if Cast(S.InvokeXuenTheWhiteTiger, Settings.Windwalker.GCDasOffGCD.InvokeXuenTheWhiteTiger, nil, not Target:IsInRange(40)) then addonTable.cast("Invoke Xuen, the White Tiger") return "invoke_xuen_the_white_tiger cd_serenity 4"; end
  end
  ...
  if S.InvokeXuenTheWhiteTiger:IsCastable() and (Target:TimeToDie() > 16 and FightRemains < 60 and (Target:DebuffRemains(S.SkyreachExhaustionDebuff) < 2 or Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 55) and S.Serenity:CooldownUp() and EnemiesCount8y < 3) then
    if Cast(S.InvokeXuenTheWhiteTiger, Settings.Windwalker.GCDasOffGCD.InvokeXuenTheWhiteTiger, nil, not Target:IsInRange(40)) then addonTable.cast("Invoke Xuen, the White Tiger") return "invoke_xuen_the_white_tiger cd_serenity 6"; end
  end
  ...
  if S.BonedustBrew:IsCastable() and (Player:BuffUp(S.InvokersDelightBuff) or Player:BuffDown(S.BonedustBrewBuff) and S.InvokeXuenTheWhiteTiger:CooldownDown() and not XuenActive or S.Serenity:CooldownRemains() > 15 or FightRemains < 30 and FightRemains > 10 or BossFightRemains < 10) then
    if Cast(S.BonedustBrew, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(40)) then addonTable.cast("Bonedust Brew") return "bonedust_brew cd_serenity 10"; end
  end
  ...
  if S.Serenity:IsCastable() and (VarSyncSerenity and (Player:BuffUp(S.InvokersDelightBuff) or VarHoldXuen and (S.DrinkingHornCover:IsAvailable() and FightRemains > 110 or not S.DrinkingHornCover:IsAvailable() and FightRemains > 105)) or not S.InvokeXuenTheWhiteTiger:IsAvailable() or BossFightRemains < 15) then
    if Cast(S.Serenity, Settings.Windwalker.OffGCDasOffGCD.Serenity) then addonTable.cast("Serenity") return "serenity cd_serenity 12"; end
  end
  ...
  if S.Serenity:IsCastable() and (not VarSyncSerenity and (Player:BuffUp(S.InvokersDelightBuff) or S.InvokeXuenTheWhiteTiger:CooldownRemains() > FightRemains or FightRemains > (S.InvokeXuenTheWhiteTiger:CooldownRemains() + 10) and FightRemains > 90)) then
    if Cast(S.Serenity, Settings.Windwalker.OffGCDasOffGCD.Serenity) then addonTable.cast("Serenity") return "serenity cd_serenity 14"; end
  end
  -- Touch of Death handling
  if S.TouchofDeath:CooldownUp() then
    local ToDTar = nil
    ...
    if ToDTar and ComboStrike(S.TouchofDeath) then
      if VarDungeonRoute then
        ...
        if (ToDTar:TimeToDie() > 60 or ToDTar:DebuffUp(S.BonedustBrewDebuff) or BossFightRemains < 10) and Player:BuffDown(S.SerenityBuff) then
          if ToDTar:GUID() == Target:GUID() then
            if Cast(S.TouchofDeath, Settings.Windwalker.GCDasOffGCD.TouchOfDeath, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Touch of Death") return "touch_of_death cd_sef main-target 18"; end
          else
        ...
        if Player:BuffDown(S.SerenityBuff) then
          if ToDTar:GUID() == Target:GUID() then
            if Cast(S.TouchofDeath, Settings.Windwalker.GCDasOffGCD.TouchOfDeath, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Touch of Death") return "touch_of_death cd_sef main-target 20"; end
          else
  ...
  if not Settings.Windwalker.IgnoreToK and S.TouchofKarma:IsCastable() and (FightRemains > 90 or BossFightRemains < 10) then
    if Cast(S.TouchofKarma, Settings.Windwalker.GCDasOffGCD.TouchOfKarma) then addonTable.cast("Touch of Karma") return "touch_of_karma cd_serenity 22"; end
  end
  if (Player:BuffUp(S.SerenityBuff) or BossFightRemains < 20) then
    -- ancestral_call,if=buff.serenity.up|fight_remains<20
    if S.AncestralCall:IsCastable() then
      if Cast(S.AncestralCall, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Ancestral Call") return "ancestral_call cd_serenity 24"; end
    end
    -- blood_fury,if=buff.serenity.up|fight_remains<20
    if S.BloodFury:IsCastable() then
      if Cast(S.BloodFury, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Blood Fury") return "blood_fury cd_serenity 26"; end
    end
    -- fireblood,if=buff.serenity.up|fight_remains<20
    if S.Fireblood:IsCastable() then
      if Cast(S.Fireblood, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Fireblood") return "fireblood cd_serenity 28"; end
    end
    -- berserking,if=buff.serenity.up|fight_remains<20
    if S.Berserking:IsCastable() then
      if Cast(S.Berserking, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Berserking") return "berserking cd_serenity 30"; end
    end
    -- bag_of_tricks,if=buff.serenity.up|fight_remains<20
    if S.BagofTricks:IsCastable() then
      if Cast(S.BagofTricks, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Bag of Tricks") return "bag_of_tricks cd_serenity 32"; end
    end
  end
  -- lights_judgment
  if S.LightsJudgment:IsCastable() then
    if Cast(S.LightsJudgment, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Light's Judgment") return "lights_judgment cd_serenity 34"; end
  end
end
...
local function SerenityAoELust()
  ...
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 1) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_aoelust 2"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (Player:HasTier(31, 4) and S.Thunderfist:IsAvailable()) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_aoelust 4"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 6"; end
  end
  ...
  if S.FistsofFury:IsReady() and (S.JadeIgnition:IsAvailable()) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_aoelust 10"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2) and Player:BuffUp(S.BonedustBrewBuff) and EnemiesCount8y > 4) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 12"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:HasTier(31, 2) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 14"; end
  end
  ...
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff) and Player:HasTier(31, 2) and (EnemiesCount8y > 5 and Player:BuffStack(S.TransferthePowerBuff) > 5 or EnemiesCount8y > 6 or EnemiesCount8y > 4 and not S.CraneVortex:IsAvailable() and Player:BuffStack(S.TransferthePowerBuff) > 5)) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_aoelust 16"; end
  end
  ...
  if S.FistsofFury:IsReady() and (not Player:HasTier(30, 2) and Player:BuffDown(S.InvokersDelightBuff) and Player:HasTier(31, 2) and (Player:BuffDown(S.BonedustBrewBuff) or EnemiesCount8y > 10) and (Player:BuffStack(S.TransferthePowerBuff) == 10 and not S.CraneVortex:IsAvailable() or EnemiesCount8y > 5 and S.CraneVortex:IsAvailable() and Player:BuffStack(S.TransferthePowerBuff) == 10 or EnemiesCount8y > 14 or EnemiesCount8y > 12 and not S.CraneVortex:IsAvailable())) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_aoelust 18"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and not SCKMax() and S.ShadowboxingTreads:IsAvailable()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoelust 20"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and Player:BuffRemains(S.TeachingsoftheMonasteryBuff) < 1) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoelust 22"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 24"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:HasTier(31, 2) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.BlackoutKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 26"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:HasTier(31, 2) and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoelust 28"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&set_bonus.tier31_2pc&combo_strike&!buff.blackout_reinforcement.up
  -- Note: Skipping target_if condition.
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:HasTier(31, 2) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 30"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (EnemiesCount8y < 6 and ComboStrike(S.BlackoutKick) and Player:HasTier(31, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoelust 32"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_aoelust 34"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:BuffUp(S.CalltoDominanceBuff) and EnemiesCount8y < 10) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, EvaluateTargetIfSotWL, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "strike_of_the_windlord serenity_aoelust 36"; end
  end
  ...
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 2) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_aoelust 38"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_aoelust 40"; end
  end
  ...
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff)) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_aoelust 42"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 46"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (EnemiesCount8y < 6 and ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoelust 48"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 50"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnemiesCount8y == 5) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane102, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_aoelust 52"; end
  end
  ...
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind serenity_aoelust 54"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and EnemiesCount8y >= 3 and ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoelust 56"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains
  if S.StrikeoftheWindlord:IsReady() then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_aoelust 58"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoelust 60"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity_aoelust 62"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind serenity_aoelust 64"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoelust 66"; end
  end
  ...
  if S.TigerPalm:IsReady() and (S.TeachingsoftheMonastery:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_aoelust 68"; end
  end
end
...
local function SerenityLust()
  -- jadefire_stomp,if=debuff.jadefire_brand_damage.remains<1
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 1) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_lust 2"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:BuffRemains(S.SerenityBuff) < 1.5 and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 4"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&buff.teachings_of_the_monastery.stack=3&buff.teachings_of_the_monastery.remains<1
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and Player:BuffRemains(S.TeachingsoftheMonasteryBuff) < 1) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_lust 6"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2) and EnemiesCount8y > 2) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 8"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (ComboStrike(S.RisingSunKick) and Player:BuffUp(S.KicksofFlowingMomentumBuff) and EnemiesCount8y == 1) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_lust 9"; end
  end
  ...
  if S.FistsofFury:IsReady() and (Player:HasTier(29, 4) and EnemiesCount8y == 1) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, EvaluateTargetIfSotWL2, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_lust 10"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_lust 11"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff) and EnemiesCount8y > 2) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_lust 12"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (ComboStrike(S.RisingSunKick) and (EnemiesCount8y < 3 or not Player:HasTier(31, 2))) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_lust 14"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.FistsofFury) and S.ShadowboxingTreads:IsAvailable() and Player:HasTier(31, 2) and not S.DanceofChiji:IsAvailable()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_lust 16"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.FistsofFury) and Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 55 and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 18"; end
  end
  ...
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff) and (EnemiesCount8y < 3 or not Player:HasTier(31, 2))) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_lust 20"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and SCKMax() and Player:BuffDown(S.BlackoutReinforcementBuff) and EnemiesCount8y > 2 and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 22"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and SCKMax() and Player:BuffUp(S.BlackoutReinforcementBuff) and EnemiesCount8y > 2 and Player:PrevGCD(1, S.BlackoutKick) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 24"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and SCKMax() and Player:BuffUp(S.BonedustBrewBuff) and EnemiesCount8y > 2 and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 26"; end
  end
  ...
  -- Note: Skipping target_if condition.
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 30"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:BuffUp(S.BonedustBrewBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 32"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_lust 34"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up
  -- Note: Skipping target_if condition.
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_lust 36"; end
  end
  -- blackout_kick,target_if=max:debuff.keefers_skyreach.remains,if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_lust 38"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity_lust 40"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=talent.teachings_of_the_monastery&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (S.TeachingsoftheMonastery:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_lust 42"; end
  end
end
...
local function SerenityAoE()
  -- jadefire_stomp,if=debuff.jadefire_brand_damage.remains<1
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 1) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_aoe 2"; end
  end
  -- strike_of_the_windlord,if=set_bonus.tier31_4pc&talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (Player:HasTier(31, 4) and S.Thunderfist:IsAvailable()) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_aoe 4"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&set_bonus.tier31_2pc
  -- Note: Skipping target_if condition.
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 6"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&!buff.blackout_reinforcement.up&set_bonus.tier31_2pc&buff.bonedust_brew.up&active_enemies>4
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2) and Player:BuffUp(S.BonedustBrewBuff) and EnemiesCount8y > 4) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 8"; end
  end
  -- fists_of_fury,target_if=max:debuff.keefers_skyreach.remains,if=talent.jade_ignition,interrupt_if=buff.chi_energy.stack=30
  if Player:IsChanneling(S.FistsofFury) and Player:BuffStack(S.ChiEnergyBuff) == 30 then
  ...
  if S.FistsofFury:IsReady() and (S.JadeIgnition:IsAvailable()) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_aoe 12"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&set_bonus.tier31_2pc&combo_strike&!buff.blackout_reinforcement.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:HasTier(31, 2) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 14"; end
  end
  ...
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff) and Player:HasTier(31, 2) and (Player:BuffDown(S.BonedustBrewBuff) or EnemiesCount8y > 10) and (Player:BuffStack(S.TransferthePowerBuff) == 10 and not S.CraneVortex:IsAvailable() or EnemiesCount8y > 5 and S.CraneVortex:IsAvailable() and Player:BuffStack(S.TransferthePowerBuff) == 10 or EnemiesCount8y > 14 or EnemiesCount8y > 12 and not S.CraneVortex:IsAvailable())) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_aoe 16"; end
  end
  ...
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&!spinning_crane_kick.max&talent.shadowboxing_treads
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and not SCKMax() and S.ShadowboxingTreads:IsAvailable()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoe 20"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&buff.teachings_of_the_monastery.stack=3&buff.teachings_of_the_monastery.remains<1
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and Player:BuffRemains(S.TeachingsoftheMonasteryBuff) < 1) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoe 22"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 24"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&set_bonus.tier31_2pc&combo_strike&buff.blackout_reinforcement.up&prev.blackout_kick&buff.dance_of_chiji.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:HasTier(31, 2) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.BlackoutKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 26"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier31_2pc&combo_strike&buff.blackout_reinforcement.up
  if S.BlackoutKick:IsReady() and (Player:HasTier(31, 2) and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoe 28"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:HasTier(31, 2) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.BlackoutKick)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 30"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.pressure_point.up&set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_aoe 32"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_aoe 34"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist&buff.call_to_dominance.up&debuff.skyreach_exhaustion.remains>buff.call_to_dominance.remains&active_enemies<10
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:BuffUp(S.CalltoDominanceBuff) and EnemiesCount8y < 10) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, EvaluateTargetIfSotWL, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_aoe 36"; end
  end
  -- jadefire_stomp,if=debuff.jadefire_brand_damage.remains<2
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 2) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_aoe 38"; end
  end
  -- fists_of_fury,target_if=max:debuff.keefers_skyreach.remains,if=buff.invokers_delight.up,interrupt=1
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff)) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_aoe 40"; end
  end
  -- fists_of_fury_cancel,target_if=max:debuff.keefers_skyreach.remains
  if Player:IsChanneling(S.FistsofFury) then
  ...
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_aoe 44"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 46"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=active_enemies<6&combo_strike&set_bonus.tier30_2pc
  if S.BlackoutKick:IsReady() and (EnemiesCount8y < 6 and ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoe 48"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&spinning_crane_kick.max
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 50"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=!debuff.skyreach_exhaustion.up*20&combo_strike&active_enemies=5
  -- Note: I believe the '!debuff.skyreach_exhaustion.up*20' is supposed to be part of the target_if clause, as it makes no sense in the if clause.
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and EnemiesCount8y == 5) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane102, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_aoe 52"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind serenity_aoe 54"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=talent.shadowboxing_treads&active_enemies>=3&combo_strike
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and EnemiesCount8y >= 3 and ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoe 56"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains
  if S.StrikeoftheWindlord:IsReady() then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_aoe 58"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_aoe 60"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity_aoe 62"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  -- Note: Already covered 5 lines higher. No need to add it again.
  -- blackout_kick,target_if=max:debuff.keefers_skyreach.remains,if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_aoe 64"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=talent.teachings_of_the_monastery&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (S.TeachingsoftheMonastery:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_aoe 66"; end
  end
end
...
local function Serenity4T()
  -- jadefire_stomp,if=debuff.jadefire_brand_damage.remains<1
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 1) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_4t 2"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&buff.serenity.remains<1.5&combo_strike&!buff.blackout_reinforcement.remains&set_bonus.tier31_2pc
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:BuffRemains(S.SerenityBuff) < 1.5 and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_4t 4"; end
  end
  -- strike_of_the_windlord,if=set_bonus.tier31_4pc&talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (Player:HasTier(31, 4) and S.Thunderfist:IsAvailable()) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_4t 6"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_4t 8"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:HasTier(31, 2) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_4t 12"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&buff.teachings_of_the_monastery.stack=3&buff.teachings_of_the_monastery.remains<1
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and Player:BuffRemains(S.TeachingsoftheMonasteryBuff) < 1) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_4t 14"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier31_2pc&combo_strike&buff.blackout_reinforcement.up
  if S.BlackoutKick:IsReady() and (Player:HasTier(31, 2) and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_4t 16"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:HasTier(31, 2) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and S.CraneVortex:IsAvailable()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_4t 18"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and not S.BonedustBrew:IsAvailable()) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_4t 20"; end
  end
  ...
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff) and Player:HasTier(31, 2) and Player:BuffStack(S.TransferthePowerBuff) > 5 and not S.CraneVortex:IsAvailable() and Player:BloodlustUp()) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_4t 22"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.pressure_point.up&set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_4t 24"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_4t 26"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&set_bonus.tier31_2pc&combo_strike&!buff.blackout_reinforcement.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:HasTier(31, 2) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_4t 28"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:BuffUp(S.CalltoDominanceBuff)) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, EvaluateTargetIfSotWL, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_4t 30"; end
  end
  -- jadefire_stomp,if=debuff.jadefire_brand_damage.remains<2
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 2) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_4t 32"; end
  end
  -- fists_of_fury,target_if=max:debuff.keefers_skyreach.remains,if=buff.invokers_delight.up,interrupt=1
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff)) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_4t 34"; end
  end
  -- fists_of_fury_cancel,target_if=max:debuff.keefers_skyreach.remains
  if Player:IsChanneling(S.FistsofFury) then
  ...
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_4t 38"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!buff.blackout_reinforcement.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_4t 40"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_4t 42"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_4t 44"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&spinning_crane_kick.max
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_4t 46"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_4t 48"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains
  if S.StrikeoftheWindlord:IsReady() then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_4t 50"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_4t 52"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity_4t 54"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind serenity_4t 56"; end
  end
  -- blackout_kick,target_if=max:debuff.keefers_skyreach.remains,if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_4t 58"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=talent.teachings_of_the_monastery&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (S.TeachingsoftheMonastery:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_4t 60"; end
  end
end
...
local function Serenity3T()
  -- jadefire_stomp,if=debuff.jadefire_brand_damage.remains<1
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 1) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_3t 2"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier31_2pc&combo_strike&buff.blackout_reinforcement.up
  if S.BlackoutKick:IsReady() and (Player:HasTier(31, 2) and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_3t 4"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm)) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane102, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_3t 6"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and SCKMax() and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_3t 8"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist&buff.call_to_dominance.up&debuff.skyreach_exhaustion.remains>buff.call_to_dominance.remains
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:BuffUp(S.CalltoDominanceBuff)) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, EvaluateTargetIfSotWL, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_3t 10"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, EvaluateTargetIfSotWL2, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_3t 12"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_3t 14"; end
  end
  ...
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff) and not Player:HasTier(31, 2)) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_3t 16"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and SCKMax() and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2) and S.CraneVortex:IsAvailable()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_3t 20"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and SCKMax() and Player:BuffUp(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2) and Player:PrevGCD(1, S.BlackoutKick) and S.CraneVortex:IsAvailable()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_3t 22"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffDown(S.PressurePointBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_3t 24"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.pressure_point.up
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_3t 26"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.pressure_point.up&set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_3t 28"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_3t 30"; end
  end
  -- jadefire_stomp,if=debuff.jadefire_brand_damage.remains<2
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 2) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_3t 32"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_3t 34"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and SCKMax() and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_3t 36"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&set_bonus.tier30_2pc
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_3t 38"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&!buff.blackout_reinforcement.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_3t 40"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 2) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_3t 42"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=talent.shadowboxing_treads&combo_strike
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_3t 44"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains
  if S.StrikeoftheWindlord:IsReady() then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_3t 46"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_3t 47"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity_3t 48"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind serenity_3t 50"; end
  end
  -- blackout_kick,target_if=max:debuff.keefers_skyreach.remains,if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_3t 52"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=talent.teachings_of_the_monastery&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (S.TeachingsoftheMonastery:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_3t 54"; end
  end
end
...
local function Serenity2T()
  ...
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 2 and not (Target:DebuffRemains(S.SkyreachExhaustionDebuff) < 2) and Target:DebuffDown(S.SkyreachExhaustionDebuff)) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_2t 2"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=!debuff.skyreach_exhaustion.up*20&combo_strike
  -- Note: I believe the '!debuff.skyreach_exhaustion.up*20' is supposed to be part of the target_if clause, as it makes no sense in the if clause.
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm)) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane102, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_2t 4"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&buff.teachings_of_the_monastery.stack=3&buff.teachings_of_the_monastery.remains<1
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and Player:BuffRemains(S.TeachingsoftheMonasteryBuff) < 1) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_2t 6"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.FistsofFury) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_2t 8"; end
  end
  ...
  if S.RisingSunKick:IsReady() then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, EvaluateTargetIfRSK, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_2t 10"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.pressure_point.up&set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_2t 12"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_2t 14"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist&buff.call_to_dominance.up&debuff.skyreach_exhaustion.remains>buff.call_to_dominance.remains
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:BuffUp(S.CalltoDominanceBuff)) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, EvaluateTargetIfSotWL, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_2t 16"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist&debuff.skyreach_exhaustion.remains>55
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, EvaluateTargetIfSotWL2, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_2t 18"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&buff.blackout_reinforcement.up
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_2t 20"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!buff.blackout_reinforcement.up&set_bonus.tier31_2pc
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_2t 22"; end
  end
  -- fists_of_fury,target_if=max:debuff.keefers_skyreach.remains,if=buff.invokers_delight.up,interrupt=1
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff)) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_2t 24"; end
  end
  -- fists_of_fury_cancel,target_if=max:debuff.keefers_skyreach.remains
  if Player:IsChanneling(S.FistsofFury) then
  ...
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_2t 28"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!buff.blackout_reinforcement.up&set_bonus.tier31_2pc
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_2t 30"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_2t 32"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&set_bonus.tier30_2pc
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_2t 34"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&buff.teachings_of_the_monastery.stack=2
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 2) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_2t 36"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownRemains() > 5 and S.ShadowboxingTreads:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 1) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_2t 38"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity_2t 40"; end
  end
  -- blackout_kick,target_if=max:debuff.keefers_skyreach.remains,if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_2t 42"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=talent.teachings_of_the_monastery&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (S.TeachingsoftheMonastery:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_2t 44"; end
  end
end
...
local function SerenityST()
  ...
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 2 and Target:DebuffDown(S.SkyreachExhaustionDebuff)) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_st 2"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:BuffRemains(S.SerenityBuff) < 1.5 and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 4)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_st 4"; end
  end
  ...
  if S.TigerPalm:IsReady() and (Target:DebuffDown(S.SkyreachExhaustionDebuff) and ComboStrike(S.TigerPalm)) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_st 6"; end
  end
  -- blackout_kick,if=combo_strike&buff.teachings_of_the_monastery.stack=3&buff.teachings_of_the_monastery.remains<1
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and Player:BuffRemains(S.TeachingsoftheMonasteryBuff) < 1) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_st 8"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:HasTier(31, 4)) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_st 10"; end
  end
  -- rising_sun_kick,if=combo_strike
  if S.RisingSunKick:IsReady() and (ComboStrike(S.RisingSunKick)) then
    if Cast(S.RisingSunKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity_st 12"; end
  end
  -- jadefire_stomp,if=debuff.jadefire_brand_damage.remains<2
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.JadefireBrandDebuff) < 2) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp serenity_st 14"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:BuffUp(S.CalltoDominanceBuff) and Target:DebuffRemains(S.SkyreachExhaustionDebuff) > Player:BuffRemains(S.CalltoDominanceBuff)) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_st 16"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 55) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_st 18"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.RisingSunKick) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_st 20"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:HasTier(31, 2) and Player:BuffUp(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.RisingSunKick)) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_st 22"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!buff.blackout_reinforcement.up&prev.fists_of_fury&set_bonus.tier31_2pc
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.FistsofFury) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_st 24"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff) and Player:PrevGCD(1, S.FistsofFury) and Player:HasTier(31, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_st 26"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2) and Player:PrevGCD(1, S.FistsofFury)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_st 27"; end
  end
  -- fists_of_fury,if=buff.invokers_delight.up,interrupt=1
  if S.FistsofFury:IsReady() and (Player:BuffUp(S.InvokersDelightBuff)) then
    if Cast(S.FistsofFury, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity_st 28"; end
  end
  ...
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&!buff.blackout_reinforcement.up&set_bonus.tier31_2pc
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_st 32"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (Target:DebuffRemains(S.SkyreachExhaustionDebuff) > Player:BuffRemains(S.CalltoDominanceBuff)) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity_st 34"; end
  end
  -- blackout_kick,if=combo_strike&set_bonus.tier30_2pc
  --if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2)) then
  --  if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_st 36"; end
  --end
  -- blackout_kick,if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity_st 38"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity_st 40"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity_st 42"; end
  end
  -- tiger_palm,if=talent.teachings_of_the_monastery&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (S.TeachingsoftheMonastery:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity_st 44"; end
  end
end
...
local function DefaultAoE()
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_aoe 2"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and not S.HitCombo:IsAvailable() and SCKMax() and Player:BuffUp(S.BonedustBrewBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_aoe 4"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_aoe 6"; end
  end
  -- whirling_dragon_punch,if=active_enemies>8
  if S.WhirlingDragonPunch:IsReady() and (EnemiesCount8y > 8) then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch default_aoe 8"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:BuffUp(S.BonedustBrewBuff) and ComboStrike(S.SpinningCraneKick) and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_aoe 10"; end
  end
  -- fists_of_fury,target_if=max:debuff.keefers_skyreach.remains
  if S.FistsofFury:IsReady() then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury default_aoe 12"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.BonedustBrewBuff) and Player:BuffUp(S.PressurePointBuff) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_aoe 14"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and S.ShadowboxingTreads:IsAvailable()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_aoe 16"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_aoe 18"; end
  end
  -- whirling_dragon_punch,if=active_enemies>=5
  if S.WhirlingDragonPunch:IsReady() and (EnemiesCount8y >= 5) then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch default_aoe 20"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.pressure_point.up&set_bonus.tier30_2pc
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier30_2pc
  ...
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2) or S.WhirlingDragonPunch:IsAvailable() and S.WhirlingDragonPunch:CooldownRemains() < 3 and S.FistsofFury:CooldownRemains() > 3 and Player:BuffDown(S.KicksofFlowingMomentumBuff)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_aoe 22"; end
  end
  ...
  if S.ExpelHarm:IsReady() and (Player:Chi() == 1 and (S.RisingSunKick:CooldownUp() or S.StrikeoftheWindlord:CooldownUp()) or Player:Chi() == 2 and S.FistsofFury:CooldownUp()) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm default_aoe 24"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and S.FistsofFury:CooldownRemains() < 5 and Player:BuffStack(S.ChiEnergyBuff) > 10) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_aoe 26"; end
  end
  ...
  if S.ChiBurst:IsCastable() and (Player:Chi() < 5 and (Player:BloodlustUp() or Player:Energy() < 50)) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst default_aoe 28"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and (S.FistsofFury:CooldownRemains() > 3 or Player:Chi() > 2) and SCKMax() and Player:BuffDown(S.BlackoutReinforcementBuff) and (Player:BloodlustUp() or Player:BuffUp(S.InvokersDelightBuff))) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_aoe 30"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2) and Player:BuffDown(S.BonedustBrewBuff) and (EnemiesCount8y < 15 and not S.CraneVortex:IsAvailable() or EnemiesCount8y < 8)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_aoe 32"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and (S.FistsofFury:CooldownRemains() > 3 or Player:Chi() > 4) and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_aoe 34"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_aoe 38"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains
  if S.StrikeoftheWindlord:IsReady() then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_aoe 40"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and ComboStrike(S.BlackoutKick) and not SCKMax()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_aoe 42"; end
  end
  ...
  if S.ChiBurst:IsCastable() and (Player:ChiDeficit() >= 1 and EnemiesCount8y == 1 or Player:ChiDeficit() >= 2) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst default_aoe 44"; end
  end
end
...
local function Default4T()
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:Chi() < 2 and (S.FistsofFury:CooldownRemains() < 1 or S.StrikeoftheWindlord:CooldownRemains() < 1) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane101, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm default_4t 2"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&spinning_crane_kick.max&!buff.blackout_reinforcement.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and SCKMax() and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_4t 4"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_4t 6"; end
  end
  -- fists_of_fury,target_if=max:debuff.keefers_skyreach.remains
  if S.FistsofFury:IsReady() then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury default_4t 8"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.bonedust_brew.up&buff.pressure_point.up&set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.BonedustBrewBuff) and Player:BuffUp(S.PressurePointBuff) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_4t 10"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=talent.shadowboxing_treads&combo_strike&buff.blackout_reinforcement.up
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_4t 12"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&buff.bonedust_brew.up&combo_strike&spinning_crane_kick.max
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:BuffUp(S.BonedustBrewBuff) and ComboStrike(S.SpinningCraneKick) and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_4t 14"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch default_4t 16"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffDown(S.BonedustBrewBuff) and Player:BuffUp(S.PressurePointBuff) and S.FistsofFury:CooldownRemains() > 5) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_4t 18"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind default_4t 20"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.teachings_of_the_monastery.stack=3&talent.shadowboxing_treads
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and S.ShadowboxingTreads:IsAvailable()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_4t 22"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_4t 24"; end
  end
  -- expel_harm,if=chi=1&(!cooldown.rising_sun_kick.remains|!cooldown.strike_of_the_windlord.remains)|chi=2&!cooldown.fists_of_fury.remains
  if S.ExpelHarm:IsReady() and (Player:Chi() == 1 and (S.RisingSunKick:CooldownUp() or S.StrikeoftheWindlord:CooldownUp()) or Player:Chi() == 2 and S.FistsofFury:CooldownUp()) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm default_4t 26"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and S.FistsofFury:CooldownRemains() > 3 and Player:BuffStack(S.ChiEnergyBuff) > 10) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_4t 28"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike&set_bonus.tier30_2pc
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_4t 30"; end
  end
  -- chi_burst,if=buff.bloodlust.up&chi<5
  -- chi_burst,if=chi<5&energy<50
  -- Note: Combining both lines.
  if S.ChiBurst:IsCastable() and (Player:Chi() < 5 and (Player:BloodlustUp() or Player:Energy() < 50)) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst default_4t 32"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&(cooldown.fists_of_fury.remains>3|chi>4)&spinning_crane_kick.max
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and (S.FistsofFury:CooldownRemains() > 3 or Player:Chi() > 4) and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_4t 34"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.teachings_of_the_monastery.stack=3
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_4t 36"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains
  if S.StrikeoftheWindlord:IsReady() then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_4t 40"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and (S.FistsofFury:CooldownRemains() > 3 or Player:Chi() > 4)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_4t 42"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_4t 44"; end
  end
end
...
local function Default3T()
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:Chi() < 2 and (S.RisingSunKick:CooldownRemains() < 1 or S.FistsofFury:CooldownRemains() < 1 or S.StrikeoftheWindlord:CooldownRemains() < 1) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane101, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm default_3t 2"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!buff.blackout_reinforcement.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_3t 4"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist&set_bonus.tier31_4pc
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:HasTier(31, 4)) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_3t 6"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 20 or BossFightRemains < 5)) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_3t 8"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.teachings_of_the_monastery.stack=3&talent.shadowboxing_treads
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and S.ShadowboxingTreads:IsAvailable()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_3t 10"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=talent.shadowboxing_treads&combo_strike&buff.blackout_reinforcement.up
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_3t 12"; end
  end
  -- fists_of_fury,target_if=max:debuff.keefers_skyreach.remains
  if S.FistsofFury:IsReady() then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury default_3t 14"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.bonedust_brew.up&buff.pressure_point.up&set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.BonedustBrewBuff) and Player:BuffUp(S.PressurePointBuff) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_3t 16"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:BuffUp(S.BonedustBrewBuff) and ComboStrike(S.SpinningCraneKick)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_3t 18"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffDown(S.BonedustBrewBuff) and Player:BuffUp(S.PressurePointBuff)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_3t 20"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_3t 22"; end
  end
  -- expel_harm,if=chi=1&(!cooldown.rising_sun_kick.remains|!cooldown.strike_of_the_windlord.remains)|chi=2&!cooldown.fists_of_fury.remains
  if S.ExpelHarm:IsReady() and (Player:Chi() == 1 and (S.RisingSunKick:CooldownUp() or S.StrikeoftheWindlord:CooldownUp()) or Player:Chi() == 2 and S.FistsofFury:CooldownUp()) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm default_3t 24"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 2) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_3t 26"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains
  if S.StrikeoftheWindlord:IsReady() then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_3t 28"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.TeachingsoftheMonasteryBuff) and (S.ShadowboxingTreads:IsAvailable() or S.RisingSunKick:CooldownRemains() > 1)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_3t 30"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch default_3t 32"; end
  end
  -- chi_burst,if=buff.bloodlust.up&chi<5
  -- chi_burst,if=chi<5&energy<50
  -- Note: Combining both lines.
  if S.ChiBurst:IsCastable() and (Player:Chi() < 5 and (Player:BloodlustUp() or Player:Energy() < 50)) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst default_3t 34"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.teachings_of_the_monastery.stack=3
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_3t 36"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and S.FistsofFury:CooldownRemains() < 3 and Player:BuffStack(S.ChiEnergyBuff) > 15) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_3t 38"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (S.FistsofFury:CooldownRemains() > 4 and Player:Chi() > 3) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_3t 40"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and S.RisingSunKick:CooldownDown() and S.FistsofFury:CooldownDown() and Player:Chi() > 4 and ((S.StormEarthAndFire:IsAvailable() and not S.BonedustBrew:IsAvailable()) or S.Serenity:IsAvailable())) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_3t 42"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.FistsofFury:CooldownDown()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_3t 44"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind default_3t 46"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.ShadowboxingTreads:IsAvailable() and not SCKMax()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_3t 48"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and (ComboStrike(S.SpinningCraneKick) and (Player:Chi() > 5 and S.StormEarthAndFire:IsAvailable() or Player:Chi() > 4 and S.Serenity:IsAvailable()))) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_3t 50"; end
  end
end
...
local function Default2T()
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains+(debuff.skyreach_exhaustion.up*20),if=combo_strike&chi<2&(cooldown.rising_sun_kick.remains<1|cooldown.fists_of_fury.remains<1|cooldown.strike_of_the_windlord.remains<1)&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:Chi() < 2 and (S.RisingSunKick:CooldownRemains() < 1 or S.FistsofFury:CooldownRemains() < 1 or S.StrikeoftheWindlord:CooldownRemains() < 1) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane101, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm default_2t 2"; end
  end
  -- expel_harm,if=chi=1&(!cooldown.rising_sun_kick.remains|!cooldown.strike_of_the_windlord.remains)|chi=2&!cooldown.fists_of_fury.remains
  if S.ExpelHarm:IsReady() and (Player:Chi() == 1 and (S.RisingSunKick:CooldownUp() or S.StrikeoftheWindlord:CooldownUp()) or Player:Chi() == 2 and S.FistsofFury:CooldownUp()) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm default_2t 4"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.teachings_of_the_monastery.stack=3&talent.shadowboxing_treads
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and S.ShadowboxingTreads:IsAvailable()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_2t 6"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist&set_bonus.tier31_4pc
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and Player:HasTier(31, 4)) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_2t 8"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains,if=talent.thunderfist&(cooldown.invoke_xuen_the_white_tiger.remains>20|fight_remains<5)
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable() and (S.InvokeXuenTheWhiteTiger:CooldownRemains() > 20 or BossFightRemains < 5)) then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_2t 10"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!buff.blackout_reinforcement.up&set_bonus.tier31_2pc
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff) and Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_2t 12"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=talent.shadowboxing_treads&combo_strike&buff.blackout_reinforcement.up
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.BlackoutReinforcementBuff)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_2t 13"; end
  end
  ...
  if S.FistsofFury:IsReady() and (not Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies8y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury default_2t 14"; end
  end
  -- fists_of_fury,target_if=max:debuff.keefers_skyreach.remains
  if S.FistsofFury:IsReady() then
    if Everyone.CastTargetIf(S.FistsofFury, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury default_2t 16"; end
  end
  -- rising_sun_kick,if=!cooldown.fists_of_fury.remains
  if S.RisingSunKick:IsReady() and (S.FistsofFury:CooldownUp()) then
    if Cast(S.RisingSunKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_2t 18"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=set_bonus.tier30_2pc
  if S.RisingSunKick:IsReady() and (Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_2t 20"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.KicksofFlowingMomentumBuff) or Player:BuffUp(S.PressurePointBuff)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, EvaluateTargetIfRSK2, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_2t 22"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!buff.blackout_reinforcement.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_2t 24"; end
  end
  -- chi_burst,if=buff.bloodlust.up&chi<5
  if S.ChiBurst:IsCastable() and (Player:BloodlustUp() and Player:Chi() < 5) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst default_2t 26"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.teachings_of_the_monastery.stack=2
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 2) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_2t 28"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and Player:Chi() > 2 and Player:PrevGCD(1, S.RisingSunKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_2t 30"; end
  end
  -- chi_burst,if=chi<5&energy<50
  if S.ChiBurst:IsCastable() and (Player:Chi() < 5 and Player:Energy() < 50) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst default_2t 32"; end
  end
  -- strike_of_the_windlord,target_if=max:debuff.keefers_skyreach.remains
  if S.StrikeoftheWindlord:IsReady() then
    if Everyone.CastTargetIf(S.StrikeoftheWindlord, Enemies5y, "max", EvaluateTargetIfFilterSkyreach, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_2t 34"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.teachings_of_the_monastery.up&(talent.shadowboxing_treads|cooldown.rising_sun_kick.remains>1)
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.TeachingsoftheMonasteryBuff) and (S.ShadowboxingTreads:IsAvailable() or S.RisingSunKick:CooldownRemains() > 1)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_2t 36"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch default_2t 38"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=buff.teachings_of_the_monastery.stack=3
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_2t 40"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (not S.ShadowboxingTreads:IsAvailable() and S.FistsofFury:CooldownRemains() > 4 and S.XuensBattlegear:IsAvailable()) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_2t 42"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and S.RisingSunKick:CooldownDown() and S.FistsofFury:CooldownDown() and (Player:BuffDown(S.BonedustBrewBuff) or SCKModifier() < 1.5)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_2t 44"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind default_2t 46"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:BuffUp(S.BonedustBrewBuff) and ComboStrike(S.SpinningCraneKick) and SCKModifier() >= 2.7) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_2t 48"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains
  if S.RisingSunKick:IsReady() then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_2t 50"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains-spinning_crane_kick.max*(target.time_to_die+debuff.keefers_skyreach.remains*20),if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_2t 52"; end
  end
  -- jadefire_stomp,if=combo_strike
  if S.JadefireStomp:IsCastable() and (ComboStrike(S.JadefireStomp)) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp default_2t 54"; end
  end
end
...
local function DefaultST()
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains+(debuff.skyreach_exhaustion.up*20),if=combo_strike&chi<2&(cooldown.rising_sun_kick.remains<1|cooldown.fists_of_fury.remains<1|cooldown.strike_of_the_windlord.remains<1)&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:Chi() < 2 and (S.RisingSunKick:CooldownRemains() < 1 or S.FistsofFury:CooldownRemains() < 1 or S.StrikeoftheWindlord:CooldownRemains() < 1) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane101, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm default_st 2"; end
  end
  ...
  if S.ExpelHarm:IsReady() and (Player:Chi() == 1 and (S.RisingSunKick:CooldownUp() or S.StrikeoftheWindlord:CooldownUp()) or Player:Chi() == 2 and S.FistsofFury:CooldownUp() and S.RisingSunKick:CooldownDown()) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(20)) then addonTable.cast("Expel Harm") return "expel_harm default_st 4"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (Player:BuffUp(S.DomineeringArroganceBuff) and S.Thunderfist:IsAvailable() and S.Serenity:IsAvailable() and S.InvokeXuenTheWhiteTiger:CooldownRemains() > 20 or BossFightRemains < 5 or S.Thunderfist:IsAvailable() and Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 10 and Player:BuffDown(S.DomineeringArroganceBuff) or S.Thunderfist:IsAvailable() and Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 35 and not S.Serenity:IsAvailable()) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_st 6"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&set_bonus.tier31_2pc&!buff.blackout_reinforcement.up
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and Player:HasTier(31, 2) and Player:BuffDown(S.BlackoutReinforcementBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_st 10"; end
  end
  -- rising_sun_kick,if=!cooldown.fists_of_fury.remains
  if S.RisingSunKick:IsReady() and (S.FistsofFury:CooldownUp()) then
    if Cast(S.RisingSunKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_st 12"; end
  end
  ...
  if S.FistsofFury:IsReady() and (Player:BuffDown(S.PressurePointBuff) and Target:DebuffRemains(S.SkyreachExhaustionDebuff) < 55 and (Target:DebuffRemains(S.JadefireBrandDebuff) > 2 or S.JadefireStomp:CooldownDown())) then
    if Cast(S.FistsofFury, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury default_st 14"; end
  end
  ...
  if S.JadefireStomp:IsCastable() and (Target:DebuffRemains(S.SkyreachExhaustionDebuff) < 1 and Target:DebuffRemains(S.JadefireBrandDebuff) < 3) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp default_st 16"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) or Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 55) then
    if Cast(S.RisingSunKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_st 18"; end
  end
  -- blackout_kick,if=buff.pressure_point.remains&chi>2&prev.rising_sun_kick
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.PressurePointBuff) and Player:Chi() > 2 and Player:PrevGCD(1, S.RisingSunKick)) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Blackout Kick") return "blackout_kick default_st 20"; end
  end
  -- blackout_kick,if=buff.teachings_of_the_monastery.stack=3
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3) and Player:Chi() > 2 then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Blackout Kick") return "blackout_kick default_st 22"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.BlackoutReinforcementBuff) and S.RisingSunKick:CooldownDown() and ComboStrike(S.BlackoutKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_st 24"; end
  end
  -- rising_sun_kick
  if S.RisingSunKick:IsReady() then
    if Cast(S.RisingSunKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick default_st 26"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.BlackoutReinforcementBuff) and ComboStrike(S.BlackoutKick)) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_st 28"; end
  end
  -- fists_of_fury
  if S.FistsofFury:IsReady() then
    if Cast(S.FistsofFury, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury default_st 30"; end
  end
  ...
  if S.WhirlingDragonPunch:IsReady() and (Player:BuffDown(S.PressurePointBuff)) then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch default_st 32"; end
  end
  -- chi_burst,if=buff.bloodlust.up&chi<5
  if S.ChiBurst:IsCastable() and (Player:BloodlustUp() and Player:Chi() < 5) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst default_st 34"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 2 and Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 1) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_st 36"; end
  end
  -- chi_burst,if=chi<5&energy<50
  if S.ChiBurst:IsCastable() and (Player:Chi() < 5 and Player:Energy() < 50) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst default_st 38"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (Target:DebuffRemains(S.SkyreachExhaustionDebuff) > 30 or BossFightRemains < 5) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord default_st 40"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&combo_strike&buff.dance_of_chiji.up&!set_bonus.tier31_2pc
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and not Player:HasTier(31, 2)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_st 42"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (Player:BuffUp(S.TeachingsoftheMonasteryBuff) and S.RisingSunKick:CooldownRemains() > 1) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_st 44"; end
  end
  -- spinning_crane_kick,target_if=max:target.time_to_die,if=target.time_to_die>duration&buff.bonedust_brew.up&combo_strike&spinning_crane_kick.modifier>=2.7
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and Player:BuffUp(S.BonedustBrewBuff) and ComboStrike(S.SpinningCraneKick) and SCKModifier() >= 2.7) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick default_st 46"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch default_st 48"; end
  end
  -- rushing_jade_wind,if=!buff.rushing_jade_wind.up
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff)) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind default_st 50"; end
  end
  -- blackout_kick,if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick default_st 52"; end
  end
end
...
local function Fallthru()
  ...
  if S.CracklingJadeLightning:IsReady() and (Player:BuffStack(S.TheEmperorsCapacitorBuff) > 19 and EnergyTimeToMaxRounded() > S.CracklingJadeLightning:ExecuteTime() - 1 and S.RisingSunKick:CooldownRemains() > S.CracklingJadeLightning:ExecuteTime() or Player:BuffStack(S.TheEmperorsCapacitorBuff) > 14 and (S.Serenity:CooldownRemains() < 5 and S.Serenity:IsAvailable() or BossFightRemains < 5)) then
    if Cast(S.CracklingJadeLightning, nil, nil, not Target:IsSpellInRange(S.CracklingJadeLightning)) then addonTable.cast("Crackling Jade Lightning") return "crackling_jade_lightning fallthru 2"; end
  end
  -- jadefire_stomp,if=combo_strike
  if S.JadefireStomp:IsCastable() and (ComboStrike(S.JadefireStomp)) then
    if Cast(S.JadefireStomp, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp fallthru 4"; end
  end
  ...
  if S.TigerPalm:IsReady() and (ComboStrike(S.TigerPalm) and Player:ChiDeficit() >= (2 + num(Player:BuffUp(S.PowerStrikesBuff)))) then
    if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane101, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm fallthru 6"; end
  end
  -- expel_harm,if=chi.max-chi>=1&active_enemies>2
  if S.ExpelHarm:IsReady() and (Player:ChiDeficit() >= 1 and EnemiesCount8y > 2) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm fallthru 8"; end
  end
  ...
  if S.ChiBurst:IsCastable() and (Player:ChiDeficit() >= 1 and EnemiesCount8y == 1 or Player:ChiDeficit() >= 2 and EnemiesCount8y >= 2) then
    if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst fallthru 10"; end
  end
  -- chi_wave
  if S.ChiWave:IsCastable() then
    if Cast(S.ChiWave, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Wave") return "chi_wave fallthru 12"; end
  end
  -- expel_harm,if=chi.max-chi>=1
  if S.ExpelHarm:IsReady() and (Player:ChiDeficit() >= 1) then
    if Cast(S.ExpelHarm, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Expel Harm") return "expel_harm fallthru 14"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and EnemiesCount8y >= 5) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane103, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick fallthru 16"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (FightRemains > (Player:SpellHaste() * 1.5) and ComboStrike(S.SpinningCraneKick) and Player:BuffStack(S.ChiEnergyBuff) > 30 - 5 * EnemiesCount8y and Player:BuffDown(S.StormEarthAndFireBuff) and (S.RisingSunKick:CooldownRemains() > 2 and S.FistsofFury:CooldownRemains() > 2 or S.RisingSunKick:CooldownRemains() < 3 and S.FistsofFury:CooldownRemains() > 3 and Player:Chi() > 3 or S.RisingSunKick:CooldownRemains() > 3 and S.FistsofFury:CooldownRemains() < 3 and Player:Chi() > 4 or Player:ChiDeficit() <= 1 and EnergyTimeToMaxRounded() < 2) or Player:BuffStack(S.ChiEnergyBuff) > 10 and BossFightRemains < 7) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick fallthru 18"; end
  end
  -- arcane_torrent,if=chi.max-chi>=1
  if S.ArcaneTorrent:IsCastable() and (Player:ChiDeficit() >= 1) then
    if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials) then addonTable.cast("Arcane Torrent") return "arcane_torrent fallthru 20"; end
  end
  -- flying_serpent_kick,interrupt=1
  if S.FlyingSerpentKick:IsCastable() and not Settings.Windwalker.IgnoreFSK then
    if Cast(S.FlyingSerpentKick, nil, nil, not Target:IsInRange(30)) then addonTable.cast("Flying Serpent Kick") return "flying_serpent_kick fallthru 22"; end
  end
  -- tiger_palm
  if S.TigerPalm:IsReady() then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm fallthru 24"; end
  end
end
...
local function Serenity()
  ...
  if S.FistsofFury:IsReady() and (Player:BuffRemains(S.SerenityBuff) < 1) then
    if Cast(S.FistsofFury, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity 2"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and not SCKMax() and EnemiesCount8y > 4 and S.ShadowboxingTreads:IsAvailable()) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity 4"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 3 and Player:BuffRemains(S.TeachingsoftheMonasteryBuff) < 1) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity 6"; end
  end
  ...
  if S.RisingSunKick:IsReady() and ((EnemiesCount8y == 4 and Player:BuffUp(S.PressurePointBuff) and not S.BonedustBrew:IsAvailable()) or EnemiesCount8y == 1 or (EnemiesCount8y <= 3 and Player:BuffUp(S.PressurePointBuff)) or (Player:BuffUp(S.PressurePointBuff) and Player:HasTier(30, 2))) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity 8"; end
  end
  ...
  if S.FistsofFury:IsReady() and ((Player:BuffUp(S.InvokersDelightBuff) and (EnemiesCount8y < 3 and S.JadeIgnition:IsAvailable() or EnemiesCount8y > 4)) or Player:BloodlustUp() or EnemiesCount8y == 2) then
    if Cast(S.FistsofFury, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Fists of Fury") return "fists_of_fury serenity 10"; end
  end
  ...
  if S.StrikeoftheWindlord:IsReady() and (S.Thunderfist:IsAvailable()) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity 2"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff) and EnemiesCount8y >= 2) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity 14"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (EnemiesCount8y == 4 and Player:BuffUp(S.PressurePointBuff)) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity 16"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (EnemiesCount8y == 3 and ComboStrike(S.BlackoutKick) and Player:HasTier(30, 2)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity "; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and EnemiesCount8y >= 3 and SCKMax()) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity 20"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and EnemiesCount8y > 1 and EnemiesCount8y < 4 and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 2) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity 18"; end
  end
  ...
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff) and EnemiesCount8y >= 5) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind serenity 22"; end
  end
  -- blackout_kick,target_if=min:debuff.mark_of_the_crane.remains,if=talent.shadowboxing_treads&active_enemies>=3&combo_strike
  if S.BlackoutKick:IsReady() and (S.ShadowboxingTreads:IsAvailable() and EnemiesCount8y >= 3 and ComboStrike(S.BlackoutKick)) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity 24"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and (EnemiesCount8y > 3 or EnemiesCount8y > 2 and SCKModifier() >= 2.3)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity 26"; end
  end
  -- strike_of_the_windlord,if=active_enemies>=3
  if S.StrikeoftheWindlord:IsReady() and (EnemiesCount8y >= 3) then
    if Cast(S.StrikeoftheWindlord, nil, nil, not Target:IsInMeleeRange(9)) then addonTable.cast("Strike of the Windlord") return "strike_of_the_windlord serenity 28"; end
  end
  ...
  if S.RisingSunKick:IsReady() and (EnemiesCount8y == 2 and S.FistsofFury:CooldownRemains() > 5) then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity 30"; end
  end
  ...
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick) and EnemiesCount8y == 2 and S.FistsofFury:CooldownRemains() > 5 and S.ShadowboxingTreads:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) == 1) then
    if Everyone.CastTargetIf(S.BlackoutKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity 32"; end
  end
  ...
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and EnemiesCount8y > 1) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity 34"; end
  end
  -- whirling_dragon_punch,if=active_enemies>1
  if S.WhirlingDragonPunch:IsReady() and (EnemiesCount8y > 1) then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity 36"; end
  end
  ...
  if S.RushingJadeWind:IsReady() and (Player:BuffDown(S.RushingJadeWindBuff) and EnemiesCount8y >= 3) then
    if Cast(S.RushingJadeWind, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Rushing Jade Wind") return "rushing_jade_wind serenity 38"; end
  end
  -- rising_sun_kick,target_if=min:debuff.mark_of_the_crane.remains
  if S.RisingSunKick:IsReady() then
    if Everyone.CastTargetIf(S.RisingSunKick, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane100, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Rising Sun Kick") return "rising_sun_kick serenity 40"; end
  end
  -- spinning_crane_kick,if=combo_strike&buff.dance_of_chiji.up
  if S.SpinningCraneKick:IsReady() and (ComboStrike(S.SpinningCraneKick) and Player:BuffUp(S.DanceofChijiBuff)) then
    if Cast(S.SpinningCraneKick, nil, nil, not Target:IsInMeleeRange(8)) then addonTable.cast("Spinning Crane Kick") return "spinning_crane_kick serenity 42"; end
  end
  -- blackout_kick,if=combo_strike
  if S.BlackoutKick:IsReady() and (ComboStrike(S.BlackoutKick)) then
    if Cast(S.BlackoutKick, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Blackout Kick") return "blackout_kick serenity 44"; end
  end
  -- whirling_dragon_punch
  if S.WhirlingDragonPunch:IsReady() then
    if Cast(S.WhirlingDragonPunch, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Whirling Dragon Punch") return "whirling_dragon_punch serenity 46"; end
  end
  -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=talent.teachings_of_the_monastery&buff.teachings_of_the_monastery.stack<3
  if S.TigerPalm:IsReady() and (S.TeachingsoftheMonastery:IsAvailable() and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3) then
    if Cast(S.TigerPalm, nil, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm serenity 48"; end
  end
end
...
addonTable.resetPixels()
  ...
  if Everyone.TargetIsValid() then
    ...
    if not Settings.Windwalker.IgnoreFSK and Player:PrevGCD(1, S.FlyingSerpentKick) then
      if Cast(S.FlyingSerpentKickLand) then addonTable.cast("Flying Serpent Kick") return "flying_serpent_kick land"; end
    end
    ...
    if S.FortifyingBrew:IsReady() and Settings.Windwalker.ShowFortifyingBrewCD and Player:HealthPercentage() <= Settings.Windwalker.FortifyingBrewHP then
      if Cast(S.FortifyingBrew, Settings.Windwalker.GCDasOffGCD.FortifyingBrew, nil, not Target:IsSpellInRange(S.FortifyingBrew)) then addonTable.cast("Fortifying Brew") return "fortifying_brew main 2"; end
    end
    ...
    if S.JadefireStomp:IsCastable() and (ComboStrike(S.JadefireStomp) and S.JadefireHarmony:IsAvailable()) then
      if Everyone.CastTargetIf(S.JadefireStomp, Enemies8y, "min", EvaluateTargetIfFilterFaeExposure, EvaluateTargetIfJadefireStomp, not Target:IsInRange(30)) then addonTable.cast("Jadefire Stomp") return "jadefire_stomp main 8"; end
    end
    ...
    if S.BonedustBrew:IsCastable() and (EnemiesCount8y == 1 and Target:DebuffDown(S.SkyreachExhaustionDebuff) and (XuenActive or S.InvokeXuenTheWhiteTiger:CooldownDown())) then
      if Cast(S.BonedustBrew, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsInRange(40)) then addonTable.cast("Bonedust Brew") return "bonedust_brew main 9"; end
    end
    ...
    if S.TigerPalm:IsReady() and (Player:BuffDown(S.SerenityBuff) and Player:Energy() > 50 and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3 and ComboStrike(S.TigerPalm) and Player:ChiDeficit() >= (2 + num(Player:BuffUp(S.PowerStrikesBuff))) and (not S.InvokeXuenTheWhiteTiger:IsAvailable() and not S.Serenity:IsAvailable() or ((not S.Skyreach:IsAvailable() and not S.Skytouch:IsAvailable()) or HL.CombatTime() > 5 or XuenActive)) and not VarHoldTPRSK and (EnemiesCount8y > 1 or not S.BonedustBrew:IsAvailable() or S.BonedustBrew:IsAvailable() and EnemiesCount8y == 1 and S.BonedustBrew:CooldownDown())) then
      if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane101, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm main 10"; end
    end
    ...
    if S.TigerPalm:IsReady() and (Player:BuffDown(S.SerenityBuff) and Player:BuffStack(S.TeachingsoftheMonasteryBuff) < 3 and ComboStrike(S.TigerPalm) and Player:ChiDeficit() >= (2 + num(Player:BuffUp(S.PowerStrikesBuff))) and (not S.InvokeXuenTheWhiteTiger:IsAvailable() and not S.Serenity:IsAvailable() or ((not S.Skyreach:IsAvailable() and not S.Skytouch:IsAvailable()) or HL.CombatTime() > 5 or XuenActive)) and not VarHoldTPRSK and (EnemiesCount8y > 1 or not S.BonedustBrew:IsAvailable() or S.BonedustBrew:IsAvailable() and EnemiesCount8y == 1 and S.BonedustBrew:CooldownDown())) then
      if Everyone.CastTargetIf(S.TigerPalm, Enemies5y, "min", EvaluateTargetIfFilterMarkoftheCrane101, nil, not Target:IsInMeleeRange(5)) then addonTable.cast("Tiger Palm") return "tiger_palm main 12"; end
    end
    ...
    if S.ChiBurst:IsCastable() and (S.JadefireStomp:IsAvailable() and S.JadefireStomp:CooldownDown() and (Player:ChiDeficit() >= 1 and EnemiesCount8y == 1 or Player:ChiDeficit() >= 2 and EnemiesCount8y >= 2) and not S.JadefireHarmony:IsAvailable()) then
      if Cast(S.ChiBurst, nil, nil, not Target:IsInRange(40)) then addonTable.cast("Chi Burst") return "chi_burst main 14"; end
    end
    ...
    if S.SummonWhiteTigerStatue:IsCastable() then
      if Cast(S.SummonWhiteTigerStatue, Settings.CommonsOGCD.GCDasOffGCD.SummonWhiteTigerStatue, nil, not Target:IsInRange(40)) then addonTable.cast("Summon White Tiger Statue") return "summon_white_tiger_statue main 16"; end
    end
