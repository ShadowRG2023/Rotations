...
addonTable.spells = {
  ...
  { spell = "SPELL Glaive Tempest", name = "Glaive Tempest" },
  { spell = "SPELL Felblade", name = "Felblade" },
  { spell = "SPELL Fel Rush", name = "Fel Rush" },
  ...
  { spell = "SPELL Blur", name = "Blur" },
  { spell = "SPELL Blade Dance", name = "Blade Dance" },
  { spell = "SPELL Arcane Torrent", name = "Arcane Torrent" },
  { spell = "SPELL Annihilation", name = "Annihilation" },
  { spell = "SPELL Lifestorm", name = "Lifestorm" },
  -- INTERRUPTS
...
local VarFuryGen = 0
local VarGCDDrain = 0
local GCD = Player:GCD() or 0.75
local GCDMax = GCD + 0.25
local CombatTime = 0
...
local function Opener()
  ...
  if CDsON() then
    ...
    if S.TheHunt:IsCastable() and (Target:DebuffDown(S.EssenceBreakDebuff) and CombatTime > 0.25) then
      if Cast(S.TheHunt, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsSpellInRange(S.TheHunt)) then addonTable.cast("The Hunt") return "the_hunt opener 6"; end
    end
  end
  ...
  if S.EyeBeam:IsReady() and (Target:DebuffDown(S.EssenceBreakDebuff) and Player:BuffDown(S.InnerDemonBuff) and (Player:BuffDown(S.MetamorphosisBuff) or S.BladeDance:CooldownDown())) then
    if Cast(S.EyeBeam, Settings.Havoc.GCDasOffGCD.EyeBeam, nil, not IsInMeleeRange(20)) then addonTable.cast("Eye Beam") return "eye_beam opener 8"; end
  end
  -- vengeful_retreat/essence break coombo
  if S.VengefulRetreat:IsCastable() and addonTable.config.Veng and S.EssenceBreak:IsCastable() and (not Player:IsChanneling(S.EyeBeam)) then
...
local function Meta()
  ...
  if S.DeathSweep:IsReady() and (Player:BuffRemains(S.MetamorphosisBuff) < GCDMax) then
    if Cast(S.DeathSweep, nil, nil, not IsInMeleeRange(8)) then addonTable.cast("Death Sweep") return "death_sweep meta 2"; end
  end
  ...
  if S.Annihilation:IsReady() and (Player:BuffRemains(S.MetamorphosisBuff) < GCDMax) then
    if Cast(S.Annihilation, nil, nil, not IsInMeleeRange(5)) then addonTable.cast("Annihilation") return "annihilation meta 4"; end
  end
  -- fel_rush,if=buff.unbound_chaos.up&talent.inertia
  -- fel_rush,if=talent.momentum&buff.momentum.remains<gcd.max*2
  if S.FelRush:IsCastable() and UseFelRush() and ((Player:BuffUp(S.UnboundChaosBuff) and S.Inertia:IsAvailable()) or (S.Momentum:IsAvailable() and Player:BuffRemains(S.MomentumBuff) < GCDMax * 2)) then
  ...
  if S.Annihilation:IsReady() and (Player:BuffUp(S.InnerDemonBuff) and (S.EyeBeam:CooldownRemains() < GCDMax * 3 and S.BladeDance:CooldownUp() or S.Metamorphosis:CooldownRemains() < GCDMax * 3)) then
    if Cast(S.Annihilation, nil, nil, not IsInMeleeRange(5)) then addonTable.cast("Annihilation") return "annihilation meta 10"; end
  end
  ...
  if S.EyeBeam:IsReady() and (Target:DebuffDown(S.EssenceBreakDebuff) and Player:BuffDown(S.InnerDemonBuff)) then
    if Cast(S.EyeBeam, Settings.Havoc.GCDasOffGCD.EyeBeam, nil, not IsInMeleeRange(20)) then addonTable.cast("Eye Beam") return "eye_beam meta 20"; end
  end
  ...
  if S.GlaiveTempest:IsReady() and (Target:DebuffDown(S.EssenceBreakDebuff) and (S.BladeDance:CooldownRemains() > GCDMax * 2 or Player:Fury() > 60)) then
    if Cast(S.GlaiveTempest, Settings.Havoc.GCDasOffGCD.GlaiveTempest) then addonTable.cast("Glaive Tempest") return "glaive_tempest meta 22"; end
  end
  -- sigil_of_flame,if=active_enemies>2
  if S.SigilofFlame:IsCastable() and (Enemies8yCount > 2) then
    if Cast(S.SigilofFlame, nil, Settings.CommonsDS.DisplayStyle.Sigils, not Target:IsInRange(30)) then addonTable.cast("Sigil of Flame") return "sigil_of_flame meta 22"; end
  end
  ...
  if S.Annihilation:IsReady() and (S.BladeDance:CooldownRemains() > GCDMax * 2 or Player:Fury() > 60 or Player:BuffRemains(S.MetamorphosisBuff) < 5 and S.Felblade:CooldownUp()) then
    if Cast(S.Annihilation, nil, nil, not IsInMeleeRange(5)) then addonTable.cast("Annihilation") return "annihilation meta 24"; end
  end
  ...
  if S.SigilofFlame:IsCastable() and (Player:BuffRemains(S.MetamorphosisBuff) > 5) then
    if Cast(S.SigilofFlame, nil, Settings.CommonsDS.DisplayStyle.Sigils, not Target:IsInRange(30)) then addonTable.cast("Sigil of Flame") return "sigil_of_flame meta 26"; end
  end
  -- felblade
  if not Target:IsInMeleeRange(5) and S.Felblade:IsCastable() then
    if Cast(S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then addonTable.cast("Felblade") return "felblade meta 28"; end
  end
  ...
  if S.DemonsBite:IsCastable() then
    if Cast(S.DemonsBite, nil, nil, not IsInMeleeRange(5)) then addonTable.cast("Demon's Bite") return "demons_bite meta 38"; end
  end
end
...
local function FelBarrageFunc()
  ...
  if S.Annihilation:IsReady() and (Player:BuffUp(S.InnerDemonBuff)) then
    if Cast(S.Annihilation, nil, nil, not IsInMeleeRange(5)) then addonTable.cast("Annihilation") return "annihilation fel_barrage 2"; end
  end
  ...
  if S.EyeBeam:IsReady() and (Player:BuffDown(S.FelBarrage)) then
    if Cast(S.EyeBeam, Settings.Havoc.GCDasOffGCD.EyeBeam, nil, not IsInMeleeRange(20)) then addonTable.cast("Eye Beam") return "eye_beam fel_barrage 4"; end
  end
  ...
  if S.EssenceBreak:IsCastable() and (Player:BuffDown(S.FelBarrage) and Player:BuffUp(S.MetamorphosisBuff)) then
    if Cast(S.EssenceBreak, Settings.Havoc.GCDasOffGCD.EssenceBreak, nil, not IsInMeleeRange(10)) then addonTable.cast("Essence Break") return "essence_break fel_barrage 6"; end
  end
  ...
  if S.FelBarrage:IsReady() and (Player:Fury() > 100) then
    if Cast(S.FelBarrage, Settings.Havoc.GCDasOffGCD.FelBarrage, nil, not IsInMeleeRange(8)) then addonTable.cast("Fel Barrage") return "fel_barrage fel_barrage 16"; end
  end
  ...
  if S.FelRush:IsCastable() and UseFelRush() and (Player:BuffUp(S.UnboundChaosBuff) and Player:Fury() > 20 and Player:BuffUp(S.FelBarrage)) then
    if Cast(S.FelRush, nil, Settings.CommonsDS.DisplayStyle.FelRush) then addonTable.cast("Fel Rush") return "fel_rush fel_barrage 18"; end
  end
  ...
  if S.SigilofFlame:IsCastable() and (Player:FuryDeficit() > 40 and Player:BuffUp(S.FelBarrage)) then
    if Cast(S.SigilofFlame, nil, Settings.CommonsDS.DisplayStyle.Sigils, not Target:IsInRange(30)) then addonTable.cast("Sigil of Flame") return "sigil_of_flame fel_barrage 20"; end
  end
  ...
  if S.Felblade:IsCastable() and (Player:BuffUp(S.FelBarrage) and Player:FuryDeficit() > 40) then
    if Cast(S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then addonTable.cast("Felblade") return "felblade fel_barrage 22"; end
  end
  ...
  if S.DeathSweep:IsReady() and (Player:Fury() - VarGCDDrain - 35 > 0 and (Player:BuffRemains(S.FelBarrage) < 3 or VarGeneratorUp or Player:Fury() > 80 or VarFuryGen > 18)) then
    if Cast(S.DeathSweep, nil, nil, not IsInMeleeRange(8)) then addonTable.cast("Death Sweep") return "death_sweep fel_barrage 24"; end
  end
  ...
  if CDsON() and S.ArcaneTorrent:IsCastable() and (Player:FuryDeficit() > 40 and Player:BuffUp(S.FelBarrage)) then
    if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsInRange(8)) then addonTable.cast("Arcane Torrent") return "arcane_torrent fel_barrage 30"; end
  end
  -- fel_rush,if=buff.unbound_chaos.up
  if S.FelRush:IsCastable() and UseFelRush() and (Player:BuffUp(S.UnboundChaosBuff)) then
    if Cast(S.FelRush, nil, Settings.CommonsDS.DisplayStyle.FelRush) then addonTable.cast("Fel Rush") return "fel_rush fel_barrage 32"; end
  end
  ...
  if CDsON() and S.TheHunt:IsCastable() and (Player:Fury() > 40) then
    if Cast(S.TheHunt, nil, Settings.CommonsDS.DisplayStyle.Signature, not Target:IsSpellInRange(S.TheHunt)) then addonTable.cast("The Hunt") return "the_hunt fel_barrage 34"; end
  end
  -- demons_bite
  if S.DemonsBite:IsCastable() then
    if Cast(S.DemonsBite, nil, nil, not IsInMeleeRange(5)) then addonTable.cast("Demon's Bite") return "demons_bite fel_barrage 36"; end
  end
end
...
addonTable.resetPixels()
  ...
  if Everyone.TargetIsValid() then
    ...
    if S.Blur:IsCastable() and Player:HealthPercentage() <= Settings.Havoc.BlurHealthThreshold then
      if Cast(S.Blur, Settings.Havoc.OffGCDasOffGCD.Blur) then addonTable.cast("Blur") return "blur defensive"; end
    end
    ...
    if (S.EyeBeam:CooldownUp() or S.Metamorphosis:CooldownUp()) and CombatTime < 5 then
    ...
    --  if Cast(S.VengefulRetreat, true) then addonTable.cast("Vengeful Retreat") return "vengeful_retreat main 12"; end
    --end
    ...
    if S.FelRush:IsCastable() and UseFelRush() and (Player:BuffUp(S.UnboundChaosBuff) and S.Inertia:IsAvailable() and Player:BuffDown(S.InertiaBuff) and S.BladeDance:CooldownRemains() < 4 and S.EyeBeam:CooldownRemains() > 5 and (S.ImmolationAura:Charges() > 0 or S.ImmolationAura:Recharge() + 2 < S.EyeBeam:CooldownRemains() or S.EyeBeam:CooldownRemains() > Player:BuffRemains(S.UnboundChaosBuff) - 2)) then
      if Cast(S.FelRush, nil, Settings.CommonsDS.DisplayStyle.FelRush) then addonTable.cast("Fel Rush") return "fel_rush main 14"; end
    end
    ...
    if S.FelRush:IsCastable() and UseFelRush() and (S.Momentum:IsAvailable() and S.EyeBeam:CooldownRemains() < GCDMax * 2) then
      if Cast(S.FelRush, nil, Settings.CommonsDS.DisplayStyle.FelRush) then addonTable.cast("Fel Rush") return "fel_rush main 16"; end
    end
    ...
    if S.EyeBeam:IsReady() and (not S.EssenceBreak:IsAvailable() and (not S.ChaoticTransformation:IsAvailable() or S.Metamorphosis:CooldownRemains() < 5 + 3 * num(S.ShatteredDestiny:IsAvailable()) or S.Metamorphosis:CooldownRemains() > 15)) then
      if Cast(S.EyeBeam, Settings.Havoc.GCDasOffGCD.EyeBeam, nil, not IsInMeleeRange(20)) then addonTable.cast("Eye Beam") return "eye_beam main 20"; end
    end
    ...
    if S.GlaiveTempest:IsReady() then
      if Cast(S.GlaiveTempest, Settings.Havoc.GCDasOffGCD.GlaiveTempest) then addonTable.cast("Glaive Tempest") return "glaive_tempest main 26"; end
    end
    -- sigil_of_flame,if=active_enemies>3
    if S.SigilofFlame:IsCastable() and (Enemies8yCount > 3) then
      if Cast(S.SigilofFlame, nil, Settings.CommonsDS.DisplayStyle.Sigils, not Target:IsInRange(30)) then addonTable.cast("Sigil of Flame") return "sigil_of_flame main 28"; end
    end
    -- chaos_strike,if=debuff.essence_break.up
    if S.ChaosStrike:IsReady() and (Target:DebuffUp(S.EssenceBreakDebuff)) then
      if Cast(S.ChaosStrike, nil, nil, not Target:IsSpellInRange(S.ChaosStrike)) then addonTable.cast("Chaos Strike") return "chaos_strike main 30"; end
    end
    -- felblade
    if S.Felblade:IsCastable() then
      if Cast(S.Felblade, nil, nil, not Target:IsSpellInRange(S.Felblade)) then addonTable.cast("Felblade") return "felblade main 32"; end
    end
    ...
    if S.ThrowGlaive:IsReady() and (S.ThrowGlaive:FullRechargeTime() <= S.BladeDance:CooldownRemains() and S.Metamorphosis:CooldownRemains() > 5 and S.Soulscar:IsAvailable() and Player:HasTier(31, 2)) then
      if Cast(S.ThrowGlaive, Settings.Havoc.GCDasOffGCD.ThrowGlaive, nil, not Target:IsSpellInRange(S.ThrowGlaive)) then addonTable.cast("Throw Glaive") return "throw_glaive main 34"; end
    end
    ...
    if S.ThrowGlaive:IsReady() and (not Player:HasTier(31, 2) and (Enemies20yCount > 1 or S.Soulscar:IsAvailable())) then
      if Cast(S.ThrowGlaive, Settings.Havoc.GCDasOffGCD.ThrowGlaive, nil, not Target:IsSpellInRange(S.ThrowGlaive)) then addonTable.cast("Throw Glaive") return "throw_glaive main 36"; end
    end
    ...
    if S.SigilofFlame:IsCastable() and (IsInMeleeRange(8) and Target:DebuffDown(S.EssenceBreakDebuff) and (not S.FelBarrage:IsAvailable() or S.FelBarrage:CooldownRemains() > 25 or Enemies8yCount == 1)) then
      if Cast(S.SigilofFlame, nil, Settings.CommonsDS.DisplayStyle.Sigils, not Target:IsInRange(30)) then addonTable.cast("Sigil of Flame") return "sigil_of_flame main 42"; end
    end
    -- demons_bite
    if S.DemonsBite:IsCastable() then
      if Cast(S.DemonsBite, nil, nil, not Target:IsSpellInRange(S.DemonsBite)) then addonTable.cast("Demon's Bite") return "demons_bite main 44"; end
    end
    ...
    if S.FelRush:IsCastable() and UseFelRush() and (Player:BuffDown(S.UnboundChaosBuff) and S.FelRush:Recharge() < S.EyeBeam:CooldownRemains() and Target:DebuffDown(S.EssenceBreakDebuff) and (S.EyeBeam:CooldownRemains() > 8 or S.FelRush:ChargesFractional() > 1.01)) then
      if Cast(S.FelRush, nil, Settings.CommonsDS.DisplayStyle.FelRush) then addonTable.cast("Fel Rush") return "fel_rush main 46"; end
    end
    ...
    if S.ArcaneTorrent:IsCastable() and (IsInMeleeRange(8) and Target:DebuffDown(S.EssenceBreakDebuff) and Player:Fury() < 100) then
      if Cast(S.ArcaneTorrent, Settings.CommonsOGCD.OffGCDasOffGCD.Racials, nil, not Target:IsInRange(8)) then addonTable.cast("Arcane Torrent") return "arcane_torrent main 48"; end
    end
